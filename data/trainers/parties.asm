; Trainer data structure:
; - db "NAME@", TRAINERTYPE_* constants |ed together
; - 1 to 6 Pokémon:
;    * in all cases:              db level, species
;    * with TRAINERTYPE_DVS:      db atk|def dv, spd|spc dv
;    * with TRAINERTYPE_STAT_EXP: dw hp, atk, def, spd, spc
;    * with TRAINERTYPE_ITEM:     db item
;    * with TRAINERTYPE_MOVES:    db move 1, move 2, move 3, move 4
;    (TRAINERTYPE_ITEM_MOVES is just TRAINERTYPE_ITEM | TRAINERTYPE_MOVES)
; - db $ff ; end
SECTION "Enemy Trainer Parties 1", ROMX

FalknerGroup:
	; FALKNER (1)
	db "NATE@", TRAINERTYPE_VARIABLE
	
	;0
    db  0
	
	db 14, WEEDLE
	db 15, KINGLER
	db 16, MEOWTH
	db $fe
	
	;1
	db  1
	
	db 19, LAPRAS,     DOUBLESLAP, HIDDEN_POWER, LEECH_SEED, CHARM 
	db 19, WEEDLE,     HEADBUTT, BUBBLE, HIDDEN_POWER, LICK
	db 19, KINGLER,    POUND, HIDDEN_POWER, BONE_CLUB, LICK
	db 23, MEOWTH,     QUICK_ATTACK, GUST, HIDDEN_POWER, PURSUIT
	db $fe
	
	;2
	db  1
	
	db 24, LAPRAS,     DOUBLESLAP, HIDDEN_POWER, LEECH_SEED, CHARM 
	db 24, WEEDLE,     HEADBUTT, BUBBLEBEAM, HIDDEN_POWER, BITE
	db 24, KINGLER,    DOUBLESLAP, HIDDEN_POWER, BONE_CLUB, LICK
	db 29, MEOWTH,     CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, PURSUIT
	db $fe
	
	;3
	db  1
	
	db 27, LAPRAS,     DOUBLESLAP, HIDDEN_POWER, LEECH_SEED, CHARM 
	db 27, KAKUNA,     HEADBUTT, BUBBLEBEAM, HIDDEN_POWER, BITE
	db 27, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 32, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;4
	db  1
	
	db 31, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 31, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 31, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 34, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;5
	db  1
	
	db 32, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 32, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 32, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 36, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;6
	db  1
	
	db 34, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 34, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 34, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 39, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;7
	db  1
	
	db 39, NIDOQUEEN,  TAKE_DOWN, RAGE, CHARM, HIDDEN_POWER
	db 39, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 39, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 39, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 44, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;8
	db  1
	
	db 44, NIDOQUEEN,  TAKE_DOWN, RAGE, CHARM, HIDDEN_POWER
	db 44, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 44, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 44, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 50, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;9
	db  1
	
	db 49, NIDOQUEEN,  TAKE_DOWN, RAGE, CHARM, HIDDEN_POWER
	db 49, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 49, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 49, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 55, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;10
	db  1
	
	db 53, NIDOQUEEN,  TAKE_DOWN, RAGE, CHARM, HIDDEN_POWER
	db 53, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 53, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 53, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 60, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;11
	db  1
	
	db 58, NIDORAN_F,  SPIKES, CRUNCH, HIDDEN_POWER, HYPER_BEAM
	db 58, NIDOQUEEN,  TAKE_DOWN, RAGE, CHARM, HIDDEN_POWER
	db 58, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 58, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, BITE
	db 58, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONE_RUSH, SHADOW_BALL
	db 65, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, TRI_ATTACK
	db $fe
	
	;12
	db  1
	
	db 63, NIDORAN_F,  SPIKES, CRUNCH, HIDDEN_POWER, HYPER_BEAM
	db 63, NIDOQUEEN,  TAKE_DOWN, RAGE, MEGAHORN, HIDDEN_POWER
	db 63, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 63, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, CRUNCH
	db 63, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONEMERANG, SHADOW_BALL
	db 70, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, AEROBLAST
	db $fe
	
	;13
	db  1
	
	db 65, NIDORAN_F,  SPIKES, CRUNCH, HIDDEN_POWER, HYPER_BEAM
	db 65, NIDOQUEEN,  TAKE_DOWN, RAGE, MEGAHORN, HIDDEN_POWER
	db 65, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 65, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, CRUNCH
	db 65, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONEMERANG, SHADOW_BALL
	db 74, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, AEROBLAST
	db $fe
	
	;14
	db  1
	
	db 74, NIDORAN_F,  SPIKES, CRUNCH, HIDDEN_POWER, HYPER_BEAM
	db 74, NIDOQUEEN,  TAKE_DOWN, RAGE, MEGAHORN, HIDDEN_POWER
	db 74, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 74, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, CRUNCH
	db 74, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONEMERANG, SHADOW_BALL
	db 80, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, AEROBLAST
	db $fe
	
	;15
	db  1
	
	db 80, NIDORAN_F,  SPIKES, CRUNCH, HIDDEN_POWER, HYPER_BEAM
	db 80, NIDOQUEEN,  TAKE_DOWN, RAGE, MEGAHORN, HIDDEN_POWER
	db 80, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 80, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, CRUNCH
	db 80, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONEMERANG, SHADOW_BALL
	db 90, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, AEROBLAST
	db $fe
	
	;16
	db  1
	
	db 100, NIDORAN_F,  SPIKES, CRUNCH, HIDDEN_POWER, HYPER_BEAM
	db 100, NIDOQUEEN,  TAKE_DOWN, RAGE, MEGAHORN, HIDDEN_POWER
	db 100, DITTO,      HEADBUTT, CHARM, LEECH_SEED, GIGA_DRAIN 
	db 100, KAKUNA,     HEADBUTT, SURF, HIDDEN_POWER, CRUNCH
	db 100, VOLTORB,    HEADBUTT, HIDDEN_POWER, BONEMERANG, SHADOW_BALL
	db 100, PERSIAN,    CONFUSE_RAY, SONICBOOM, HIDDEN_POWER, AEROBLAST
	db $ff
	
	


WhitneyGroup:
	; WHITNEY (1)
	db "PLUTO@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 14, TENTACOOL
	db 14, SEAKING
	db 16, MAGIKARP
	db $fe
	
	;1
	db 1
	
	db 19, TENTACOOL, CONFUSION, DOUBLESLAP, DRAGONBREATH, NO_MOVE
	db 19, SEAKING,   CONFUSION, THUNDERSHOCK, CONFUSE_RAY, NO_MOVE
	db 23, MAGIKARP,  CONFUSION, TELEPORT, BEAT_UP, NO_MOVE
	db $fe
	
	;2
	db 1
	
	db 24, TENTACOOL, CONFUSION, DOUBLESLAP, DRAGONBREATH, NO_MOVE
	db 24, SEAKING,   CONFUSION, THUNDERSHOCK, CONFUSE_RAY, NO_MOVE
	db 29, GYARADOS,  PSYBEAM, TELEPORT, BEAT_UP, NO_MOVE
	db $fe
	
	;3
	db 1
	
	db 27, HAUNTER,   CURSE, PSYBEAM, BITE, NO_MOVE 
	db 27, TENTACOOL, PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 27, STARYU,    PSYBEAM, THUNDERSHOCK, CONFUSE_RAY, AMNESIA
	db 32, GYARADOS,  PSYBEAM, TELEPORT, BEAT_UP, FUTURE_SIGHT
	db $fe
	
	;4
	db 1
	
	db 31, GENGAR,    CURSE, PSYBEAM, BITE, NO_MOVE 
	db 31, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 31, STARYU,    PSYCHIC_M, THUNDERSHOCK, CONFUSE_RAY, AMNESIA
	db 34, GYARADOS,  PSYCHIC_M, TELEPORT, BEAT_UP, FUTURE_SIGHT
	db $fe
	
	;5
	db 1
	
	db 32, GENGAR,    CURSE, PSYBEAM, BITE, NO_MOVE 
	db 32, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 32, STARYU,    PSYCHIC_M, THUNDERSHOCK, CONFUSE_RAY, AMNESIA
	db 36, GYARADOS,  PSYCHIC_M, TELEPORT, BEAT_UP, FUTURE_SIGHT
	db $fe
	
	;6
	db 1
	
	db 34, GENGAR,    CURSE, SUBMISSION, BITE, NO_MOVE 
	db 34, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 34, STARYU,    PSYCHIC_M, THUNDERSHOCK, CONFUSE_RAY, AMNESIA
	db 39, GYARADOS,  PSYCHIC_M, TELEPORT, BEAT_UP, FUTURE_SIGHT
	db $fe
	
	;7
	db 1
	
	db 39, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 39, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 39, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 44, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;8
	db 1
	
	db 44, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 44, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 44, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 50, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;9
	db 1
	
	db 49, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 49, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 49, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 55, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;10
	db 1
	
	db 53, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 53, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 53, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 60, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;11
	db 1
	
	db 58, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 58, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 58, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 65, GYARADOS,  PSYCHIC_M, TELEPORT, SKULL_BASH, FUTURE_SIGHT
	db $fe
	
	;12
	db 1
	
	db 63, PRIMEAPE,  PSYCHIC, LEECH_SEED, GIGA_DRAIN, TOXIC
	db 63, ZAPDOS,    SUBMISSION, MEGA_PUNCH, NO_MOVE, NO_MOVE
	db 63, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 63, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 63, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 70, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;13
	db 1
	
	db 65, PRIMEAPE,  PSYCHIC, LEECH_SEED, GIGA_DRAIN, TOXIC
	db 65, ZAPDOS,    SUBMISSION, MEGA_PUNCH, NO_MOVE, NO_MOVE
	db 65, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 65, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 65, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 74, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;14
	db 1
	
	db 74, PRIMEAPE,  PSYCHIC, LEECH_SEED, GIGA_DRAIN, TOXIC
	db 74, ZAPDOS,    SUBMISSION, MEGA_PUNCH, NO_MOVE, NO_MOVE
	db 74, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 74, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 74, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 80, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
    ;15
	db 1
	
	db 80, PRIMEAPE,  PSYCHIC, LEECH_SEED, GIGA_DRAIN, TOXIC
	db 80, ZAPDOS,    SUBMISSION, MEGA_PUNCH, NO_MOVE, NO_MOVE
	db 80, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 80, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 80, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 90, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $fe
	
	;16
	db 1
	
	db 100, PRIMEAPE,  PSYCHIC, LEECH_SEED, GIGA_DRAIN, TOXIC
	db 100, ZAPDOS,    SUBMISSION, MEGA_PUNCH, NO_MOVE, NO_MOVE
	db 100, GENGAR,    CURSE, SUBMISSION, SHADOW_BALL, NO_MOVE 
	db 100, TENTACRUEL,PSYCHIC_M, DRAGONBREATH, NO_MOVE, NO_MOVE
	db 100, STARYU,    PSYCHIC_M, THUNDERBOLT, CONFUSE_RAY, AMNESIA
	db 100, GYARADOS,  PSYCHIC_M, TELEPORT, CRUNCH, FUTURE_SIGHT
	db $ff

BugsyGroup:
	; BUGSY (1)
	db "BUGSY@", TRAINERTYPE_MOVES
	db 14, METAPOD,    TACKLE, STRING_SHOT, HARDEN, NO_MOVE
	db 14, KAKUNA,     POISON_STING, STRING_SHOT, HARDEN, NO_MOVE
	db 16, SCYTHER,    QUICK_ATTACK, LEER, FURY_CUTTER, NO_MOVE
	db $ff ; end


MortyGroup:
	; MORTY (1)
	db "MORTY@", TRAINERTYPE_MOVES
	db 21, GASTLY,     LICK, SPITE, MEAN_LOOK, CURSE
	db 21, HAUNTER,    HYPNOSIS, MIMIC, CURSE, NIGHT_SHADE
	db 25, GENGAR,     HYPNOSIS, SHADOW_BALL, MEAN_LOOK, DREAM_EATER
	db 23, HAUNTER,    SPITE, MEAN_LOOK, MIMIC, NIGHT_SHADE
	db $ff ; end


PryceGroup:
	; PRYCE (1)
	db "PRYCE@", TRAINERTYPE_MOVES
	db 27, SEEL,       HEADBUTT, ICY_WIND, AURORA_BEAM, REST
	db 29, DEWGONG,    HEADBUTT, ICY_WIND, AURORA_BEAM, REST
	db 31, PILOSWINE,  ICY_WIND, FURY_ATTACK, MIST, BLIZZARD
	db $ff ; end


JasmineGroup:
	; JASMINE (1)
	db "JASMINE@", TRAINERTYPE_MOVES
	db 30, MAGNEMITE,  THUNDERBOLT, SUPERSONIC, SONICBOOM, THUNDER_WAVE
	db 30, MAGNEMITE,  THUNDERBOLT, SUPERSONIC, SONICBOOM, THUNDER_WAVE
	db 35, STEELIX,    SCREECH, SUNNY_DAY, ROCK_THROW, IRON_TAIL
	db $ff ; end


ChuckGroup:
	; CHUCK (1)
	db "CHUCK@", TRAINERTYPE_MOVES
	db 27, PRIMEAPE,   LEER, RAGE, KARATE_CHOP, FURY_SWIPES
	db 30, POLIWRATH,  HYPNOSIS, MIND_READER, SURF, DYNAMICPUNCH
	db $ff ; end


ClairGroup:
	; CLAIR (1)
	db "FRANCIS@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 14, PIDGEOTTO
	db 14, WEEZING
	db 16, STARMIE
	db $fe
	
	;1
	db 1
	
	db 19, POLIWAG, SMOKESCREEN, EMBER, POISON_GAS, SELFDESTRUCT
	db 19, WEEZING, DOUBLE_KICK, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db 19, PIDGEOT, MUD_SLAP, FLAME_WHEEL, NO_MOVE, NO_MOVE
	db 23, STARMIE, FIRE_PUNCH, POWDER_SNOW, WATER_GUN, NO_MOVE
	db $fe
	
	;2
	db 1
	
	db 24, POLIWAG, SMOKESCREEN, EMBER, POISON_GAS, SELFDESTRUCT
	db 24, WEEZING, MEGA_PUNCH, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db 24, CHARMELEON, FIRE_SPIN, WRAP, TOXIC, PROTECT
	db 24, PIDGEOT, DIG, FLAME_WHEEL, ROLLOUT, DEFENSE_CURL
	db 29, MR__MIME, FIRE_PUNCH, ICE_PUNCH, WATER_GUN, NO_MOVE
	db $fe
	
	;3
	db 1
	
	db 27, POLIWAG, SMOKESCREEN, EMBER, POISON_GAS, SELFDESTRUCT
	db 27, WEEZING, MEGA_PUNCH, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db 27, CHARMELEON, FIRE_SPIN, WRAP, TOXIC, PROTECT
	db 27, PIDGEOT, DIG, FLAME_WHEEL, ROLLOUT, DEFENSE_CURL
	db 32, MR__MIME, FIRE_PUNCH, ICE_PUNCH, WHIRLPOOL, NO_MOVE
	db $fe
	
	;4
	db 1
	
	db 31, POLIWAG, SMOKESCREEN, EMBER, POISON_GAS, SELFDESTRUCT
	db 31, WEEZING, MEGA_PUNCH, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db 31, CHARMELEON, FIRE_SPIN, DRAGON_RAGE, TOXIC, PROTECT
	db 31, PIDGEOT, DIG, FLAME_WHEEL, ROLLOUT, DEFENSE_CURL
	db 34, MR__MIME, FIRE_PUNCH, ICE_PUNCH, WHIRLPOOL, NO_MOVE
	db $fe
	
	;5
	db 1
	
	db 32, POLIWAG, SMOKESCREEN, EMBER, TOXIC, EXPLOSION
	db 32, WEEZING, MEGA_PUNCH, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db 32, CHARMELEON, FIRE_SPIN, DRAGON_RAGE, TOXIC, PROTECT
	db 32, PIDGEOT, DIG, FLAME_WHEEL, ROLLOUT, DEFENSE_CURL
	db 36, MR__MIME, FIRE_PUNCH, ICE_PUNCH, WHIRLPOOL, NO_MOVE
	db $fe
	
	;6
	db 1
	
	db 34, POLIWHIRL, SMOKESCREEN, FLAMETHROWER, TOXIC, EXPLOSION
	db 34, RHYHORN, MEGA_PUNCH, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db 34, CHARMELEON, FIRE_SPIN, DRAGON_RAGE, TOXIC, PROTECT
	db 34, RATTATA, MAGNITUDE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 34, GLOOM, SOFTBOILED, DRILL_PECK, FLAMETHROWER, SWIFT
	db 39, MR__MIME, FIRE_PUNCH, ICE_PUNCH, WHIRLPOOL, NO_MOVE
	db $fe
	
	;7
	db 1
	
	db 39, POLIWHIRL, LEECH_LIFE, FLAMETHROWER, TOXIC, EXPLOSION
	db 39, RHYHORN, COMET_PUNCH, FIRE_PUNCH, SUNNY_DAY, NO_MOVE
	db 39, CHARIZARD, FLAMETHROWER, DRAGON_RAGE, CRUNCH, SUNNY_DAY
	db 39, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 39, GLOOM, SOFTBOILED, DRILL_PECK, FLAMETHROWER, SWIFT
	db 44, MR__MIME, FIRE_PUNCH, ICE_PUNCH, SURF, NO_MOVE
	db $fe
	
	;8
	db 1
	
	db 44, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 44, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 44, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 44, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 44, GLOOM, SOFTBOILED, DRILL_PECK, FLAMETHROWER, SWIFT
	db 50, MR__MIME, FIRE_PUNCH, ICE_PUNCH, SURF, NO_MOVE
	db $fe
	
	;9
	db 1
	
	db 49, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 49, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 49, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 49, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 49, GLOOM, SOFTBOILED, DRILL_PECK, FLAMETHROWER, SWIFT
	db 55, MR__MIME, FIRE_PUNCH, ICE_PUNCH, SURF, NO_MOVE
	db $fe
	
	;10
	db 1
	
	db 53, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 53, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 53, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 53, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 53, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 60, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $fe
	
	;11
	db 1
	
	db 58, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 58, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 58, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 58, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 58, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 65, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $fe
	
	;12
	db 1
	
	db 63, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 63, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 63, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 63, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 63, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 70, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $fe
	
	;13
	db 1
	
	db 65, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 65, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 65, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 65, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 65, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 74, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $fe
	
	;14
	db 1
	
	db 74, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 74, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 74, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 74, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 74, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 80, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $fe
	
	;15
	db 1
	
	db 80, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 80, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 80, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, SUNNY_DAY
	db 80, RATTATA, EARTHQUAKE, FIRE_PUNCH, ROLLOUT, DEFENSE_CURL
	db 80, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 90, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $fe
	
	;16
	db 1
	
	db 100, POLIWHIRL, LEECH_LIFE, FIRE_BLAST, TOXIC, EXPLOSION
	db 100, RHYHORN, COMET_PUNCH, FIRE_BLAST, SUNNY_DAY, EARTHQUAKE
	db 100, CHARIZARD, FIRE_BLAST, SCREECH, CRUNCH, EARTHQUAKE
	db 100, RATTATA, EARTHQUAKE, FIRE_PUNCH, HYPER_FANG, SUNNY_DAY
	db 100, GLOOM, SOFTBOILED, SKY_ATTACK, FIRE_BLAST, EXTREMESPEED
	db 100, MR__MIME, FIRE_PUNCH, BARRAGE, WATERFALL, EARTHQUAKE
	db $ff

SECTION "Enemy Trainer Parties 2", ROMX

Rival1Group:
	; RIVAL1 (1)
	db "?@", TRAINERTYPE_VARIABLE
	
	db 0
	
	db  5, BULBASAUR
	db $ff; end

	; RIVAL1 (2)
	db "?@", TRAINERTYPE_VARIABLE
	
	db 0
	
	db  5, CHARMANDER
	db $ff ; end

	; RIVAL1 (3)
	db "?@", TRAINERTYPE_VARIABLE
	
	db 0

	db  5, SQUIRTLE
	db $ff ; end

	; RIVAL1 (4)
	db "?@", TRAINERTYPE_NORMAL
	db 12, GASTLY
	db 14, ZUBAT
	db 16, BAYLEEF
	db $ff ; end

	; RIVAL1 (5)
	db "?@", TRAINERTYPE_NORMAL
	db 12, GASTLY
	db 14, ZUBAT
	db 16, QUILAVA
	db $ff ; end

	; RIVAL1 (6)
	db "?@", TRAINERTYPE_NORMAL
	db 12, GASTLY
	db 14, ZUBAT
	db 16, CROCONAW
	db $ff ; end

	; RIVAL1 (7)
	db "?@", TRAINERTYPE_MOVES
	db 20, HAUNTER,    LICK, SPITE, MEAN_LOOK, CURSE
	db 18, MAGNEMITE,  TACKLE, THUNDERSHOCK, SUPERSONIC, SONICBOOM
	db 20, ZUBAT,      LEECH_LIFE, SUPERSONIC, BITE, CONFUSE_RAY
	db 22, BAYLEEF,    GROWL, REFLECT, RAZOR_LEAF, POISONPOWDER
	db $ff ; end

	; RIVAL1 (8)
	db "?@", TRAINERTYPE_MOVES
	db 20, HAUNTER,    LICK, SPITE, MEAN_LOOK, CURSE
	db 18, MAGNEMITE,  TACKLE, THUNDERSHOCK, SUPERSONIC, SONICBOOM
	db 20, ZUBAT,      LEECH_LIFE, SUPERSONIC, BITE, CONFUSE_RAY
	db 22, QUILAVA,    LEER, SMOKESCREEN, EMBER, QUICK_ATTACK
	db $ff ; end

	; RIVAL1 (9)
	db "?@", TRAINERTYPE_MOVES
	db 20, HAUNTER,    LICK, SPITE, MEAN_LOOK, CURSE
	db 18, MAGNEMITE,  TACKLE, THUNDERSHOCK, SUPERSONIC, SONICBOOM
	db 20, ZUBAT,      LEECH_LIFE, SUPERSONIC, BITE, CONFUSE_RAY
	db 22, CROCONAW,   LEER, RAGE, WATER_GUN, BITE
	db $ff ; end

	; RIVAL1 (10)
	db "?@", TRAINERTYPE_MOVES
	db 30, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 28, MAGNEMITE,  TACKLE, THUNDERSHOCK, SONICBOOM, THUNDER_WAVE
	db 30, HAUNTER,    LICK, MEAN_LOOK, CURSE, SHADOW_BALL
	db 32, SNEASEL,    LEER, QUICK_ATTACK, SCREECH, FAINT_ATTACK
	db 32, MEGANIUM,   REFLECT, RAZOR_LEAF, POISONPOWDER, BODY_SLAM
	db $ff ; end

	; RIVAL1 (11)
	db "?@", TRAINERTYPE_MOVES
	db 30, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 28, MAGNEMITE,  TACKLE, THUNDERSHOCK, SONICBOOM, THUNDER_WAVE
	db 30, HAUNTER,    LICK, MEAN_LOOK, CURSE, SHADOW_BALL
	db 32, SNEASEL,    LEER, QUICK_ATTACK, SCREECH, FAINT_ATTACK
	db 32, QUILAVA,    SMOKESCREEN, EMBER, QUICK_ATTACK, FLAME_WHEEL
	db $ff ; end

	; RIVAL1 (12)
	db "?@", TRAINERTYPE_MOVES
	db 30, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 28, MAGNEMITE,  TACKLE, THUNDERSHOCK, SONICBOOM, THUNDER_WAVE
	db 30, HAUNTER,    LICK, MEAN_LOOK, CURSE, SHADOW_BALL
	db 32, SNEASEL,    LEER, QUICK_ATTACK, SCREECH, FAINT_ATTACK
	db 32, FERALIGATR, RAGE, WATER_GUN, BITE, SCARY_FACE
	db $ff ; end

	; RIVAL1 (13)
	db "?@", TRAINERTYPE_MOVES
	db 34, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 35, MAGNETON,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 35, HAUNTER,    MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 35, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db 38, MEGANIUM,   REFLECT, RAZOR_LEAF, POISONPOWDER, BODY_SLAM
	db $ff ; end

	; RIVAL1 (14)
	db "?@", TRAINERTYPE_MOVES
	db 34, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 35, MAGNETON,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 35, HAUNTER,    MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 35, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db 38, TYPHLOSION, SMOKESCREEN, EMBER, QUICK_ATTACK, FLAME_WHEEL
	db $ff ; end

	; RIVAL1 (15)
	db "?@", TRAINERTYPE_MOVES
	db 34, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 34, MAGNETON,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 35, HAUNTER,    MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 35, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db 38, FERALIGATR, RAGE, WATER_GUN, SCARY_FACE, SLASH
	db $ff ; end

SECTION "Enemy Trainer Parties 3", ROMX

PokemonProfGroup:


WillGroup:
	; WILL (1)
	db "WILL@", TRAINERTYPE_MOVES
	db 40, XATU,       QUICK_ATTACK, FUTURE_SIGHT, CONFUSE_RAY, PSYCHIC_M
	db 41, JYNX,       DOUBLESLAP, LOVELY_KISS, ICE_PUNCH, PSYCHIC_M
	db 41, EXEGGUTOR,  REFLECT, LEECH_SEED, EGG_BOMB, PSYCHIC_M
	db 41, SLOWBRO,    CURSE, AMNESIA, BODY_SLAM, PSYCHIC_M
	db 42, XATU,       QUICK_ATTACK, FUTURE_SIGHT, CONFUSE_RAY, PSYCHIC_M
	db $ff ; end


PKMNTrainerGroup:
	; CAL (1)
	db "CAL@", TRAINERTYPE_NORMAL
	db 10, CHIKORITA
	db 10, CYNDAQUIL
	db 10, TOTODILE
	db $ff ; end

	; CAL (2)
	db "CAL@", TRAINERTYPE_NORMAL
	db 30, BAYLEEF
	db 30, QUILAVA
	db 30, CROCONAW
	db $ff ; end

	; CAL (3)
	db "CAL@", TRAINERTYPE_NORMAL
	db 50, MEGANIUM
	db 50, TYPHLOSION
	db 50, FERALIGATR
	db $ff ; end


BrunoGroup:
	; BRUNO (1)
	db "BRUNO@", TRAINERTYPE_MOVES
	db 42, HITMONTOP,  PURSUIT, QUICK_ATTACK, DIG, DETECT
	db 42, HITMONLEE,  SWAGGER, DOUBLE_KICK, HI_JUMP_KICK, FORESIGHT
	db 42, HITMONCHAN, THUNDERPUNCH, ICE_PUNCH, FIRE_PUNCH, MACH_PUNCH
	db 43, ONIX,       BIND, EARTHQUAKE, SANDSTORM, ROCK_SLIDE
	db 46, MACHAMP,    ROCK_SLIDE, FORESIGHT, VITAL_THROW, CROSS_CHOP
	db $ff ; end


KarenGroup:
	; KAREN (1)
	db "KAREN@", TRAINERTYPE_MOVES
	db 42, UMBREON,    SAND_ATTACK, CONFUSE_RAY, FAINT_ATTACK, MEAN_LOOK
	db 42, VILEPLUME,  STUN_SPORE, ACID, MOONLIGHT, PETAL_DANCE
	db 45, GENGAR,     LICK, SPITE, CURSE, DESTINY_BOND
	db 44, MURKROW,    QUICK_ATTACK, WHIRLWIND, PURSUIT, FAINT_ATTACK
	db 47, HOUNDOOM,   ROAR, PURSUIT, FLAMETHROWER, CRUNCH
	db $ff ; end


KogaGroup:
	; KOGA (1)
	db "KOGA@", TRAINERTYPE_MOVES
	db 40, ARIADOS,    DOUBLE_TEAM, SPIDER_WEB, BATON_PASS, GIGA_DRAIN
	db 41, VENOMOTH,   SUPERSONIC, GUST, PSYCHIC_M, TOXIC
	db 43, FORRETRESS, PROTECT, SWIFT, EXPLOSION, SPIKES
	db 42, MUK,        MINIMIZE, ACID_ARMOR, SLUDGE_BOMB, TOXIC
	db 44, CROBAT,     DOUBLE_TEAM, QUICK_ATTACK, WING_ATTACK, TOXIC
	db $ff ; end


ChampionGroup:
	; CHAMPION (1)
	db "LANCE@", TRAINERTYPE_MOVES
	db 44, GYARADOS,   FLAIL, RAIN_DANCE, SURF, HYPER_BEAM
	db 47, DRAGONITE,  THUNDER_WAVE, TWISTER, THUNDER, HYPER_BEAM
	db 47, DRAGONITE,  THUNDER_WAVE, TWISTER, BLIZZARD, HYPER_BEAM
	db 46, AERODACTYL, WING_ATTACK, ANCIENTPOWER, ROCK_SLIDE, HYPER_BEAM
	db 46, CHARIZARD,  FLAMETHROWER, WING_ATTACK, SLASH, HYPER_BEAM
	db 50, DRAGONITE,  FIRE_BLAST, SAFEGUARD, OUTRAGE, HYPER_BEAM
	db $ff ; end

SECTION "Enemy Trainer Parties 4", ROMX	

BrockGroup:
	; BROCK (1)
	db "BROCK@", TRAINERTYPE_MOVES
	db 41, GRAVELER,   DEFENSE_CURL, ROCK_SLIDE, ROLLOUT, EARTHQUAKE
	db 41, RHYHORN,    FURY_ATTACK, SCARY_FACE, EARTHQUAKE, HORN_DRILL
	db 42, OMASTAR,    BITE, SURF, PROTECT, SPIKE_CANNON
	db 44, ONIX,       BIND, ROCK_SLIDE, BIDE, SANDSTORM
	db 42, KABUTOPS,   SLASH, SURF, ENDURE, GIGA_DRAIN
	db $ff ; end


MistyGroup:
	; MISTY (1)
	db "MISTY@", TRAINERTYPE_MOVES
	db 42, GOLDUCK,    SURF, DISABLE, PSYCH_UP, PSYCHIC_M
	db 42, QUAGSIRE,   SURF, AMNESIA, EARTHQUAKE, RAIN_DANCE
	db 44, LAPRAS,     SURF, PERISH_SONG, BLIZZARD, RAIN_DANCE
	db 47, STARMIE,    SURF, CONFUSE_RAY, RECOVER, ICE_BEAM
	db $ff ; end


LtSurgeGroup:
	; LT_SURGE (1)
	db "LT.SURGE@", TRAINERTYPE_MOVES
	db 44, RAICHU,     THUNDER_WAVE, QUICK_ATTACK, THUNDERBOLT, THUNDER
	db 40, ELECTRODE,  SCREECH, DOUBLE_TEAM, SWIFT, EXPLOSION
	db 40, MAGNETON,   LOCK_ON, DOUBLE_TEAM, SWIFT, ZAP_CANNON
	db 40, ELECTRODE,  SCREECH, DOUBLE_TEAM, SWIFT, EXPLOSION
	db 46, ELECTABUZZ, QUICK_ATTACK, THUNDERPUNCH, LIGHT_SCREEN, THUNDER
	db $ff ; end


ScientistGroup:
	; SCIENTIST (1)
	db "ROSS@", TRAINERTYPE_NORMAL
	db 22, KOFFING
	db 22, KOFFING
	db $ff ; end

	; SCIENTIST (2)
	db "MITCH@", TRAINERTYPE_NORMAL
	db 24, DITTO
	db $ff ; end

	; SCIENTIST (3)
	db "JED@", TRAINERTYPE_NORMAL
	db 20, MAGNEMITE
	db 20, MAGNEMITE
	db 20, MAGNEMITE
	db $ff ; end

	; SCIENTIST (4)
	db "MARC@", TRAINERTYPE_NORMAL
	db 27, MAGNEMITE
	db 27, MAGNEMITE
	db 27, MAGNEMITE
	db $ff ; end

	; SCIENTIST (5)
	db "RICH@", TRAINERTYPE_MOVES
	db 30, PORYGON,    CONVERSION, QUIVER_DANCE, RECOVER, TRI_ATTACK
	db $ff ; end


ErikaGroup:
	; ERIKA (1)
	db "ERIKA@", TRAINERTYPE_MOVES
	db 42, TANGELA,    VINE_WHIP, BIND, GIGA_DRAIN, SLEEP_POWDER
	db 41, JUMPLUFF,   MEGA_DRAIN, LEECH_SEED, COTTON_SPORE, GIGA_DRAIN
	db 46, VICTREEBEL, SUNNY_DAY, SYNTHESIS, ACID, RAZOR_LEAF
	db 46, BELLOSSOM,  SUNNY_DAY, SYNTHESIS, PETAL_DANCE, SOLARBEAM
	db $ff ; end

SECTION "Enemy Trainer Parties 5", ROMX

YoungsterGroup:
	; YOUNGSTER (1)
	db "JOEY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, SANDSLASH
	db  5, SANDSLASH
	db $fe
	
	;1
	db 0
	
	db 15, SANDSLASH
	db 15, SANDSLASH
    db $fe
	
	;2
	db 0
	
	db 20, SANDSLASH
	db 20, SANDSLASH
	db 20, SANDSLASH
	db $fe
	
	;3
	db 0
	
	db 25, SANDSLASH
	db 25, SANDSLASH
	db 25, SANDSLASH
	db $fe
	
	;4
	db 0
	
	db 30, SANDSLASH
	db 30, SANDSLASH
	db 30, SANDSLASH
	db $fe
	
	;5
	db 0
	
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 34, SANDSLASH
	db $fe
	
	;6
	db 0
	
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 36, SANDSLASH
	db $fe
	
	;7
	db 0
	
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 40, NIDORAN_F
	db $fe
	
	;8
	db 0
	
	db 38, SANDSLASH
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 42, NIDORAN_F
	db $fe
	
	;9
	db 0
	
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 46, SANDSLASH
	db $fe
	
	;10
	db 0
	
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 50, SANDSLASH
	db $fe
	
	;11
	db 0
	
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 60, SANDSLASH
	db $fe
	
	;12
	db 0
	
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 64, SANDSLASH
	db $fe
	
	;13
	db 0
	
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 66, SANDSLASH
	db $fe
	
	;14
	db 0
	
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 70, SANDSLASH
	db $fe
	
	;15
	db 0
	
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 74, SANDSLASH
	db $fe
	
	;16
	db 0
	
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 85, NIDORAN_F
	db 90, NIDORAN_F
	db 100, SANDSLASH
	db $ff

	;
	db "ROD@", TRAINERTYPE_VARIABLE
	
	;0
	db  0
	
	db  9, LAPRAS
	db 12, NIDORINA
	db $fe
	
	;1
	db  0
	
	db 12, LAPRAS
	db 14, NIDORINA
	db $fe
	
	;2
	db  0
	
	db 19, LAPRAS
	db 20, NIDORINA
	db $fe
	
	;3
	db  0
	
	db 24, LAPRAS
	db 25, NIDORINA
	db $fe
	
	;4
	db  0
	
	db 27, DITTO
	db 28, NIDOQUEEN
	db $fe
	
	;5
	db  0
	
	db 30, DITTO
	db 31, NIDOQUEEN
	db $fe
	
	;6
	db  0
	
	db 32, DITTO
	db 33, NIDOQUEEN
	db $fe
	
	;7
	db  0
	
	db 37, DITTO
	db 38, NIDOQUEEN
	db $fe
	
	;8
	db  0
	
	db 42, DITTO
	db 43, NIDOQUEEN
	db $fe
	
	;9
	db  0
	
	db 47, DITTO
	db 48, NIDOQUEEN
	db $fe
	
	;10
	db  0
	
	db 51, DITTO
	db 52, NIDOQUEEN
	db $fe
	
	;11
	db  0
	
	db 56, DITTO
	db 57, NIDOQUEEN
	db $fe
	
	;12
	db  0
	
	db 61, DITTO
	db 62, NIDOQUEEN
	db $fe
	
	;13
	db  0
	
	db 63, DITTO
	db 64, NIDOQUEEN
	db $fe
	
	;14
	db  0
	
	db 72, DITTO
	db 73, NIDOQUEEN
	db $fe
	
	;15
	db  0
	
	db 78, DITTO
	db 79, NIDOQUEEN
	db $fe
	
	;16
	db  0
	
	db 84, DITTO
	db 87, NIDOQUEEN
	db $ff
	
	; YOUNGSTER (2)
	db "JIM@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  4, BULBASAUR
	db  5, CHARMANDER
	db  6, SQUIRTLE
	db $fe
	
	;1
	db 3
	
	db  8, BULBASAUR, NO_ITEM, TACKLE, GROWL, NO_MOVE, NO_MOVE
	db  9, CHARMANDER, NO_ITEM, CONSTRICT, TAIL_WHIP, NO_MOVE, NO_MOVE
	db 10, SQUIRTLE, BERRY, POUND, GROWL, NO_MOVE, NO_MOVE
    db $fe
	
	;2
	db 3
	
	db 13, BULBASAUR, NO_ITEM, VINE_WHIP, HORN_ATTACK, NO_MOVE, NO_MOVE
	db 14, CHARMANDER, NO_ITEM, EMBER, WRAP, SMOKESCREEN, NO_MOVE
	db 15, SQUIRTLE, BERRY, BUBBLE, POWDER_SNOW, NO_MOVE, NO_MOVE
	db $fe
	
	;3
	db 3
	
	db 16, IVYSAUR, NO_ITEM, VINE_WHIP, HORN_ATTACK, NO_MOVE, NO_MOVE
	db 17, CHARMELEON, NO_ITEM, EMBER, WRAP, SMOKESCREEN, BITE
	db 18, WARTORTLE, BERRY, WATER_GUN, POWDER_SNOW, LICK, NO_MOVE
	db $fe
	
	;4
	db 3
	
	db 20, IVYSAUR, NO_ITEM, VINE_WHIP, HORN_ATTACK, BITE, VICEGRIP
	db 21, CHARMELEON, NO_ITEM, FIRE_SPIN, WRAP, SMOKESCREEN, BITE
	db 22, WARTORTLE, BERRY, WATER_GUN, ICY_WIND, NO_MOVE, NO_MOVE
	db $fe
	
	;5
	db 3
	
	db 24, IVYSAUR, NO_ITEM, VINE_WHIP, ROCK_THROW, BITE, VICEGRIP
	db 25, CHARMELEON, NO_ITEM, FIRE_SPIN, WRAP, SMOKESCREEN, BITE
	db 26, WARTORTLE, BERRY, BUBBLEBEAM, ICY_WIND, NO_MOVE, NO_MOVE
	db $fe
	
	;6
	db 3
	
	db 28, IVYSAUR, NO_ITEM, VINE_WHIP, ROCK_THROW, BITE, VICEGRIP
	db 29, CHARMELEON, NO_ITEM, FIRE_SPIN, WRAP, TOXIC, PROTECT
	db 30, WARTORTLE, BERRY, SURF, ICE_BEAM, RAGE, NO_MOVE
	db $fe
	
	;7
	db 3
	
	db 31, IVYSAUR, NO_ITEM, VINE_WHIP, ROCK_THROW, BITE, VICEGRIP
	db 32, CHARMELEON, NO_ITEM, FIRE_SPIN, DRAGON_RAGE, TOXIC, PROTECT
	db 33, WARTORTLE, BERRY, SURF, ICE_BEAM, RAGE, NO_MOVE
	db $fe
	
	;8
	db 3
	
	db 36, VENUSAUR, LEFTOVERS, HORN_DRILL, ROCK_THROW, BITE, VICEGRIP
	db 37, CHARIZARD, LEFTOVERS, FLAMETHROWER, DRAGON_RAGE, CRUNCH, SMOKESCREEN 
	db 38, BLASTOISE, LEFTOVERS, SURF, BARRAGE, RAGE, NO_MOVE
	db $fe
	
	;9
	db 3
	
	db 40, VENUSAUR, LEFTOVERS, HORN_DRILL, ROCK_THROW, BITE, SPIKE_CANNON
	db 41, CHARIZARD, LEFTOVERS, FLAMETHROWER, DRAGONBREATH, CRUNCH, SMOKESCREEN 
	db 42, BLASTOISE, LEFTOVERS, SURF, BARRAGE, RAGE, CRUNCH
	db $fe
	
	;10
	db 3
	
	db 43, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, BITE, SPIKE_CANNON
	db 44, CHARIZARD, LEFTOVERS, FLAMETHROWER, DRAGONBREATH, CRUNCH, SMOKESCREEN 
	db 45, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $fe
	
	;11
	db 3
	
	db 46, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, MEGAHORN, SPIKE_CANNON
	db 47, CHARIZARD, LEFTOVERS, FLAMETHROWER, DRAGONBREATH, CRUNCH, SMOKESCREEN 
	db 48, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $fe
	
	;12
	db 3
	
	db 49, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, MEGAHORN, SPIKE_CANNON
	db 50, CHARIZARD, LEFTOVERS, FLAMETHROWER, OUTRAGE, CRUNCH, SMOKESCREEN 
	db 51, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $fe
	
	;13
	db 3
	
	db 55, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, MEGAHORN, SPIKE_CANNON
	db 57, CHARIZARD, LEFTOVERS, FLAMETHROWER, OUTRAGE, CRUNCH, SMOKESCREEN 
	db 59, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $fe
	
	;14
	db 3
	
	db 65, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, MEGAHORN, SPIKE_CANNON
	db 69, CHARIZARD, LEFTOVERS, FLAMETHROWER, SCREECH, CRUNCH, SMOKESCREEN 
	db 72, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $fe
	
	;15
	db 3
	
	db 74, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, MEGAHORN, SPIKE_CANNON
	db 78, CHARIZARD, LEFTOVERS, FLAMETHROWER, SCREECH, CRUNCH, SMOKESCREEN 
	db 82, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $fe
	
	;16
	db 3
	
	db 85, VENUSAUR, LEFTOVERS, HORN_DRILL, DOUBLE_EDGE, MEGAHORN, SPIKE_CANNON
	db 90, CHARIZARD, LEFTOVERS, FLAMETHROWER, SCREECH, CRUNCH, SMOKESCREEN 
	db 95, BLASTOISE, LEFTOVERS, WATERFALL, BARRAGE, RAGE, CROSS_CHOP
	db $ff

	; YOUNGSTER (3)
	db "ALBERT@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, POLIWRATH
	db  8, TENTACOOL
	db $fe
	
	;1
	db 0
	
	db 13, POLIWRATH
	db 13, TENTACOOL
	db $fe
	
	;2
	db 0
	
	db 20, POLIWRATH
	db 20, TENTACOOL
	db $fe
	
	;3
	db 0
	
	db 24, POLIWRATH
	db 24, TENTACOOL
	db $fe
	
	;5
	db 0
	
	db 29, ABRA
	db 29, TENTACRUEL
	db $fe
	
	;6
	db 0
	
	db 32, ABRA
	db 32, TENTACRUEL
	db $fe
	
	;7
	db 0
	
	db 35, ABRA
	db 35, TENTACRUEL
	db $fe
	
	;8
	db 0
	
	db 42, ABRA
	db 42, TENTACRUEL
	db $fe
	
	;9
	db 0
	
	db 47, ABRA
	db 47, TENTACRUEL
	db $fe
	
	;10
	db 0
	
	db 51, ABRA
	db 51, TENTACRUEL
	db $fe
	
	;11
	db 0
	
	db 55, ABRA
	db 55, TENTACRUEL
	db $fe
	
	;12
	db 0
	
	db 59, ABRA
	db 59, TENTACRUEL
	db $fe
	
	;13
	db 0
	
	db 63, ABRA
	db 63, TENTACRUEL
	db $fe

	;14
	db 0
	
	db 68, ABRA
	db 68, TENTACRUEL
	db $fe
	
	;15
	db 0
	
	db 76, ABRA
	db 76, TENTACRUEL
	db $fe
	
	;16
	db 0
	
	db 80, ABRA
	db 80, TENTACRUEL
	db $ff
	
	; YOUNGSTER (4)
	db "GORDON@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, PINSIR
	db $fe
	
	;1
	db 0
	
	db 17, PINSIR
	db $fe
	
	;2
	db 0
	
	db 22, PINSIR
	db $fe
	
	;3
	db 0
	
	db 26, TAUROS
	db $fe
	
	;4
	db 0
	
	db 30, TAUROS
	db $fe
	
	;5
	db 0
	
	db 31, TAUROS
	db $fe
	
	;6
	db 0
	
	db 33, TAUROS
	db $fe

	;7
	db 0
	
	db 38, TAUROS
	db $fe
	
	;8
	db 0
	
	db 43, TAUROS
	db $fe
	
	;9
	db 0
	
	db 48, TAUROS
	db $fe
	
	;10
	db 0
	
	db 52, TAUROS
	db $fe
	
	;11
	db 0
	
	db 57, TAUROS
	db $fe
	
	;12
	db 0
	
	db 62, TAUROS
	db $fe
	
	;13
	db 0
	
	db 64, TAUROS
	db $fe
	
	;14
	db 0
	
	db 73, TAUROS
	db $fe
	
	;15
	db 0
	
	db 79, TAUROS
	db $fe
	
	;16
	db 0
	
	db 95, TAUROS
	db $ff
	
	; YOUNGSTER (5)
	db "SAMUEL@", TRAINERTYPE_NORMAL
	db  7, RATTATA
	db 10, SANDSHREW
	db  8, SPEAROW
	db  8, SPEAROW
	db $ff ; end

	; YOUNGSTER (6)
	db "IAN@", TRAINERTYPE_NORMAL
	db 10, MANKEY
	db 12, DIGLETT
	db $ff ; end

	; YOUNGSTER (7)
	db "JOEY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, SANDSLASH
	db  5, SANDSLASH
	db $fe
	
	;1
	db 0
	
	db 15, SANDSLASH
	db 15, SANDSLASH
    db $fe
	
	;2
	db 0
	
	db 20, SANDSLASH
	db 20, SANDSLASH
	db 20, SANDSLASH
	db $fe
	
	;3
	db 0
	
	db 25, SANDSLASH
	db 25, SANDSLASH
	db 25, SANDSLASH
	db $fe
	
	;4
	db 0
	
	db 30, SANDSLASH
	db 30, SANDSLASH
	db 30, SANDSLASH
	db $fe
	
	;5
	db 0
	
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 34, SANDSLASH
	db $fe
	
	;6
	db 0
	
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 36, SANDSLASH
	db $fe
	
	;7
	db 0
	
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 40, NIDORAN_F
	db $fe
	
	;8
	db 0
	
	db 38, SANDSLASH
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 42, NIDORAN_F
	db $fe
	
	;9
	db 0
	
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 46, SANDSLASH
	db $fe
	
	;10
	db 0
	
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 50, SANDSLASH
	db $fe
	
	;11
	db 0
	
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 60, SANDSLASH
	db $fe
	
	;12
	db 0
	
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 64, SANDSLASH
	db $fe
	
	;13
	db 0
	
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 66, SANDSLASH
	db $fe
	
	;14
	db 0
	
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 70, SANDSLASH
	db $fe
	
	;15
	db 0
	
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 74, SANDSLASH
	db $fe
	
	;16
	db 0
	
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 85, NIDORAN_F
	db 90, NIDORAN_F
	db 100, SANDSLASH
	db $ff

	; YOUNGSTER (8)
	db "JOEY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, SANDSLASH
	db  5, SANDSLASH
	db $fe
	
	;1
	db 0
	
	db 15, SANDSLASH
	db 15, SANDSLASH
    db $fe
	
	;2
	db 0
	
	db 20, SANDSLASH
	db 20, SANDSLASH
	db 20, SANDSLASH
	db $fe
	
	;3
	db 0
	
	db 25, SANDSLASH
	db 25, SANDSLASH
	db 25, SANDSLASH
	db $fe
	
	;4
	db 0
	
	db 30, SANDSLASH
	db 30, SANDSLASH
	db 30, SANDSLASH
	db $fe
	
	;5
	db 0
	
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 34, SANDSLASH
	db $fe
	
	;6
	db 0
	
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 36, SANDSLASH
	db $fe
	
	;7
	db 0
	
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 40, NIDORAN_F
	db $fe
	
	;8
	db 0
	
	db 38, SANDSLASH
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 42, NIDORAN_F
	db $fe
	
	;9
	db 0
	
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 46, SANDSLASH
	db $fe
	
	;10
	db 0
	
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 50, SANDSLASH
	db $fe
	
	;11
	db 0
	
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 60, SANDSLASH
	db $fe
	
	;12
	db 0
	
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 64, SANDSLASH
	db $fe
	
	;13
	db 0
	
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 66, SANDSLASH
	db $fe
	
	;14
	db 0
	
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 70, SANDSLASH
	db $fe
	
	;15
	db 0
	
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 74, SANDSLASH
	db $fe
	
	;16
	db 0
	
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 85, NIDORAN_F
	db 90, NIDORAN_F
	db 100, SANDSLASH
	db $ff

	; YOUNGSTER (9)
	db "WARREN@", TRAINERTYPE_NORMAL
	db 35, FEAROW
	db $ff ; end

	; YOUNGSTER (10)
	db "JIMMY@", TRAINERTYPE_NORMAL
	db 33, RATICATE
	db 33, ARBOK
	db $ff ; end

	; YOUNGSTER (11)
	db "OWEN@", TRAINERTYPE_NORMAL
	db 35, GROWLITHE
	db $ff ; end

	; YOUNGSTER (12)
	db "JASON@", TRAINERTYPE_NORMAL
	db 33, SANDSLASH
	db 33, CROBAT
	db $ff ; end

	; YOUNGSTER (13)
	db "JOEY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, SANDSLASH
	db  5, SANDSLASH
	db $fe
	
	;1
	db 0
	
	db 15, SANDSLASH
	db 15, SANDSLASH
    db $fe
	
	;2
	db 0
	
	db 20, SANDSLASH
	db 20, SANDSLASH
	db 20, SANDSLASH
	db $fe
	
	;3
	db 0
	
	db 25, SANDSLASH
	db 25, SANDSLASH
	db 25, SANDSLASH
	db $fe
	
	;4
	db 0
	
	db 30, SANDSLASH
	db 30, SANDSLASH
	db 30, SANDSLASH
	db $fe
	
	;5
	db 0
	
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 34, SANDSLASH
	db $fe
	
	;6
	db 0
	
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 36, SANDSLASH
	db $fe
	
	;7
	db 0
	
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 40, NIDORAN_F
	db $fe
	
	;8
	db 0
	
	db 38, SANDSLASH
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 42, NIDORAN_F
	db $fe
	
	;9
	db 0
	
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 46, SANDSLASH
	db $fe
	
	;10
	db 0
	
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 50, SANDSLASH
	db $fe
	
	;11
	db 0
	
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 60, SANDSLASH
	db $fe
	
	;12
	db 0
	
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 64, SANDSLASH
	db $fe
	
	;13
	db 0
	
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 66, SANDSLASH
	db $fe
	
	;14
	db 0
	
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 70, SANDSLASH
	db $fe
	
	;15
	db 0
	
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 74, SANDSLASH
	db $fe
	
	;16
	db 0
	
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 85, NIDORAN_F
	db 90, NIDORAN_F
	db 100, SANDSLASH
	db $ff

	; YOUNGSTER (14)
	db "JOEY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, SANDSLASH
	db  5, SANDSLASH
	db $fe
	
	;1
	db 0
	
	db 15, SANDSLASH
	db 15, SANDSLASH
    db $fe
	
	;2
	db 0
	
	db 20, SANDSLASH
	db 20, SANDSLASH
	db 20, SANDSLASH
	db $fe
	
	;3
	db 0
	
	db 25, SANDSLASH
	db 25, SANDSLASH
	db 25, SANDSLASH
	db $fe
	
	;4
	db 0
	
	db 30, SANDSLASH
	db 30, SANDSLASH
	db 30, SANDSLASH
	db $fe
	
	;5
	db 0
	
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 32, SANDSLASH
	db 34, SANDSLASH
	db $fe
	
	;6
	db 0
	
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 34, SANDSLASH
	db 36, SANDSLASH
	db $fe
	
	;7
	db 0
	
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 36, SANDSLASH
	db 40, NIDORAN_F
	db $fe
	
	;8
	db 0
	
	db 38, SANDSLASH
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 38, NIDORAN_F
	db 42, NIDORAN_F
	db $fe
	
	;9
	db 0
	
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 40, NIDORAN_F
	db 46, SANDSLASH
	db $fe
	
	;10
	db 0
	
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 44, NIDORAN_F
	db 50, SANDSLASH
	db $fe
	
	;11
	db 0
	
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 46, NIDORAN_F
	db 60, SANDSLASH
	db $fe
	
	;12
	db 0
	
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 48, NIDORAN_F
	db 64, SANDSLASH
	db $fe
	
	;13
	db 0
	
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 50, NIDORAN_F
	db 66, SANDSLASH
	db $fe
	
	;14
	db 0
	
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 55, NIDORAN_F
	db 70, SANDSLASH
	db $fe
	
	;15
	db 0
	
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 65, NIDORAN_F
	db 74, SANDSLASH
	db $fe
	
	;16
	db 0
	
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 80, NIDORAN_F
	db 85, NIDORAN_F
	db 90, NIDORAN_F
	db 100, SANDSLASH
	db $ff

SECTION "Enemy Trainer Parties 6", ROMX

SchoolboyGroup:
	; SCHOOLBOY (1)
	db "JACK@", TRAINERTYPE_NORMAL
	db 12, ODDISH
	db 15, VOLTORB
	db $ff ; end

	; SCHOOLBOY (2)
	db "KIPP@", TRAINERTYPE_NORMAL
	db 27, VOLTORB
	db 27, MAGNEMITE
	db 31, VOLTORB
	db 31, MAGNETON
	db $ff ; end

	; SCHOOLBOY (3)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 16, TANGELA
	db $ff ; end

	; SCHOOLBOY (4)
	db "JOHNNY@", TRAINERTYPE_NORMAL
	db 29, BELLSPROUT
	db 31, WEEPINBELL
	db 33, VICTREEBEL
	db $ff ; end

	; SCHOOLBOY (5)
	db "DANNY@", TRAINERTYPE_NORMAL
	db 31, JYNX
	db 31, ELECTABUZZ
	db 31, MAGMAR
	db $ff ; end

	; SCHOOLBOY (6)
	db "TOMMY@", TRAINERTYPE_NORMAL
	db 32, XATU
	db 34, ALAKAZAM
	db $ff ; end

	; SCHOOLBOY (7)
	db "DUDLEY@", TRAINERTYPE_NORMAL
	db 35, ODDISH
	db $ff ; end

	; SCHOOLBOY (8)
	db "JOE@", TRAINERTYPE_NORMAL
	db 33, TANGELA
	db 33, VAPOREON
	db $ff ; end

	; SCHOOLBOY (9)
	db "BILLY@", TRAINERTYPE_NORMAL
	db 27, PARAS
	db 27, PARAS
	db 27, POLIWHIRL
	db 35, DITTO
	db $ff ; end

	; SCHOOLBOY (10)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 19, MR__MIME
	db $ff ; end

	; SCHOOLBOY (11)
	db "NATE@", TRAINERTYPE_NORMAL
	db 32, LEDIAN
	db 32, EXEGGUTOR
	db $ff ; end

	; SCHOOLBOY (12)
	db "RICKY@", TRAINERTYPE_NORMAL
	db 32, AIPOM
	db 32, DITTO
	db $ff ; end

	; SCHOOLBOY (13)
	db "JACK@", TRAINERTYPE_NORMAL
	db 14, ODDISH
	db 17, VOLTORB
	db $ff ; end

	; SCHOOLBOY (14)
	db "JACK@", TRAINERTYPE_NORMAL
	db 28, GLOOM
	db 31, ELECTRODE
	db $ff ; end

	; SCHOOLBOY (15)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 17, TANGELA
	db 17, YANMA
	db $ff ; end

	; SCHOOLBOY (16)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 20, NATU
	db 22, TANGELA
	db 20, QUAGSIRE
	db 25, YANMA
	db $ff ; end

	; SCHOOLBOY (17)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 19, MR__MIME
	db 19, MAGNEMITE
	db $ff ; end

	; SCHOOLBOY (18)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 27, MR__MIME
	db 31, MAGNETON
	db $ff ; end

	; SCHOOLBOY (19)
	db "JACK@", TRAINERTYPE_NORMAL
	db 30, GLOOM
	db 33, GROWLITHE
	db 33, ELECTRODE
	db $ff ; end

	; SCHOOLBOY (20)
	db "JACK@", TRAINERTYPE_MOVES
	db 35, ELECTRODE,  SCREECH, SONICBOOM, ROLLOUT, LIGHT_SCREEN
	db 35, GROWLITHE,  SUNNY_DAY, LEER, TAKE_DOWN, FLAME_WHEEL
	db 37, VILEPLUME,  SOLARBEAM, SLEEP_POWDER, ACID, MOONLIGHT
	db $ff ; end

	; SCHOOLBOY (21)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 27, NATU
	db 27, TANGELA
	db 30, QUAGSIRE
	db 30, YANMA
	db $ff ; end

	; SCHOOLBOY (22)
	db "ALAN@", TRAINERTYPE_MOVES
	db 35, XATU,       PECK, NIGHT_SHADE, SWIFT, FUTURE_SIGHT
	db 32, TANGELA,    POISONPOWDER, VINE_WHIP, BIND, MEGA_DRAIN
	db 32, YANMA,      QUICK_ATTACK, DOUBLE_TEAM, SONICBOOM, SUPERSONIC
	db 35, QUAGSIRE,   TAIL_WHIP, SLAM, AMNESIA, EARTHQUAKE
	db $ff ; end

	; SCHOOLBOY (23)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 30, MR__MIME
	db 34, MAGNETON
	db $ff ; end

	; SCHOOLBOY (24)
	db "CHAD@", TRAINERTYPE_MOVES
	db 34, MR__MIME,   PSYCHIC_M, LIGHT_SCREEN, REFLECT, ENCORE
	db 38, MAGNETON,   ZAP_CANNON, THUNDER_WAVE, LOCK_ON, SWIFT
	db $ff ; end


BirdKeeperGroup:
	; BIRD_KEEPER (1)
	

	; BIRD_KEEPER (2)
	

	; BIRD_KEEPER (3)
	db "BRYAN@", TRAINERTYPE_NORMAL
	db 12, PIDGEY
	db 14, PIDGEOTTO
	db $ff ; end

	; BIRD_KEEPER (4)
	db "THEO@", TRAINERTYPE_NORMAL
	db 17, PIDGEY
	db 15, PIDGEY
	db 19, PIDGEY
	db 15, PIDGEY
	db 15, PIDGEY
	db $ff ; end

	; BIRD_KEEPER (5)
	db "TOBY@", TRAINERTYPE_NORMAL
	db 15, DODUO
	db 16, DODUO
	db 17, DODUO
	db $ff ; end

	; BIRD_KEEPER (6)
	db "DENIS@", TRAINERTYPE_NORMAL
	db 18, SPEAROW
	db 20, FEAROW
	db 18, SPEAROW
	db $ff ; end

	; BIRD_KEEPER (7)
	db "VANCE@", TRAINERTYPE_NORMAL
	db 25, PIDGEOTTO
	db 25, PIDGEOTTO
	db $ff ; end

	; BIRD_KEEPER (8)
	db "HANK@", TRAINERTYPE_NORMAL
	db 12, PIDGEY
	db 34, PIDGEOT
	db $ff ; end

	; BIRD_KEEPER (9)
	db "ROY@", TRAINERTYPE_NORMAL
	db 29, FEAROW
	db 35, FEAROW
	db $ff ; end

	; BIRD_KEEPER (10)
	db "BORIS@", TRAINERTYPE_NORMAL
	db 30, DODUO
	db 28, DODUO
	db 32, DODRIO
	db $ff ; end

	; BIRD_KEEPER (11)
	db "BOB@", TRAINERTYPE_NORMAL
	db 34, NOCTOWL
	db $ff ; end

	; BIRD_KEEPER (12)
	db "JOSE@", TRAINERTYPE_NORMAL
	db 36, FARFETCH_D
	db $ff ; end

	; BIRD_KEEPER (13)
	db "PETER@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, EEVEE
	db 10, ODDISH
	db 10, VILEPLUME
	db $fe
	
	;1
	db 0
	
	db 14, EEVEE
	db 16, ODDISH
	db 16, VILEPLUME
	db $fe
	
	;2
	db 0
	
	db 20, EEVEE
	db 21, ODDISH
	db 21, VILEPLUME
	db $fe
	
	;3
	db 0
	
	db 24, EEVEE
	db 25, ODDISH
	db 25, VILEPLUME
	db $fe
	
	;4
	db 0
	
	db 28, EEVEE
	db 29, ODDISH
	db 29, VILEPLUME
	db $fe
	
	;5
	db 0
	
	db 29, VAPOREON
	db 30, GLOOM
	db 30, PARAS
	db $fe
	
	;6
	db 0
	
	db 31, VAPOREON
	db 32, GLOOM
	db 32, PARAS
	db $fe
	
	;7
	db 0
	
	db 36, VAPOREON
	db 37, GLOOM
	db 37, PARAS
	db $fe
	
	;8
	db 0
	
	db 42, VAPOREON
	db 43, GLOOM
	db 43, PARAS
	db $fe
	
	;9
	db 0
	
	db 45, VAPOREON
	db 46, GLOOM
	db 46, PARAS
	db $fe
	
	;10
	db 0
	
	db 50, VAPOREON
	db 51, GLOOM
	db 51, PARAS
	db $fe
	
	;11
	db 0
	
	db 55, VAPOREON
	db 56, GLOOM
	db 56, PARAS
	db $fe
	
	;12
	db 0
	
	db 60, VAPOREON
	db 61, GLOOM
	db 61, PARAS
	db $fe
	
	;13
	db 0
	
	db 63, VAPOREON
	db 64, GLOOM
	db 64, PARAS
	db $fe
	
	;14
	db 0
	
	db 71, VAPOREON
	db 72, GLOOM
	db 72, PARAS
	db $fe
	
	;15
	db 0
	
	db 76, VAPOREON
	db 77, GLOOM
	db 77, PARAS
	db $fe
	
	;16
	db 0
	
	db 90, VAPOREON
	db 93, GLOOM
	db 93, PARAS
	db $ff

	; BIRD_KEEPER (14)
	db "JOSE@", TRAINERTYPE_NORMAL
	db 34, FARFETCH_D
	db $ff ; end

	; BIRD_KEEPER (15)
	db "PERRY@", TRAINERTYPE_NORMAL
	db 34, FARFETCH_D
	db $ff ; end

	; BIRD_KEEPER (16)
	db "BRET@", TRAINERTYPE_NORMAL
	db 32, PIDGEOTTO
	db 32, FEAROW
	db $ff ; end

	; BIRD_KEEPER (17)
	db "JOSE@", TRAINERTYPE_MOVES
	db 40, FARFETCH_D, FURY_ATTACK, DETECT, FLY, SLASH
	db $ff ; end

	; BIRD_KEEPER (18)
	db "VANCE@", TRAINERTYPE_NORMAL
	db 32, PIDGEOTTO
	db 32, PIDGEOTTO
	db $ff ; end

	; BIRD_KEEPER (19)
	db "VANCE@", TRAINERTYPE_MOVES
	db 38, PIDGEOT,    TOXIC, QUICK_ATTACK, WHIRLWIND, FLY
	db 38, PIDGEOT,    SWIFT, DETECT, STEEL_WING, FLY
	db $ff ; end


LassGroup:
	; LASS (1)
	db "CARRIE@", TRAINERTYPE_MOVES
	db 18, SNUBBULL,   SCARY_FACE, CHARM, BITE, LICK
	db $ff ; end

	; LASS (2)
	db "BRIDGET@", TRAINERTYPE_NORMAL
	db 15, JIGGLYPUFF
	db 15, JIGGLYPUFF
	db 15, JIGGLYPUFF
	db $ff ; end

	; LASS (3)
	db "ALICE@", TRAINERTYPE_NORMAL
	db 30, GLOOM
	db 34, ARBOK
	db 30, GLOOM
	db $ff ; end

	; LASS (4)
	db "KRISE@", TRAINERTYPE_NORMAL
	db 12, ODDISH
	db 15, CUBONE
	db $ff ; end

	; LASS (5)
	db "CONNIE@", TRAINERTYPE_NORMAL
	db 21, MARILL
	db $ff ; end

	; LASS (6)
	db "LINDA@", TRAINERTYPE_NORMAL
	db 30, BULBASAUR
	db 32, IVYSAUR
	db 34, VENUSAUR
	db $ff ; end

	; LASS (7)
	db "LAURA@", TRAINERTYPE_NORMAL
	db 28, GLOOM
	db 31, PIDGEOTTO
	db 31, BELLOSSOM
	db $ff ; end

	; LASS (8)
	db "SHANNON@", TRAINERTYPE_NORMAL
	db 29, PARAS
	db 29, PARAS
	db 32, PARASECT
	db $ff ; end

	; LASS (9)
	db "MICHELLE@", TRAINERTYPE_NORMAL
	db 32, SKIPLOOM
	db 33, HOPPIP
	db 34, JUMPLUFF
	db $ff ; end

	; LASS (10)
	db "DANA@", TRAINERTYPE_MOVES
	db 18, FLAAFFY,    TACKLE, GROWL, THUNDERSHOCK, THUNDER_WAVE
	db 18, PSYDUCK,    SCRATCH, TAIL_WHIP, DISABLE, CONFUSION
	db $ff ; end

	; LASS (11)
	db "ELLEN@", TRAINERTYPE_NORMAL
	db 30, WIGGLYTUFF
	db 34, GRANBULL
	db $ff ; end

	; LASS (12)
	db "CONNIE@", TRAINERTYPE_NORMAL
	db 21, MARILL
	db $ff ; end

	; LASS (13)
	db "CONNIE@", TRAINERTYPE_NORMAL
	db 21, MARILL
	db $ff ; end

	; LASS (14)
	db "DANA@", TRAINERTYPE_MOVES
	db 21, FLAAFFY,    TACKLE, GROWL, THUNDERSHOCK, THUNDER_WAVE
	db 21, PSYDUCK,    SCRATCH, TAIL_WHIP, DISABLE, CONFUSION
	db $ff ; end

	; LASS (15)
	db "DANA@", TRAINERTYPE_MOVES
	db 29, PSYDUCK,    SCRATCH, DISABLE, CONFUSION, SCREECH
	db 29, AMPHAROS,   TACKLE, THUNDERSHOCK, THUNDER_WAVE, COTTON_SPORE
	db $ff ; end

	; LASS (16)
	db "DANA@", TRAINERTYPE_MOVES
	db 32, PSYDUCK,    SCRATCH, DISABLE, CONFUSION, SCREECH
	db 32, AMPHAROS,   TACKLE, THUNDERPUNCH, THUNDER_WAVE, COTTON_SPORE
	db $ff ; end

	; LASS (17)
	db "DANA@", TRAINERTYPE_MOVES
	db 36, AMPHAROS,   SWIFT, THUNDERPUNCH, THUNDER_WAVE, COTTON_SPORE
	db 36, GOLDUCK,    DISABLE, SURF, PSYCHIC_M, SCREECH
	db $ff ; end


JanineGroup:
	; JANINE (1)
	db "JANINE@", TRAINERTYPE_MOVES
	db 36, CROBAT,     SCREECH, SUPERSONIC, CONFUSE_RAY, WING_ATTACK
	db 36, WEEZING,    SMOG, SLUDGE_BOMB, TOXIC, EXPLOSION
	db 36, WEEZING,    SMOG, SLUDGE_BOMB, TOXIC, EXPLOSION
	db 33, ARIADOS,    SCARY_FACE, GIGA_DRAIN, STRING_SHOT, NIGHT_SHADE
	db 39, VENOMOTH,   FORESIGHT, DOUBLE_TEAM, GUST, PSYCHIC_M
	db $ff ; end


CooltrainerMGroup:
	; COOLTRAINERM (1)
	db "NICK@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 11, BEEDRILL
	db 11, PORYGON
	db $fe
	
	;1
	db 0
	
	db 19, BEEDRILL
	db 19, PORYGON
	db $fe
	
	;2
	db 0
	
	db 23, BEEDRILL
	db 23, PORYGON
	db $fe
	
	;3
	db 0
	
	db 26, BEEDRILL
	db 26, PORYGON
	db $fe
	
	;4
	db 0
	
	db 29, PIDGEY
	db 29, OMANYTE
	db $fe
	
	;5
	db 0
	
	db 33, PIDGEY
	db 33, OMANYTE
	db $fe
	
	;6
	db 0
	
	db 38, PIDGEY
	db 38, OMANYTE
	db $fe
	
	;7
	db 0
	
	db 43, PIDGEY
	db 43, OMANYTE
	db $fe
	
	;8
	db 0
	
	db 47, PIDGEY
	db 47, OMANYTE
	db $fe
	
	;9
	db 0
	
	db 53, PIDGEY
	db 53, OMANYTE
	db $fe
	
	;10
	db 0
	
	db 57, PIDGEY
	db 57, OMANYTE
	db $fe
	
	;11
	db 0
	
	db 60, PIDGEY
	db 60, OMANYTE
	db $fe
	
	;12
	db 0
	
	db 65, PIDGEY
	db 65, OMANYTE
	db $fe
	
	;13
	db 0
	
	db 70, PIDGEY
	db 70, OMANYTE
	db $fe
	
	;14
	db 0
	
	db 73, PIDGEY
	db 73, PIDGEY
	db 73, OMANYTE
	db $fe
	
	;15
	db 0
	
	db 77, PIDGEY
	db 77, PIDGEY
	db 77, OMANYTE
	db $fe
	
	;16
	db 0
	
	db 83, PIDGEY
	db 83, PIDGEY
	db 83, OMANYTE
	db $ff

	; COOLTRAINERM (2)
	db "AARON@", TRAINERTYPE_NORMAL
	db 24, IVYSAUR
	db 24, CHARMELEON
	db 24, WARTORTLE
	db $ff ; end

	; COOLTRAINERM (3)
	db "PAUL@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 11, WEEZING
	db 12, PIDGEOTTO
	db $fe
	
	;1
	db 0
	
	db 17, WEEZING
	db 17, PIDGEOT
	db $fe
	
	;2
	db 0
	
	db 22, WEEZING
	db 22, PIDGEOT
	db $fe
	
	;3
	db 0
	
	db 26, WEEZING
	db 26, PIDGEOT
	db $fe
	
	;4
	db 0
	
	db 28, WEEZING
	db 28, PIDGEOT
	db $fe
	
	;5
	db 0
	
	db 32, RHYHORN
	db 32, RATTATA
	db $fe
	
	;6
	db 0
	
	db 36, RHYHORN
	db 36, RATTATA
	db $fe
	
	;7
	db 0
	
	db 38, RHYHORN
	db 38, RATTATA
	db $fe
	
	;8
	db 0
	
	db 40, RHYHORN
	db 40, RATTATA
	db $fe
	
	;9
	db 0
	
	db 43, RHYHORN
	db 43, RATTATA
	db $fe
	
	;10
	db 0
	
	db 46, RHYHORN
	db 46, RATTATA
	db $fe
	
	;11
	db 0
	
	db 49, RHYHORN
	db 49, RATTATA
	db $fe
    
	;12
	db 0
	
	db 52, RHYHORN
	db 54, RATTATA
	db $fe
	
	;13
	db 0
	
	db 60, RHYHORN
	db 62, RATTATA
	db $fe
	
	;14
	db 0
	
	db 64, RHYHORN
	db 67, RATTATA
	db $fe
	
	;15
	db 0
	
	db 70, RHYHORN
	db 70, RATTATA
	db $fe
	
	;16
	db 0
	
	db 80, RHYHORN
	db 85, RATTATA
	db $ff
	
	; COOLTRAINERM (4)
	db "CODY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, RHYDON
	db  8, ODDISH
	db $fe
	
	;1
	db 0
	
	db  12, RHYDON
	db  12, ODDISH
	db $fe
	
	;2
	db 0
	
	db  18, RHYDON
	db  18, ODDISH
	db $fe
	
	;3
	db 0
	
	db  23, RHYDON
	db  23, ODDISH
	db $fe
	
	;4
	db 0
	
	db 27, RHYDON
	db 27, ODDISH
	db $fe

	;5
	db 0
	
	db 28, RHYDON
	db 28, ODDISH
	db $fe
	
	;6
	db 0
	
	db 30, RHYDON
	db 30, ODDISH
	db $fe
	
	;7
	db 0
	
	db 34, CHANSEY
	db 34, GLOOM
	db $fe
	
	;8
	db 0
	
	db 39, CHANSEY
	db 39, GLOOM
	db $fe
	
	;9
	db 0
	
	db 44, CHANSEY
	db 44, GLOOM
	db $fe
	
	;10
	db 0
	
	db 49, CHANSEY
	db 49, GLOOM
	db $fe
	
	;11
	db 0
	
	db 53, CHANSEY
	db 53, GLOOM
	db $fe
	
	;12
	db 0
	
	db 57, CHANSEY
	db 57, GLOOM
	db $fe
	
	;13
	db 0
	
	db 62, CHANSEY
	db 62, GLOOM
	db $fe
	
	;14
	db 0
	
	db 65, CHANSEY
	db 65, GLOOM
	db $fe
	
	;15
	db 0
	
	db 73, CHANSEY
	db 73, GLOOM
	db $fe
	
	;16
	db 0
	
	db 80, CHANSEY
	db 80, GLOOM
	db $ff
	
	; COOLTRAINERM (5)
	db "MIKE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, CHIKORITA
	db 10, CHARMANDER
    db $fe	
	
	;1
	db 0
	
	db 16, CHIKORITA
	db 16, CHARMANDER
    db $fe

    ;2
    db 0
	
	db 20, CHIKORITA
	db 20, CHARMELEON
    db $fe		
	
	;3
	db 0
	
	db 24, CHIKORITA
	db 24, CHARMELEON
    db $fe	
	
	;4
	db 0
	
	db 28, CHIKORITA
	db 28, CHARMELEON
    db $fe

    ;5
    db 0
	
	db 32, BAYLEEF
	db 32, CHARMELEON
    db $fe

    ;6
    db 0
	
	db 34, BAYLEEF
	db 34, CHARMELEON
    db $fe

    ;7
    db 0
	
	db 37, BAYLEEF
	db 37, CHARMELEON
    db $fe

    ;8
    db 0
	
	db 42, BAYLEEF
	db 42, CHARIZARD
    db $fe		
	
	;9
	db 0
	
	db 46, BAYLEEF
	db 46, CHARIZARD
    db $fe

    ;10
    db 0
	
	db 50, BAYLEEF
	db 50, CHARIZARD
    db $fe

    ;11
    db 0
	
	db 54, BAYLEEF
	db 54, CHARIZARD
    db $fe

    ;12
    db 0
	
	db 57, BAYLEEF
	db 57, CHARIZARD
    db $fe

    ;13
    db 0
	
	db 62, BAYLEEF
	db 62, CHARIZARD
    db $fe	

    ;14
    db 0
	
	db 68, BAYLEEF
	db 68, CHARIZARD
    db $fe	

    ;15
    db 0
	
	db 74, BAYLEEF
	db 74, CHARIZARD
    db $fe	

    ;16
    db 0
	
	db 83, BAYLEEF
	db 83, CHARIZARD
    db $ff			

	; COOLTRAINERM (6)
	db "GAVEN@", TRAINERTYPE_MOVES
	db 35, VICTREEBEL, WRAP, TOXIC, ACID, RAZOR_LEAF
	db 35, KINGLER,    BUBBLEBEAM, STOMP, GUILLOTINE, PROTECT
	db 35, FLAREON,    SAND_ATTACK, QUICK_ATTACK, BITE, FIRE_SPIN
	db $ff ; end

	; COOLTRAINERM (7)
	db "GAVEN@", TRAINERTYPE_ITEM_MOVES
	db 39, VICTREEBEL, NO_ITEM,      GIGA_DRAIN, TOXIC, SLUDGE_BOMB, RAZOR_LEAF
	db 39, KINGLER,    KINGS_ROCK,   SURF, STOMP, GUILLOTINE, BLIZZARD
	db 39, FLAREON,    NO_ITEM,      FLAMETHROWER, QUICK_ATTACK, BITE, FIRE_SPIN
	db $ff ; end

	; COOLTRAINERM (8)
	db "RYAN@", TRAINERTYPE_MOVES
	db 25, PIDGEOT,    SAND_ATTACK, QUICK_ATTACK, WHIRLWIND, WING_ATTACK
	db 27, ELECTABUZZ, THUNDERPUNCH, LIGHT_SCREEN, SWIFT, SCREECH
	db $ff ; end

	; COOLTRAINERM (9)
	db "JAKE@", TRAINERTYPE_MOVES
	db 33, PARASECT,   LEECH_LIFE, SPORE, SLASH, SWORDS_DANCE
	db 35, GOLDUCK,    CONFUSION, SCREECH, PSYCH_UP, FURY_SWIPES
	db $ff ; end

	; COOLTRAINERM (10)
	db "GAVEN@", TRAINERTYPE_MOVES
	db 32, VICTREEBEL, WRAP, TOXIC, ACID, RAZOR_LEAF
	db 32, KINGLER,    BUBBLEBEAM, STOMP, GUILLOTINE, PROTECT
	db 32, FLAREON,    SAND_ATTACK, QUICK_ATTACK, BITE, FIRE_SPIN
	db $ff ; end

	; COOLTRAINERM (11)
	db "BLAKE@", TRAINERTYPE_MOVES
	db 33, MAGNETON,   THUNDERBOLT, SUPERSONIC, SWIFT, SCREECH
	db 31, QUAGSIRE,   WATER_GUN, SLAM, AMNESIA, EARTHQUAKE
	db 31, EXEGGCUTE,  LEECH_SEED, CONFUSION, SLEEP_POWDER, SOLARBEAM
	db $ff ; end

	; COOLTRAINERM (12)
	db "BRIAN@", TRAINERTYPE_MOVES
	db 35, SANDSLASH,  SAND_ATTACK, POISON_STING, SLASH, SWIFT
	db $ff ; end

	; COOLTRAINERM (13)
	db "ERICK@", TRAINERTYPE_NORMAL
	db 10, BULBASAUR
	db 10, CHARMANDER
	db 10, SQUIRTLE
	db $ff ; end

	; COOLTRAINERM (14)
	db "ANDY@", TRAINERTYPE_NORMAL
	db 10, BULBASAUR
	db 10, CHARMANDER
	db 10, SQUIRTLE
	db $ff ; end

	; COOLTRAINERM (15)
	db "TYLER@", TRAINERTYPE_NORMAL
	db 10, BULBASAUR
	db 10, CHARMANDER
	db 10, SQUIRTLE
	db $ff ; end

	; COOLTRAINERM (16)
	db "SEAN@", TRAINERTYPE_NORMAL
	db 35, FLAREON
	db 35, TANGELA
	db 35, TAUROS
	db $ff ; end

	; COOLTRAINERM (17)
	db "KEVIN@", TRAINERTYPE_NORMAL
	db 38, RHYHORN
	db 35, CHARMELEON
	db 35, WARTORTLE
	db $ff ; end

	; COOLTRAINERM (18)
	db "STEVE@", TRAINERTYPE_NORMAL
	db 14, BULBASAUR
	db 14, CHARMANDER
	db 14, SQUIRTLE
	db $ff ; end

	; COOLTRAINERM (19)
	db "ALLEN@", TRAINERTYPE_MOVES
	db 27, CHARMELEON, EMBER, SMOKESCREEN, RAGE, SCARY_FACE
	db $ff ; end

	; COOLTRAINERM (20)
	db "DARIN@", TRAINERTYPE_MOVES
	db 37, DRAGONAIR,  WRAP, SURF, DRAGON_RAGE, SLAM
	db $ff ; end


CooltrainerFGroup:
	; COOLTRAINERF (1)
	db "GWEN@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, NINETALES
	db  8, PARASECT
	db  9, HYPNO
	db $fe
	
	;1
	db 0
	
	db 15, NINETALES
	db 15, PARASECT
	db 15, HYPNO
	db $fe
	
	;2
	db 0
	
	db 20, JIGGLYPUFF
	db 20, PARASECT
	db 20, HYPNO
	db $fe
	
	;3
	db 0
	
	db 24, JIGGLYPUFF
	db 24, PARASECT
	db 24, KRABBY
	db $fe
	
	;4
	db 0
	
	db 27, JIGGLYPUFF
	db 27, PARASECT
	db 27, KRABBY
	db $fe
	
	;5
	db 0
	
	db 30, JIGGLYPUFF
	db 30, VENONAT
	db 30, KRABBY
	db $fe
	
	;6
	db 0
	
	db 34, JIGGLYPUFF
	db 34, VENONAT
	db 34, KRABBY
	db $fe
	
	;7
	db 0
	
	db 39, WIGGLYTUFF
	db 39, VENONAT
	db 39, KRABBY
	db $fe
	
	;8
	db 0
	
	db 43, WIGGLYTUFF
	db 43, VENONAT
	db 43, KRABBY
	db $fe
	
	;9
	db 0
	
	db 48, WIGGLYTUFF
	db 48, VENONAT
	db 48, KRABBY
	db $fe
	
	;10
	db 0
	
	db 54, WIGGLYTUFF
	db 54, VENOMOTH
	db 54, KRABBY
	db $fe
	
	;11
	db 0
	
	db 60, WIGGLYTUFF
	db 60, VENOMOTH
	db 60, KRABBY
	db $fe
	
	;12
	db 0
	
	db 64, WIGGLYTUFF
	db 64, VENOMOTH
	db 64, KRABBY
	db $fe
	
	;13
	db 0
	
	db 68, WIGGLYTUFF
	db 68, VENOMOTH
	db 68, KRABBY
	db $fe
	
	;14
	db 0
	
	db 75, WIGGLYTUFF
	db 75, VENOMOTH
	db 75, KRABBY
	db $fe
	
	;15
	db 0
	
	db 80, WIGGLYTUFF
	db 80, VENOMOTH
	db 80, KRABBY
	db $fe
	
	;16
	db 0
	
	db 85, WIGGLYTUFF
	db 85, VENOMOTH
	db 85, KRABBY
	db $ff

	; COOLTRAINERF (2)
	db "LOIS@", TRAINERTYPE_MOVES
	db 25, SKIPLOOM,   SYNTHESIS, POISONPOWDER, MEGA_DRAIN, LEECH_SEED
	db 25, NINETALES,  EMBER, QUICK_ATTACK, CONFUSE_RAY, SAFEGUARD
	db $ff ; end

	; COOLTRAINERF (3)
	db "FRAN@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, CLEFABLE
	db  8, STARMIE
	db $fe
	
	;1
	db 0
	
	db 11, CLEFABLE
	db 11, STARMIE
	db $fe
	
	;2
	db 0
	
	db 18, CLEFABLE
	db 18, STARMIE
	db $fe
	
	;3
	db 0
	
	db 23, CLEFABLE
	db 23, STARMIE
	db $fe
	
	;4
	db 0
	
	db 27, CLEFABLE
	db 27, STARMIE
	db $fe
	
	;5
	db 0
	
	db 28, CLEFABLE
	db 28, STARMIE
	db $fe
	
	;6
	db 0
	
	db 30, VULPIX
	db 30, MR__MIME
	db $fe
	
	;7
	db 0
	
	db 34, VULPIX
	db 34, MR__MIME
	db $fe
	
	;8
	db 0
	
	db 39, VULPIX
	db 39, MR__MIME
	db $fe
	
	;9
	db 0
	
	db 44, VULPIX
	db 44, MR__MIME
	db $fe
	
	;10
	db 0
	
	db 49, VULPIX
	db 49, MR__MIME
	db $fe
	
	;11
	db 0
	
	db 53, VULPIX
	db 53, MR__MIME
	db $fe
	
	;12
	db 0
	
	db 57, VULPIX
	db 57, MR__MIME
	db $fe
	
	;13
	db 0
	
	db 62, VULPIX
	db 62, MR__MIME
	db $fe
	
	;14
	db 0
	
	db 65, VULPIX
	db 65, MR__MIME
	db $fe

	;15
	db 0
	
	db 73, VULPIX
	db 73, MR__MIME
	db $fe
	
	;16
	db 0
	
	db 80, VULPIX
	db 80, MR__MIME
	db $ff
	
	; COOLTRAINERF (4)
	db "LOLA@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, POLIWAG
	db 12, PIDGEOTTO
	db $fe
	
	;1
	db 0
	
	db 12, POLIWAG
	db 15, PIDGEOTTO
	db $fe
	
	;2
	db 0
	
	db 16, POLIWAG
	db 19, PIDGEOT
	db $fe

	;3
	db 0
	
	db 22, POLIWAG
	db 24, PIDGEOT
	db $fe
	
	;4
	db 0
	
	db 27, POLIWAG
	db 29, PIDGEOT
	db $fe
	
	;5
	db 0
	
	db 28, POLIWHIRL
	db 30, PIDGEOT
	db $fe
	
	;6
	db 0
	
	db 30, POLIWHIRL
	db 32, PIDGEOT
	db $fe
	
	;7
	db 0
	
	db 34, POLIWHIRL
	db 37, RATTATA
	db $fe
	
	;8
	db 0
	
	db 39, POLIWHIRL
	db 42, RATTATA
	db $fe
	
	;9
	db 0
	
	db 44, POLIWHIRL
	db 47, RATTATA
	db $fe
	
	;10
	db 0
	
	db 49, POLIWHIRL
	db 51, RATTATA
	db $fe
	
	;11
	db 0
	
	db 53, POLIWHIRL
	db 57, RATTATA
	db $fe
	
	;12
	db 0
	
	db 57, POLIWHIRL
	db 60, RATTATA
	db $fe
	
	;13
	db 0
	
	db 62, POLIWHIRL
	db 64, RATTATA
	db $fe
	
	;14
	db 0
	
	db 65, POLIWHIRL
	db 72, RATTATA
	db $fe
	
	;15
	db 0
	
	db 73, POLIWHIRL
	db 76, RATTATA
	db $fe
	
	;16
	db 0
	
	db 80, POLIWHIRL
	db 90, RATTATA
	db $ff
	
	; COOLTRAINERF (5)
	db "KATE@", TRAINERTYPE_NORMAL
	db 26, SHELLDER
	db 28, CLOYSTER
	db $ff ; end

	; COOLTRAINERF (6)
	db "IRENE@", TRAINERTYPE_NORMAL
	db 22, GOLDEEN
	db 24, SEAKING
	db $ff ; end

	; COOLTRAINERF (7)
	db "KELLY@", TRAINERTYPE_NORMAL
	db 27, MARILL
	db 24, WARTORTLE
	db 24, WARTORTLE
	db $ff ; end

	; COOLTRAINERF (8)
	db "JOYCE@", TRAINERTYPE_MOVES
	db 36, PIKACHU,    QUICK_ATTACK, DOUBLE_TEAM, THUNDERBOLT, THUNDER
	db 32, BLASTOISE,  BITE, CURSE, SURF, RAIN_DANCE
	db $ff ; end

	; COOLTRAINERF (9)
	db "BETH@", TRAINERTYPE_MOVES
	db 36, RAPIDASH,   STOMP, FIRE_SPIN, FURY_ATTACK, AGILITY
	db $ff ; end

	; COOLTRAINERF (10)
	db "REENA@", TRAINERTYPE_NORMAL
	db 31, STARMIE
	db 33, NIDOQUEEN
	db 31, STARMIE
	db $ff ; end

	; COOLTRAINERF (11)
	db "MEGAN@", TRAINERTYPE_MOVES
	db 32, BULBASAUR,  GROWL, LEECH_SEED, POISONPOWDER, RAZOR_LEAF
	db 32, IVYSAUR,    GROWL, LEECH_SEED, POISONPOWDER, RAZOR_LEAF
	db 32, VENUSAUR,   BODY_SLAM, SLEEP_POWDER, RAZOR_LEAF, SWEET_SCENT
	db $ff ; end

	; COOLTRAINERF (12)
	db "BETH@", TRAINERTYPE_MOVES
	db 39, RAPIDASH,   STOMP, FIRE_SPIN, FURY_ATTACK, AGILITY
	db $ff ; end

	; COOLTRAINERF (13)
	db "CAROL@", TRAINERTYPE_NORMAL
	db 35, ELECTRODE
	db 35, STARMIE
	db 35, NINETALES
	db $ff ; end

	; COOLTRAINERF (14)
	db "QUINN@", TRAINERTYPE_NORMAL
	db 38, IVYSAUR
	db 38, STARMIE
	db $ff ; end

	; COOLTRAINERF (15)
	db "EMMA@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, RATICATE
	db  8, NIDORINA
	db  9, FEAROW
	db $fe
	
	;1
	db 0
	
	db 14, RATICATE
	db 14, NIDORINA
	db 16, FEAROW
	db $fe
	
	;2
	db 0
	
	db 18, RATICATE
	db 18, NIDORINA
	db 22, FEAROW
	db $fe
	
	;3
	db 0
	
	db 22, RATICATE
	db 22, NIDORINA
	db 25, FEAROW
	db $fe
	
	;4
	db 0
	
	db 26, RATICATE
	db 26, NIDORINA
	db 29, FEAROW
	db $fe
	
	;5
	db 0
	
	db 28, SPEAROW
	db 28, NIDOQUEEN
	db 33, EKANS
	db $fe
	
	;6
	db 0
	
	db 34, SPEAROW
	db 34, NIDOQUEEN
	db 37, EKANS
	db $fe
	
	;7
	db 0
	
	db 38, SPEAROW
	db 38, NIDOQUEEN
	db 40, EKANS
	db $fe
	
	;8
	db 0
	
	db 40, SPEAROW
	db 40, NIDOQUEEN
	db 43, EKANS
	db $fe
	
	;9
	db 0
	
	db 44, SPEAROW
	db 44, NIDOQUEEN
	db 49, EKANS
	db $fe
	
	;10
	db 0
	
	db 48, SPEAROW
	db 48, NIDOQUEEN
	db 53, EKANS
	db $fe
	
	;11
	db 0
	
	db 52, SPEAROW
	db 52, NIDOQUEEN
	db 54, EKANS
	db $fe
	
	;12
	db 0
	
	db 56, SPEAROW
	db 56, NIDOQUEEN
	db 59, EKANS
	db $fe
	
	;13
	db 0
	
	db 59, SPEAROW
	db 59, NIDOQUEEN
	db 65, EKANS
	db $fe
	
	;14
	db 0
	
	db 66, SPEAROW
	db 66, NIDOQUEEN
	db 70, EKANS
	db $fe
	
	;15
	db 0
	
	db 70, SPEAROW
	db 70, NIDOQUEEN
	db 76, EKANS
	db $fe
	
	;16
	db 0
	
	db 84, SPEAROW
	db 84, NIDOQUEEN
	db 93, EKANS
	db $ff

	; COOLTRAINERF (16)
	db "CYBIL@", TRAINERTYPE_MOVES
	db 25, BUTTERFREE, CONFUSION, SLEEP_POWDER, WHIRLWIND, GUST
	db 25, BELLOSSOM,  ABSORB, STUN_SPORE, ACID, SOLARBEAM
	db $ff ; end

	; COOLTRAINERF (17)
	db "JENN@", TRAINERTYPE_NORMAL
	db 24, STARYU
	db 26, STARMIE
	db $ff ; end

	; COOLTRAINERF (18)
	db "BETH@", TRAINERTYPE_ITEM_MOVES
	db 43, RAPIDASH,   FOCUS_BAND,   STOMP, FIRE_SPIN, FURY_ATTACK, FIRE_BLAST
	db $ff ; end

	; COOLTRAINERF (19)
	db "REENA@", TRAINERTYPE_NORMAL
	db 34, STARMIE
	db 36, NIDOQUEEN
	db 34, STARMIE
	db $ff ; end

	; COOLTRAINERF (20)
	db "REENA@", TRAINERTYPE_ITEM_MOVES
	db 38, STARMIE,    NO_ITEM,      DOUBLE_TEAM, PSYCHIC_M, WATERFALL, CONFUSE_RAY
	db 40, NIDOQUEEN,  PINK_BOW,     EARTHQUAKE, DOUBLE_KICK, TOXIC, BODY_SLAM
	db 38, STARMIE,    NO_ITEM,      BLIZZARD, PSYCHIC_M, WATERFALL, RECOVER
	db $ff ; end

	; COOLTRAINERF (21)
	db "CARA@", TRAINERTYPE_MOVES
	db 33, HORSEA,     SMOKESCREEN, LEER, WHIRLPOOL, TWISTER
	db 33, HORSEA,     SMOKESCREEN, LEER, WHIRLPOOL, TWISTER
	db 35, SEADRA,     SWIFT, LEER, WATERFALL, TWISTER
	db $ff ; end


BeautyGroup:
	; BEAUTY (1)
	db "VICTORIA@", TRAINERTYPE_NORMAL
	db  9, SENTRET
	db 13, SENTRET
	db 17, SENTRET
	db $ff ; end

	; BEAUTY (2)
	db "SAMANTHA@", TRAINERTYPE_MOVES
	db 16, MEOWTH,     SCRATCH, GROWL, BITE, PAY_DAY
	db 16, MEOWTH,     SCRATCH, GROWL, BITE, SLASH
	db $ff ; end

	; BEAUTY (3)
	db "JULIE@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (4)
	db "JACLYN@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (5)
	db "BRENDA@", TRAINERTYPE_NORMAL
	db 16, FURRET
	db $ff ; end

	; BEAUTY (6)
	db "CASSIE@", TRAINERTYPE_NORMAL
	db 28, VILEPLUME
	db 34, BUTTERFREE
	db $ff ; end

	; BEAUTY (7)
	db "CAROLINE@", TRAINERTYPE_NORMAL
	db 30, MARILL
	db 32, SEEL
	db 30, MARILL
	db $ff ; end

	; BEAUTY (8)
	db "CARLENE@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (9)
	db "JESSICA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (10)
	db "RACHAEL@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (11)
	db "ANGELICA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (12)
	db "KENDRA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (13)
	db "VERONICA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (14)
	db "JULIA@", TRAINERTYPE_NORMAL
	db 32, PARAS
	db 32, EXEGGCUTE
	db 35, PARASECT
	db $ff ; end

	; BEAUTY (15)
	db "THERESA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db $ff ; end

	; BEAUTY (16)
	db "VALERIE@", TRAINERTYPE_MOVES
	db 17, HOPPIP,     SYNTHESIS, TAIL_WHIP, TACKLE, POISONPOWDER
	db 17, SKIPLOOM,   SYNTHESIS, TAIL_WHIP, TACKLE, STUN_SPORE
	db $ff ; end

	; BEAUTY (17)
	db "OLIVIA@", TRAINERTYPE_NORMAL
	db 19, CORSOLA
	db $ff ; end


PokemaniacGroup:
	; POKEMANIAC (1)
	db "LARRY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 6, RAICHU
	db 6, NIDORAN_M
	db 6, POLIWAG
	db $fe
	
	;1
	db 0
	
	db 15, RAICHU
	db 15, NIDORAN_M
	db 15, POLIWAG
	db $fe 
	
	;2
	db 0
	
	db 19, RAICHU
	db 19, NIDORAN_M
	db 19, POLIWAG
	db $fe 
		
	;3
	db 0
	
	db 22, RAICHU
	db 22, NIDORAN_M
	db 22, POLIWAG
	db $fe 

	;4
	db 0
	
	db 26, RAICHU
	db 26, NIDORAN_M
	db 26, POLIWAG
	db 26, MACHOP
	db $fe 
		
	;5
	db 0
	
	db 30, RAICHU
	db 30, NIDORAN_M
	db 30, POLIWAG
	db 30, MACHOP
	db 30, WEEPINBELL
	db $fe

	;6
	db 0
	
	db 34, RAICHU
	db 34, NIDORAN_M
	db 34, POLIWAG
	db 34, MACHOP
	db 34, WEEPINBELL
	db 34, RAPIDASH
	db $fe 
	
	;7
	db 0
	
	db 40, RAICHU
	db 40, NIDORAN_M
	db 40, POLIWAG
	db 40, MACHOP
	db 40, WEEPINBELL
	db 40, RAPIDASH
	db $fe 
	
	;8
	db 0
	
	db 44, RAICHU
	db 44, NIDORAN_M
	db 44, POLIWAG
	db 44, MACHOP
	db 44, WEEPINBELL
	db 44, RAPIDASH
	db $fe 
	
	;9
	db 0
	
	db 49, RAICHU
	db 49, NIDORAN_M
	db 49, POLIWAG
	db 49, MACHOP
	db 49, WEEPINBELL
	db 49, RAPIDASH
	db $fe 
	
	;10
	db 0
	
	db 52, RAICHU
	db 52, NIDORAN_M
	db 52, POLIWAG
	db 52, MACHOP
	db 52, WEEPINBELL
	db 52, RAPIDASH
	db $fe 
	
	;11
	db 0
	
	db 56, RAICHU
	db 56, NIDORAN_M
	db 56, POLIWAG
	db 56, MACHOP
	db 56, WEEPINBELL
	db 56, RAPIDASH
	db $fe 
	
	;12
	db 0
	
	db 64, RAICHU
	db 64, NIDORAN_M
	db 64, POLIWAG
	db 64, MACHOP
	db 64, WEEPINBELL
	db 64, RAPIDASH
	db $fe 
	
	;13
	db 0
	
	db 70, RAICHU
	db 70, NIDORAN_M
	db 70, POLIWAG
	db 70, MACHOP
	db 70, WEEPINBELL
	db 70, RAPIDASH
	db $fe 
	
	;14
	db 0
	
	db 75, RAICHU
	db 75, NIDORAN_M
	db 75, POLIWAG
	db 75, MACHOP
	db 75, WEEPINBELL
	db 75, RAPIDASH
	db $fe 
	
	;15
	db 0
	
	db 80, RAICHU
	db 80, NIDORAN_M
	db 80, POLIWAG
	db 80, MACHOP
	db 80, WEEPINBELL
	db 80, RAPIDASH
	db $fe 
	
	;16
	db 0
	
	db 90, RAICHU
	db 90, NIDORAN_M
	db 90, POLIWAG
	db 90, MACHOP
	db 90, WEEPINBELL
	db 90, RAPIDASH
	db $ff
	
	; POKEMANIAC (2)
	db "ANDREW@"
	db 5
	
	;0
	db 0
	
	db  8, NIDORINA
	db  9, RAICHU
	db  9, RATICATE
	db $fe
	
	;1
	db 0
	
	db  14, NIDORINA
	db  15, RAICHU
	db  15, RATICATE
	db $fe
	
	;2
	db 0
	
	db  20, NIDORINA
	db  21, RAICHU
	db  21, RATICATE
	db $fe
	
	;3
	db 0
	
	db  22, NIDORINA
	db  24, SANDSHREW
	db  24, SPEAROW
	db $fe
	
	;4
	db 0
	
	db  24, NIDOQUEEN
	db  26, SANDSHREW
	db  26, SPEAROW
	db $fe

	;5
	db 0
	
	db  26, NIDOQUEEN
	db  29, SANDSHREW
	db  29, SPEAROW
	db $fe
	
	;6
	db 0
	
	db  29, NIDOQUEEN
	db  32, SANDSHREW
	db  32, SPEAROW
	db $fe
	
	;7
	db 0
	
	db  33, NIDOQUEEN
	db  35, SANDSHREW
	db  35, SPEAROW
	db $fe
	
	;8
	db 0
	
	db  37, PIDGEY
	db  37, NIDOQUEEN
	db  40, SANDSHREW
	db  40, SPEAROW
	db $fe
	
	;9
	db 0
	
	db  40, PIDGEY
	db  40, NIDOQUEEN
	db  43, SANDSHREW
	db  43, SPEAROW
	db $fe
	
	;10
	db 0
	
	db  44, PIDGEY
	db  44, NIDOQUEEN
	db  48, SANDSHREW
	db  48, SPEAROW
	db $fe
	
	;11
	db 0
	
	db  47, PIDGEY
	db  47, NIDOQUEEN
	db  50, SANDSHREW
	db  50, SPEAROW
	db $fe
	
	;12
	db 0
	
	db  52, PIDGEY
	db  52, NIDOQUEEN
	db  55, SANDSHREW
	db  55, SPEAROW
	db $fe
	
	;13
	db 0
	
	db  57, PIDGEY
	db  57, NIDOQUEEN
	db  60, SANDSHREW
	db  60, SPEAROW
	db $fe
	
	;14
	db 0
	
	db  64, PIDGEY
	db  64, NIDOQUEEN
	db  68, SANDSHREW
	db  68, SPEAROW
	db $fe
	
	;15
	db 0
	
	db  76, PIDGEY
	db  76, NIDOQUEEN
	db  80, SANDSHREW
	db  80, SPEAROW
	db $fe
	
	;16
	db 0
	
	db  84, PIDGEY
	db  84, NIDOQUEEN
	db  90, SANDSHREW
	db  90, SPEAROW
	db $ff
	
	; POKEMANIAC (3)
	db "CALVIN@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, BEEDRILL
	db 10, MACHOP
	db $fe
	
	;1
	db 0
	
	db 16, BEEDRILL
	db 17, MACHOP
	db $fe
	
	;2
	db 0
	
	db 21, BEEDRILL
	db 22, MACHOP
	db $fe

	;3
	db 0
	
	db 25, PIDGEY
	db 26, MACHOKE
	db $fe
	
	;4
	db 0
	
	db 27, PIDGEY
	db 28, MACHOKE
	db $fe
	
	;5
	db 0
	
	db 30, PIDGEY
	db 33, MACHOKE
	db $fe
	
	;6
	db 0
	
	db 34, PIDGEY
	db 36, MACHOKE
	db $fe
	
	;7
	db 0
	
	db 38, PIDGEY
	db 40, MACHOKE
	db $fe
	
	;8
	db 0
	
	db 42, PIDGEY
	db 44, MACHOKE
	db $fe
	
	;9
	db 0
	
	db 46, ALAKAZAM
	db 49, PIDGEY
	db 49, MACHOKE
	db $fe
	
	;10
	db 0
	
	db 50, ALAKAZAM
	db 52, PIDGEY
	db 52, MACHOKE
	db $fe
	
	;11
	db 0
	
	db 56, ALAKAZAM
	db 59, PIDGEY
	db 59, MACHOKE
	db $fe
	
	;12
	db 0
	
	db 60, ALAKAZAM
	db 64, PIDGEY
	db 64, MACHOKE
	db $fe
	
	;13
	db 0
	
	db 64, ALAKAZAM
	db 69, PIDGEY
	db 69, MACHOKE
	db $fe
	
	;14
	db 0
	
	db 70, ALAKAZAM
	db 73, PIDGEY
	db 73, MACHOKE
	db $fe
	
	;15
	db 0
	
	db 75, ALAKAZAM
	db 78, PIDGEY
	db 78, MACHOKE
	db $fe
	
	;16
	db 0
	
	db 80, ALAKAZAM
	db 85, PIDGEY
	db 92, MACHOKE
	db $ff
	
	; POKEMANIAC (4)
	db "SHANE@", TRAINERTYPE_NORMAL
	db 16, NIDORINA
	db 16, NIDORINO
	db $ff ; end

	; POKEMANIAC (5)
	db "BEN@", TRAINERTYPE_NORMAL
	db 19, SLOWBRO
	db $ff ; end

	; POKEMANIAC (6)
	db "BRENT@", TRAINERTYPE_NORMAL
	db 19, LICKITUNG
	db $ff ; end

	; POKEMANIAC (7)
	db "RON@", TRAINERTYPE_NORMAL
	db 19, NIDOKING
	db $ff ; end

	; POKEMANIAC (8)
	db "ETHAN@", TRAINERTYPE_NORMAL
	db 31, RHYHORN
	db 31, RHYDON
	db $ff ; end

	; POKEMANIAC (9)
	db "BRENT@", TRAINERTYPE_NORMAL
	db 25, KANGASKHAN
	db $ff ; end

	; POKEMANIAC (10)
	db "BRENT@", TRAINERTYPE_MOVES
	db 36, PORYGON,    RECOVER, PSYCHIC_M, QUIVER_DANCE, TRI_ATTACK
	db $ff ; end

	; POKEMANIAC (11)
	db "ISSAC@", TRAINERTYPE_MOVES
	db 12, LICKITUNG,  LICK, SUPERSONIC, CUT, NO_MOVE
	db $ff ; end

	; POKEMANIAC (12)
	db "DONALD@", TRAINERTYPE_NORMAL
	db 10, SLOWPOKE
	db 10, SLOWPOKE
	db $ff ; end

	; POKEMANIAC (13)
	db "ZACH@", TRAINERTYPE_NORMAL
	db 27, RHYHORN
	db $ff ; end

	; POKEMANIAC (14)
	db "BRENT@", TRAINERTYPE_MOVES
	db 41, CHANSEY,    ROLLOUT, ATTRACT, EGG_BOMB, SOFTBOILED
	db $ff ; end

	; POKEMANIAC (15)
	db "MILLER@", TRAINERTYPE_NORMAL
	db 17, NIDOKING
	db 17, NIDOQUEEN
	db $ff ; end


GruntMGroup:
    ; GRUNTF (1)
	db "002TRAINER@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, MEGANIUM
	db $fe
	
	;1
	db 0
	
	db 17, MEGANIUM
	db $fe
	
	;2
	db 0
	
	db 22, MEGANIUM
	db $fe
	
	;3
	db 0
	
	db 26, MEGANIUM
	db $fe
	
	;4
	db 0
	
	db 30, MEGANIUM
	db $fe
	
	;5
	db 0
	
	db 31, MEGANIUM
	db $fe
	
	;6
	db 0
	
	db 33, MEGANIUM
	db $fe
	
	;7
	db 0
	
	db 38, MEGANIUM
	db $fe
	
	;8
	db 0
	
	db 43, MEGANIUM
	db $fe
	
	;9
	db 0
	
	db 48, MEGANIUM
	db $fe
	
	;10
	db 0
	
	db 52, MEGANIUM
	db $fe
	
	;11
	db 0
	
	db 57, MEGANIUM
	db $fe
	
	;12
	db 0
	
	db 62, MEGANIUM
	db $fe
	
	;13
	db 0
	
	db 64, MEGANIUM
	db $fe
	
	;14
	db 0
	
	db 73, MEGANIUM
	db $fe
	
	;15
	db 0
	
	db 79, MEGANIUM
	db $fe
	
	;16
	db 0
	
	db 95, MEGANIUM
	db $ff

	; GRUNTM (2)
	db "002TRAINER@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, MEGANIUM
	db $fe
	
	;1
	db 0
	
	db 17, MEGANIUM
	db $fe
	
	;2
	db 0
	
	db 22, MEGANIUM
	db $fe
	
	;3
	db 0
	
	db 26, MEGANIUM
	db $fe
	
	;4
	db 0
	
	db 30, MEGANIUM
	db $fe
	
	;5
	db 0
	
	db 31, MEGANIUM
	db $fe
	
	;6
	db 0
	
	db 33, MEGANIUM
	db $fe
	
	;7
	db 0
	
	db 38, MEGANIUM
	db $fe
	
	;8
	db 0
	
	db 43, MEGANIUM
	db $fe
	
	;9
	db 0
	
	db 48, MEGANIUM
	db $fe
	
	;10
	db 0
	
	db 52, MEGANIUM
	db $fe
	
	;11
	db 0
	
	db 57, MEGANIUM
	db $fe
	
	;12
	db 0
	
	db 62, MEGANIUM
	db $fe
	
	;13
	db 0
	
	db 64, MEGANIUM
	db $fe
	
	;14
	db 0
	
	db 73, MEGANIUM
	db $fe
	
	;15
	db 0
	
	db 79, MEGANIUM
	db $fe
	
	;16
	db 0
	
	db 95, MEGANIUM
	db $ff

	; GRUNTM (3)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 24, RATICATE
	db 24, RATICATE
	db $ff ; end

	; GRUNTM (4)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 23, GRIMER
	db 23, GRIMER
	db 25, MUK
	db $ff ; end

	; GRUNTM (5)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 21, RATTATA
	db 21, RATTATA
	db 23, RATTATA
	db 23, RATTATA
	db 23, RATTATA
	db $ff ; end

	; GRUNTM (6)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 26, ZUBAT
	db 26, ZUBAT
	db $ff ; end

	; GRUNTM (7)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 23, KOFFING
	db 23, GRIMER
	db 23, ZUBAT
	db 23, RATTATA
	db $ff ; end

	; GRUNTM (8)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 26, WEEZING
	db $ff ; end

	; GRUNTM (9)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 24, RATICATE
	db 26, KOFFING
	db $ff ; end

	; GRUNTM (10)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 22, ZUBAT
	db 24, GOLBAT
	db 22, GRIMER
	db $ff ; end

	; GRUNTM (11)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 23, MUK
	db 23, KOFFING
	db 25, RATTATA
	db $ff ; end

	; GRUNTM (12)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 33, HOUNDOUR
	db $ff ; end

	; GRUNTM (13)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, RATTATA
	db $ff ; end

	; GRUNTM (14)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 24, RATICATE
	db 24, GOLBAT
	db $ff ; end

	; GRUNTM (15)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 26, GRIMER
	db 23, WEEZING
	db $ff ; end

	; GRUNTM (16)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 16, RATTATA
	db 16, RATTATA
	db 16, RATTATA
	db 16, RATTATA
	db $ff ; end

	; GRUNTM (17)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 18, GOLBAT
	db $ff ; end

	; GRUNTM (18)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 17, RATTATA
	db 17, ZUBAT
	db 17, RATTATA
	db $ff ; end

	; GRUNTM (19)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 18, VENONAT
	db 18, VENONAT
	db $ff ; end

	; GRUNTM (20)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 17, DROWZEE
	db 19, ZUBAT
	db $ff ; end

	; GRUNTM (21)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 16, ZUBAT
	db 17, GRIMER
	db 18, RATTATA
	db $ff ; end

	; GRUNTM (22)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 36, GOLBAT
	db $ff ; end

	; GRUNTM (23)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db $ff ; end

	; GRUNTM (24)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 25, KOFFING
	db 25, KOFFING
	db $ff ; end

	; GRUNTM (25)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 24, KOFFING
	db 24, MUK
	db $ff ; end

	; GRUNTM (26)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 15, RATTATA
	db 15, RATTATA
	db $ff ; end

	; GRUNTM (27)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 22, ZUBAT
	db $ff ; end

	; GRUNTM (28)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 19, RATICATE
	db $ff ; end

	; GRUNTM (29)
	db "002TRAINER@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, MEGANIUM
	db $fe
	
	;1
	db 0
	
	db 17, MEGANIUM
	db $fe
	
	;2
	db 0
	
	db 22, MEGANIUM
	db $fe
	
	;3
	db 0
	
	db 26, MEGANIUM
	db $fe
	
	;4
	db 0
	
	db 30, MEGANIUM
	db $fe
	
	;5
	db 0
	
	db 31, MEGANIUM
	db $fe
	
	;6
	db 0
	
	db 33, MEGANIUM
	db $fe
	
	;7
	db 0
	
	db 38, MEGANIUM
	db $fe
	
	;8
	db 0
	
	db 43, MEGANIUM
	db $fe
	
	;9
	db 0
	
	db 48, MEGANIUM
	db $fe
	
	;10
	db 0
	
	db 52, MEGANIUM
	db $fe
	
	;11
	db 0
	
	db 57, MEGANIUM
	db $fe
	
	;12
	db 0
	
	db 62, MEGANIUM
	db $fe
	
	;13
	db 0
	
	db 64, MEGANIUM
	db $fe
	
	;14
	db 0
	
	db 73, MEGANIUM
	db $fe
	
	;15
	db 0
	
	db 79, MEGANIUM
	db $fe
	
	;16
	db 0
	
	db 95, MEGANIUM
	db $ff
	

	; GRUNTM (30)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 25, GOLBAT
	db 25, GOLBAT
	db 30, ARBOK
	db $ff ; end

	; GRUNTM (31)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, GOLBAT
	db $ff ; end


GentlemanGroup:
	; GENTLEMAN (1)
	db "PRESTON@", TRAINERTYPE_NORMAL
	db 18, GROWLITHE
	db 18, GROWLITHE
	db $ff ; end

	; GENTLEMAN (2)
	db "EDWARD@", TRAINERTYPE_NORMAL
	db 33, PERSIAN
	db $ff ; end

	; GENTLEMAN (3)
	db "GREGORY@", TRAINERTYPE_NORMAL
	db 37, PIKACHU
	db 33, FLAAFFY
	db $ff ; end

	; GENTLEMAN (4)
	db "VIRGIL@", TRAINERTYPE_NORMAL
	db 20, PONYTA
	db $ff ; end

	; GENTLEMAN (5)
	db "ALFRED@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, MANKEY
	db 10, HAUNTER
	db $fe
	
	;1
	db 0
	
	db 14, MANKEY
	db 15, HAUNTER
	db $fe
	
	;2
	db 0
	
	db 19, MANKEY
	db 20, HAUNTER
	db $fe
	
	;3
	db 0
	
	db 24, MANKEY
	db 25, HAUNTER
	db $fe

	;4
	db 0
	
	db 27, PRIMEAPE
	db 28, GENGAR
	db $fe
	
	;5
	db 0
	
	db 29, PRIMEAPE
	db 30, GENGAR
	db $fe
	
	;6
	db 0
	
	db 31, PRIMEAPE
	db 32, GENGAR
	db $fe
	
	;7
	db 0
	
	db 36, PRIMEAPE
	db 37, GENGAR
	db $fe
	
	;8
	db 0
	
	db 40, PRIMEAPE
	db 41, GENGAR
	db $fe
	
	;9
	db 0
	
	db 44, PRIMEAPE
	db 46, GENGAR
	db $fe
	
	;10
	db 0
	
	db 49, PRIMEAPE
	db 50, GENGAR
	db $fe
	
	;11
	db 0
	
	db 54, PRIMEAPE
	db 55, GENGAR
	db $fe
	
	;12
	db 0
	
	db 57, PRIMEAPE
	db 58, GENGAR
	db $fe
	
	;13
	db 0
	
	db 62, PRIMEAPE
	db 63, GENGAR
	db $fe

	;14
	db 0
	
	db 70, PRIMEAPE
	db 71, GENGAR
	db $fe
	
	;15
	db 0
	
	db 76, PRIMEAPE
	db 77, GENGAR
	db $fe
	
	;16
	db 0
	
	db 82, PRIMEAPE
	db 83, GENGAR
	db $ff
	
SkierGroup:
	; SKIER (1)
	db "ROXANNE@", TRAINERTYPE_NORMAL
	db 28, JYNX
	db $ff ; end

	; SKIER (2)
	db "CLARISSA@", TRAINERTYPE_NORMAL
	db 28, DEWGONG
	db $ff ; end


TeacherGroup:
	; TEACHER (1)
	db "COLETTE@", TRAINERTYPE_NORMAL
	db 36, CLEFAIRY
	db $ff ; end

	; TEACHER (2)
	db "HILLARY@", TRAINERTYPE_NORMAL
	db 32, AIPOM
	db 36, CUBONE
	db $ff ; end

	; TEACHER (3)
	db "SHIRLEY@", TRAINERTYPE_NORMAL
	db 35, JIGGLYPUFF
	db $ff ; end


SabrinaGroup:
	; SABRINA (1)
	db "SABRINA@", TRAINERTYPE_MOVES
	db 46, ESPEON,     SAND_ATTACK, QUICK_ATTACK, SWIFT, PSYCHIC_M
	db 46, MR__MIME,   BARRIER, REFLECT, BATON_PASS, PSYCHIC_M
	db 48, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db $ff ; end


BugCatcherGroup:
	; BUG_CATCHER (1)
	db "DON@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  3, CATERPIE
	db  3, HORSEA
	db  3, GOLEM
	db $fe
	
	
	;1
	db 0
	
	db  8, CATERPIE
	db  8, HORSEA
	db  8, GOLEM
	db $fe
	
	
	;2
	db 0
	
	db  17, METAPOD
	db  17, HORSEA
	db  17, GOLEM
	db $fe
	
	
	;3
	db 0
	
	db  20, METAPOD
	db  20, HORSEA
	db  20, GOLEM
	db $fe
	
	
	;4
	db 0
	
	db  24, METAPOD
	db  24, HORSEA
	db  24, GOLEM
	db $fe
	
	
	;5
	db 0
	
	db  27, METAPOD
	db  27, HORSEA
	db  27, GOLEM
	db $fe

	
	;6
	db 0
	
	db  30, METAPOD
	db  30, SEADRA
	db  30, GOLEM
	db $fe
	
	
	;7
	db 0
	
	db  34, METAPOD
	db  34, HORSEA
	db  34, GOLEM
	db $fe
	
	
	;8
	db 0
	
	db  38, METAPOD
	db  38, HORSEA
	db  38, GOLEM
	db $fe
	
	
	;9
	db 0
	
	db  42, METAPOD
	db  42, HORSEA
	db  42, GOLEM
	db $fe
	
	
	;10
	db 0
	
	db  47, METAPOD
	db  47, HORSEA
	db  47, GOLEM
	db $fe
	
	
	;11
	db 0
	
	db  50, BUTTERFREE
	db  50, GOLDEEN
	db  50, PONYTA
	db $fe
	
	
	;12
	db 0
	
	db  54, BUTTERFREE
	db  54, GOLDEEN
	db  54, PONYTA
	db $fe
	
	
	;13
	db 0
	
	db  58, BUTTERFREE
	db  58, GOLDEEN
	db  58, PONYTA
	db $fe
	
	;14
	db 0
	
	db  69, BUTTERFREE
	db  69, GOLDEEN
	db  69, PONYTA
	db $fe
	
	;15
	db 0
	
	db  70, BUTTERFREE
	db  70, GOLDEEN
	db  70, PONYTA
	db $fe
	
	;16
	db 0
	
	db  85, BUTTERFREE
	db  85, GOLDEEN
	db  85, PONYTA
	db $ff

	; BUG_CATCHER (2)
	db "ROB@", TRAINERTYPE_NORMAL
	db 32, BEEDRILL
	db 32, BUTTERFREE
	db $ff ; end

	; BUG_CATCHER (3)
	db "ED@", TRAINERTYPE_NORMAL
	db 30, BEEDRILL
	db 30, BEEDRILL
	db 30, BEEDRILL
	db $ff ; end

	; BUG_CATCHER (4)
	db "WADE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, CATERPIE
	db  5, PSYDUCK
	db  6, POLIWRATH
	db  7, OMASTAR
	db $fe 
	
	;1
	db 0
	
	db 14, METAPOD
	db 14, PSYDUCK
	db 15, POLIWRATH
	db 16, OMASTAR
	db $fe 
	
	;2
	db 0
	
	db 19, METAPOD
	db 19, PSYDUCK
	db 20, POLIWRATH
	db 21, OMASTAR
	db $fe 
	
	;3
	db 0
	
	db 24, METAPOD
	db 24, PSYDUCK
	db 25, POLIWRATH
	db 26, OMASTAR
	db $fe 
	
	;4
	db 0
	
	db 27, METAPOD
	db 27, PSYDUCK
	db 28, ABRA
	db 29, KABUTO
	db $fe 
	
	;5
	db 0
	
	db 29, BUTTERFREE
	db 29, GOLDUCK
	db 30, ABRA
	db 31, KABUTO
	db $fe 
	
	;6
	db 0
	
	db 31, BUTTERFREE
	db 31, GOLDUCK
	db 32, ABRA
	db 33, KABUTO
	db $fe 
	
	;7
	db 0
	
	db 35, BUTTERFREE
	db 35, GOLDUCK
	db 36, ABRA
	db 37, KABUTO
	db $fe

    ;8
    db 0
	
	db 40, BUTTERFREE
	db 40, GOLDUCK
	db 41, ABRA
	db 42, KABUTO
	db $fe

    ;9
    db 0
	
	db 46, BUTTERFREE
	db 46, GOLDUCK
	db 47, ABRA
	db 48, KABUTO
	db $fe

    ;10
    db 0
	
	db 50, BUTTERFREE
	db 50, GOLDUCK
	db 51, ABRA
	db 52, KABUTO
	db $fe

    ;11
    db 0
	
	db 55, BUTTERFREE
	db 55, GOLDUCK
	db 56, ABRA
	db 57, KABUTO
	db $fe    

    ;12
    db 0
	
	db 58, BUTTERFREE
	db 58, GOLDUCK
	db 59, ABRA
	db 60, KABUTO
	db $fe

    ;13
    db 0
	
	db 62, BUTTERFREE
	db 62, GOLDUCK
	db 63, ABRA
	db 64, KABUTO
	db $fe   

    ;14
    db 0
	
	db 70, BUTTERFREE
	db 70, GOLDUCK
	db 71, ABRA
	db 72, KABUTO
	db $fe

    ;15
    db 0
	
	db 75, BUTTERFREE
	db 75, GOLDUCK
	db 76, ABRA
	db 77, KABUTO
	db $fe

    ;16
    db 0
	
	db 88, BUTTERFREE
	db 88, GOLDUCK
	db 89, ABRA
	db 90, KABUTO
	db $ff	
	
	; BUG_CATCHER (5)
	db "BENNY@", TRAINERTYPE_NORMAL
	db  7, WEEDLE
	db  9, KAKUNA
	db 12, BEEDRILL
	db $ff ; end

	; BUG_CATCHER (6)
	db "AL@", TRAINERTYPE_NORMAL
	db 12, CATERPIE
	db 12, WEEDLE
	db $ff ; end

	; BUG_CATCHER (7)
	db "JOSH@", TRAINERTYPE_NORMAL
	db 13, PARAS
	db $ff ; end

	; BUG_CATCHER (8)
	db "ARNIE@", TRAINERTYPE_NORMAL
	db 15, VENONAT
	db $ff ; end

	; BUG_CATCHER (9)
	db "KEN@", TRAINERTYPE_NORMAL
	db 30, ARIADOS
	db 32, PINSIR
	db $ff ; end

	; BUG_CATCHER (10)
	db "WADE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, CATERPIE
	db  5, PSYDUCK
	db  6, POLIWRATH
	db  7, OMASTAR
	db $fe 
	
	;1
	db 0
	
	db 14, METAPOD
	db 14, PSYDUCK
	db 15, POLIWRATH
	db 16, OMASTAR
	db $fe 
	
	;2
	db 0
	
	db 19, METAPOD
	db 19, PSYDUCK
	db 20, POLIWRATH
	db 21, OMASTAR
	db $fe 
	
	;3
	db 0
	
	db 24, METAPOD
	db 24, PSYDUCK
	db 25, POLIWRATH
	db 26, OMASTAR
	db $fe 
	
	;4
	db 0
	
	db 27, METAPOD
	db 27, PSYDUCK
	db 28, ABRA
	db 29, KABUTO
	db $fe 
	
	;5
	db 0
	
	db 29, BUTTERFREE
	db 29, GOLDUCK
	db 30, ABRA
	db 31, KABUTO
	db $fe 
	
	;6
	db 0
	
	db 31, BUTTERFREE
	db 31, GOLDUCK
	db 32, ABRA
	db 33, KABUTO
	db $fe 
	
	;7
	db 0
	
	db 35, BUTTERFREE
	db 35, GOLDUCK
	db 36, ABRA
	db 37, KABUTO
	db $fe

    ;8
    db 0
	
	db 40, BUTTERFREE
	db 40, GOLDUCK
	db 41, ABRA
	db 42, KABUTO
	db $fe

    ;9
    db 0
	
	db 46, BUTTERFREE
	db 46, GOLDUCK
	db 47, ABRA
	db 48, KABUTO
	db $fe

    ;10
    db 0
	
	db 50, BUTTERFREE
	db 50, GOLDUCK
	db 51, ABRA
	db 52, KABUTO
	db $fe

    ;11
    db 0
	
	db 55, BUTTERFREE
	db 55, GOLDUCK
	db 56, ABRA
	db 57, KABUTO
	db $fe    

    ;12
    db 0
	
	db 58, BUTTERFREE
	db 58, GOLDUCK
	db 59, ABRA
	db 60, KABUTO
	db $fe

    ;13
    db 0
	
	db 62, BUTTERFREE
	db 62, GOLDUCK
	db 63, ABRA
	db 64, KABUTO
	db $fe   

    ;14
    db 0
	
	db 70, BUTTERFREE
	db 70, GOLDUCK
	db 71, ABRA
	db 72, KABUTO
	db $fe

    ;15
    db 0
	
	db 75, BUTTERFREE
	db 75, GOLDUCK
	db 76, ABRA
	db 77, KABUTO
	db $fe

    ;16
    db 0
	
	db 88, BUTTERFREE
	db 88, GOLDUCK
	db 89, ABRA
	db 90, KABUTO
	db $ff   	

	; BUG_CATCHER (11)
	db "WADE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, CATERPIE
	db  5, PSYDUCK
	db  6, POLIWRATH
	db  7, OMASTAR
	db $fe 
	
	;1
	db 0
	
	db 14, METAPOD
	db 14, PSYDUCK
	db 15, POLIWRATH
	db 16, OMASTAR
	db $fe 
	
	;2
	db 0
	
	db 19, METAPOD
	db 19, PSYDUCK
	db 20, POLIWRATH
	db 21, OMASTAR
	db $fe 
	
	;3
	db 0
	
	db 24, METAPOD
	db 24, PSYDUCK
	db 25, POLIWRATH
	db 26, OMASTAR
	db $fe 
	
	;4
	db 0
	
	db 27, METAPOD
	db 27, PSYDUCK
	db 28, ABRA
	db 29, KABUTO
	db $fe 
	
	;5
	db 0
	
	db 29, BUTTERFREE
	db 29, GOLDUCK
	db 30, ABRA
	db 31, KABUTO
	db $fe 
	
	;6
	db 0
	
	db 31, BUTTERFREE
	db 31, GOLDUCK
	db 32, ABRA
	db 33, KABUTO
	db $fe 
	
	;7
	db 0
	
	db 35, BUTTERFREE
	db 35, GOLDUCK
	db 36, ABRA
	db 37, KABUTO
	db $fe

    ;8
    db 0
	
	db 40, BUTTERFREE
	db 40, GOLDUCK
	db 41, ABRA
	db 42, KABUTO
	db $fe

    ;9
    db 0
	
	db 46, BUTTERFREE
	db 46, GOLDUCK
	db 47, ABRA
	db 48, KABUTO
	db $fe

    ;10
    db 0
	
	db 50, BUTTERFREE
	db 50, GOLDUCK
	db 51, ABRA
	db 52, KABUTO
	db $fe

    ;11
    db 0
	
	db 55, BUTTERFREE
	db 55, GOLDUCK
	db 56, ABRA
	db 57, KABUTO
	db $fe    

    ;12
    db 0
	
	db 58, BUTTERFREE
	db 58, GOLDUCK
	db 59, ABRA
	db 60, KABUTO
	db $fe

    ;13
    db 0
	
	db 62, BUTTERFREE
	db 62, GOLDUCK
	db 63, ABRA
	db 64, KABUTO
	db $fe   

    ;14
    db 0
	
	db 70, BUTTERFREE
	db 70, GOLDUCK
	db 71, ABRA
	db 72, KABUTO
	db $fe

    ;15
    db 0
	
	db 75, BUTTERFREE
	db 75, GOLDUCK
	db 76, ABRA
	db 77, KABUTO
	db $fe

    ;16
    db 0
	
	db 88, BUTTERFREE
	db 88, GOLDUCK
	db 89, ABRA
	db 90, KABUTO
	db $ff   	

	; BUG_CATCHER (12)
	db "DOUG@", TRAINERTYPE_NORMAL
	db 34, ARIADOS
	db $ff ; end

	; BUG_CATCHER (13)
	db "ARNIE@", TRAINERTYPE_NORMAL
	db 19, VENONAT
	db $ff ; end

	; BUG_CATCHER (14)
	db "ARNIE@", TRAINERTYPE_MOVES
	db 28, VENOMOTH,   DISABLE, SUPERSONIC, CONFUSION, LEECH_LIFE
	db $ff ; end

	; BUG_CATCHER (15)
	db "WADE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, CATERPIE
	db  5, PSYDUCK
	db  6, POLIWRATH
	db  7, OMASTAR
	db $fe 
	
	;1
	db 0
	
	db 14, METAPOD
	db 14, PSYDUCK
	db 15, POLIWRATH
	db 16, OMASTAR
	db $fe 
	
	;2
	db 0
	
	db 19, METAPOD
	db 19, PSYDUCK
	db 20, POLIWRATH
	db 21, OMASTAR
	db $fe 
	
	;3
	db 0
	
	db 24, METAPOD
	db 24, PSYDUCK
	db 25, POLIWRATH
	db 26, OMASTAR
	db $fe 
	
	;4
	db 0
	
	db 27, METAPOD
	db 27, PSYDUCK
	db 28, ABRA
	db 29, KABUTO
	db $fe 
	
	;5
	db 0
	
	db 29, BUTTERFREE
	db 29, GOLDUCK
	db 30, ABRA
	db 31, KABUTO
	db $fe 
	
	;6
	db 0
	
	db 31, BUTTERFREE
	db 31, GOLDUCK
	db 32, ABRA
	db 33, KABUTO
	db $fe 
	
	;7
	db 0
	
	db 35, BUTTERFREE
	db 35, GOLDUCK
	db 36, ABRA
	db 37, KABUTO
	db $fe

    ;8
    db 0
	
	db 40, BUTTERFREE
	db 40, GOLDUCK
	db 41, ABRA
	db 42, KABUTO
	db $fe

    ;9
    db 0
	
	db 46, BUTTERFREE
	db 46, GOLDUCK
	db 47, ABRA
	db 48, KABUTO
	db $fe

    ;10
    db 0
	
	db 50, BUTTERFREE
	db 50, GOLDUCK
	db 51, ABRA
	db 52, KABUTO
	db $fe

    ;11
    db 0
	
	db 55, BUTTERFREE
	db 55, GOLDUCK
	db 56, ABRA
	db 57, KABUTO
	db $fe    

    ;12
    db 0
	
	db 58, BUTTERFREE
	db 58, GOLDUCK
	db 59, ABRA
	db 60, KABUTO
	db $fe

    ;13
    db 0
	
	db 62, BUTTERFREE
	db 62, GOLDUCK
	db 63, ABRA
	db 64, KABUTO
	db $fe   

    ;14
    db 0
	
	db 70, BUTTERFREE
	db 70, GOLDUCK
	db 71, ABRA
	db 72, KABUTO
	db $fe

    ;15
    db 0
	
	db 75, BUTTERFREE
	db 75, GOLDUCK
	db 76, ABRA
	db 77, KABUTO
	db $fe

    ;16
    db 0
	
	db 88, BUTTERFREE
	db 88, GOLDUCK
	db 89, ABRA
	db 90, KABUTO
	db $ff   	

	; BUG_CATCHER (16)
	db "WADE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, CATERPIE
	db  5, PSYDUCK
	db  6, POLIWRATH
	db  7, OMASTAR
	db $fe 
	
	;1
	db 0
	
	db 14, METAPOD
	db 14, PSYDUCK
	db 15, POLIWRATH
	db 16, OMASTAR
	db $fe 
	
	;2
	db 0
	
	db 19, METAPOD
	db 19, PSYDUCK
	db 20, POLIWRATH
	db 21, OMASTAR
	db $fe 
	
	;3
	db 0
	
	db 24, METAPOD
	db 24, PSYDUCK
	db 25, POLIWRATH
	db 26, OMASTAR
	db $fe 
	
	;4
	db 0
	
	db 27, METAPOD
	db 27, PSYDUCK
	db 28, ABRA
	db 29, KABUTO
	db $fe 
	
	;5
	db 0
	
	db 29, BUTTERFREE
	db 29, GOLDUCK
	db 30, ABRA
	db 31, KABUTO
	db $fe 
	
	;6
	db 0
	
	db 31, BUTTERFREE
	db 31, GOLDUCK
	db 32, ABRA
	db 33, KABUTO
	db $fe 
	
	;7
	db 0
	
	db 35, BUTTERFREE
	db 35, GOLDUCK
	db 36, ABRA
	db 37, KABUTO
	db $fe

    ;8
    db 0
	
	db 40, BUTTERFREE
	db 40, GOLDUCK
	db 41, ABRA
	db 42, KABUTO
	db $fe

    ;9
    db 0
	
	db 46, BUTTERFREE
	db 46, GOLDUCK
	db 47, ABRA
	db 48, KABUTO
	db $fe

    ;10
    db 0
	
	db 50, BUTTERFREE
	db 50, GOLDUCK
	db 51, ABRA
	db 52, KABUTO
	db $fe

    ;11
    db 0
	
	db 55, BUTTERFREE
	db 55, GOLDUCK
	db 56, ABRA
	db 57, KABUTO
	db $fe    

    ;12
    db 0
	
	db 58, BUTTERFREE
	db 58, GOLDUCK
	db 59, ABRA
	db 60, KABUTO
	db $fe

    ;13
    db 0
	
	db 62, BUTTERFREE
	db 62, GOLDUCK
	db 63, ABRA
	db 64, KABUTO
	db $fe   

    ;14
    db 0
	
	db 70, BUTTERFREE
	db 70, GOLDUCK
	db 71, ABRA
	db 72, KABUTO
	db $fe

    ;15
    db 0
	
	db 75, BUTTERFREE
	db 75, GOLDUCK
	db 76, ABRA
	db 77, KABUTO
	db $fe

    ;16
    db 0
	
	db 88, BUTTERFREE
	db 88, GOLDUCK
	db 89, ABRA
	db 90, KABUTO
	db $ff

	; BUG_CATCHER (17)
	db "ARNIE@", TRAINERTYPE_MOVES
	db 36, VENOMOTH,   GUST, SUPERSONIC, PSYBEAM, LEECH_LIFE
	db $ff ; end

	; BUG_CATCHER (18)
	db "ARNIE@", TRAINERTYPE_MOVES
	db 40, VENOMOTH,   GUST, SUPERSONIC, PSYCHIC_M, TOXIC
	db $ff ; end

	; BUG_CATCHER (19)
	db "WAYNE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, METAPOD
	db 10, KADABRA
	db $fe
	
	;1
	db 0
	
	db 14, METAPOD
	db 14, KADABRA
	db $fe
	
	;2
	db 0
	
	db 20, METAPOD
	db 20, KADABRA
	db $fe
	
	;3
	db 0
	
	db 25, METAPOD
	db 25, KADABRA
	db $fe

	;4
	db 0
	
	db 29, METAPOD
	db 29, ALAKAZAM
	db $fe
	
	;5
	db 0
	
	db 31, BUTTERFREE
	db 31, ALAKAZAM
	db $fe
	
	;6
	db 0
	
	db 32, BUTTERFREE
	db 32, ALAKAZAM
	db $fe
	
	;7
	db 0
	
	db 36, BUTTERFREE
	db 36, ALAKAZAM
	db $fe

	;8
	db 0
	
	db 40, BUTTERFREE
	db 40, ALAKAZAM
	db $fe
	
	;9
	db 0
	
	db 46, BUTTERFREE
	db 46, ALAKAZAM
	db $fe
	
	;10
	db 0
	
	db 50, BUTTERFREE
	db 50, ALAKAZAM
	db $fe
	
	;11
	db 0
	
	db 56, BUTTERFREE
	db 56, ALAKAZAM
	db $fe
	
	;12
	db 0
	
	db 60, BUTTERFREE
	db 60, ALAKAZAM
	db $fe
	
	;13
	db 0
	
	db 64, BUTTERFREE
	db 64, ALAKAZAM
	db $fe
	
	;14
	db 0
	
	db 70, BUTTERFREE
	db 70, ALAKAZAM
	db $fe
	
	;15
	db 0
	
	db 77, BUTTERFREE
	db 77, ALAKAZAM
	db $fe
	
	;16
	db 0
	
	db 85, BUTTERFREE
	db 85, ALAKAZAM
	db $ff
	
FisherGroup:
	; FISHER (1)
	db "JUSTIN@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  7, NIDORAN_M
	db  7, NIDOKING
	db $fe
	
	;1
	db 0
	
	db  11, NIDORAN_M
	db  11, NIDOKING
	db $fe
	
	;2
	db 0
	
	db  18, NIDORAN_M
	db  18, NIDOKING
	db $fe
	
	;3
	db 0
	
	db 23, NIDORAN_M
	db 23, NIDOKING
	db $fe
	
	;4
	db 0
	
	db 27, NIDORINO
	db 27, CLEFAIRY
	db $fe

	;5
	db 0
	
	db 28, NIDORINO
	db 28, CLEFAIRY
	db $fe
	
	;6
	db 0
	
	db 30, NIDORINO
	db 30, CLEFAIRY
	db $fe
	
	;7
	db 0
	
	db 34, NIDORINO
	db 34, CLEFAIRY
	db $fe
	
	;8
	db 0
	
	db 39, NIDORINO
	db 39, CLEFAIRY
	db $fe
	
	;9
	db 0
	
	db 44, NIDORINO
	db 44, CLEFAIRY
	db $fe
	
	;10
	db 0
	
	db 49, NIDORINO
	db 49, CLEFAIRY
	db $fe
	
	;11
	db 0
	
	db 53, NIDORINO
	db 53, CLEFAIRY
	db $fe
	
	;12
	db 0
	
	db 57, NIDORINO
	db 57, CLEFAIRY
	db $fe
	
	;13
	db 0
	
	db 62, NIDORINO
	db 62, CLEFAIRY
	db $fe

    ;14
    db 0
	
	db 65, NIDORINO
	db 65, CLEFAIRY
	db $fe
	
	;15
	db 0
	
	db 73, NIDORINO
	db 73, CLEFAIRY
	db $fe
	
	;16
	db 0
	
	db 80, NIDORINO
	db 80, CLEFAIRY
	db $ff
	
	; FISHER (2)
	db "RALPH@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 9, ONIX
	db 9, EXEGGUTOR
	db $fe
    
	;1
	db 0
	
	db 12, ONIX
	db 12, EXEGGUTOR
	db $fe
	
	;2
	db 0
	
	db 19, ONIX
	db 19, EXEGGUTOR
	db $fe
	
	;3
	db 0
	
	db 24, ONIX
	db 24, EXEGGUTOR
	db $fe
	
	;4
	db 0
	
	db 28, ONIX
	db 28, EXEGGUTOR
	db $fe
	
	;5
	db 0
	
	db 30, DROWZEE
	db 30, CUBONE
	db $fe
	
	;6
	db 0
	
	db 32, DROWZEE
	db 32, CUBONE
	db $fe
	
	;7
	db 0
	
	db 36, DROWZEE
	db 36, CUBONE
	db $fe
	
	;8
	db 0
	
	db 41, DROWZEE
	db 41, CUBONE
	db $fe
	
	;9
	db 0
	
	db 46, DROWZEE
	db 46, CUBONE
	db $fe
	
	;10
	db 0
	
	db 50, DROWZEE
	db 50, CUBONE
	db $fe
	
	;11
	db 0
	
	db 55, DROWZEE
	db 55, CUBONE
	db $fe

    ;12
	db 0
	
	db 60, DROWZEE
	db 60, CUBONE
	db $fe
	
	;13
	db 0
	
	db 63, DROWZEE
	db 63, CUBONE
	db $fe
	
	;14
	db 0
	
	db 70, DROWZEE
	db 70, CUBONE
	db $fe
	
	;15
	db 0
	
	db 75, DROWZEE
	db 75, CUBONE
	db $fe
	
	;16
	db 0
	
	db 85, DROWZEE
	db 85, CUBONE
	db $ff




	; FISHER (3)
	db "ARNOLD@", TRAINERTYPE_NORMAL
	db 34, TENTACRUEL
	db $ff ; end

	; FISHER (4)
	db "KYLE@", TRAINERTYPE_NORMAL
	db 28, SEAKING
	db 31, POLIWHIRL
	db 31, SEAKING
	db $ff ; end

	; FISHER (5)
	db "HENRY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, MUK
	db  8, SCYTHER
	db $fe
	
	;1
	db 0
	
	db 12, MUK
	db 12, SCYTHER
	db $fe
	
	;2
	db 0
	
	db 18, MUK
	db 18, SCYTHER
	db $fe
	
	;3
	db 0
	
	db 24, MUK
	db 24, SCYTHER
	db $fe
	
	;4
	db 0
	
	db 29, SHELLDER
	db 29, JYNX
	db $fe
	
	;5
	db 0
	
	db 30, MUK
	db 30, SCYTHER
	db $fe
	
	;6
	db 0
	
	db 32, MUK
	db 32, SCYTHER
	db $fe
	
	;7
	db 0
	
	db 36, MUK
	db 36, SCYTHER
	db $fe
	
	;8
	db 0
	
	db 41, MUK
	db 41, SCYTHER
	db $fe
	
	;9
	db 0
	
	db 46, MUK
	db 46, SCYTHER
	db $fe
	
	;10
	db 0
	
	db 50, MUK
	db 50, SCYTHER
	db $fe
	
	;11
	db 0
	
	db 55, MUK
	db 55, SCYTHER
	db $fe
	
	;12
	db 0
	
	db 60, MUK
	db 60, SCYTHER
	db $fe
	
	;13
	db 0
	
	db 63, MUK
	db 63, SCYTHER
	db $fe
	
	;14
	db 0
	
	db 70, MUK
	db 70, SCYTHER
	db $fe
	
	;15
	db 0
	
	db 76, MUK
	db 76, SCYTHER
	db $fe
	
	;16
	db 0
	
	db 88, MUK
	db 88, SCYTHER
	db $ff

	; FISHER (6)
	db "MARVIN@", TRAINERTYPE_NORMAL
	db 10, MAGIKARP
	db 10, GYARADOS
	db 15, MAGIKARP
	db 15, GYARADOS
	db $ff ; end

	; FISHER (7)
	db "TULLY@", TRAINERTYPE_NORMAL
	db 18, QWILFISH
	db $ff ; end

	; FISHER (8)
	db "ANDRE@", TRAINERTYPE_NORMAL
	db 27, GYARADOS
	db $ff ; end

	; FISHER (9)
	db "RAYMOND@", TRAINERTYPE_NORMAL
	db 22, MAGIKARP
	db 22, MAGIKARP
	db 22, MAGIKARP
	db 22, MAGIKARP
	db $ff ; end

	; FISHER (10)
	db "WILTON@", TRAINERTYPE_NORMAL
	db 23, GOLDEEN
	db 23, GOLDEEN
	db 25, SEAKING
	db $ff ; end

	; FISHER (11)
	db "EDGAR@", TRAINERTYPE_MOVES
	db 25, REMORAID,   LOCK_ON, PSYBEAM, AURORA_BEAM, BUBBLEBEAM
	db 25, REMORAID,   LOCK_ON, PSYBEAM, AURORA_BEAM, BUBBLEBEAM
	db $ff ; end

	; FISHER (12)
	db "JONAH@", TRAINERTYPE_NORMAL
	db 25, SHELLDER
	db 29, OCTILLERY
	db 25, REMORAID
	db 29, CLOYSTER
	db $ff ; end

	; FISHER (13)
	db "MARTIN@", TRAINERTYPE_NORMAL
	db 32, REMORAID
	db 32, REMORAID
	db $ff ; end

	; FISHER (14)
	db "STEPHEN@", TRAINERTYPE_NORMAL
	db 25, MAGIKARP
	db 25, MAGIKARP
	db 31, QWILFISH
	db 31, TENTACRUEL
	db $ff ; end

	; FISHER (15)
	db "BARNEY@", TRAINERTYPE_NORMAL
	db 30, GYARADOS
	db 30, GYARADOS
	db 30, GYARADOS
	db $ff ; end

	; FISHER (16)
	db "RALPH@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 9, ONIX
	db 9, EXEGGUTOR
	db $fe
    
	;1
	db 0
	
	db 12, ONIX
	db 12, EXEGGUTOR
	db $fe
	
	;2
	db 0
	
	db 19, ONIX
	db 19, EXEGGUTOR
	db $fe
	
	;3
	db 0
	
	db 24, ONIX
	db 24, EXEGGUTOR
	db $fe
	
	;4
	db 0
	
	db 28, ONIX
	db 28, EXEGGUTOR
	db $fe
	
	;5
	db 0
	
	db 30, DROWZEE
	db 30, CUBONE
	db $fe
	
	;6
	db 0
	
	db 32, DROWZEE
	db 32, CUBONE
	db $fe
	
	;7
	db 0
	
	db 36, DROWZEE
	db 36, CUBONE
	db $fe
	
	;8
	db 0
	
	db 41, DROWZEE
	db 41, CUBONE
	db $fe
	
	;9
	db 0
	
	db 46, DROWZEE
	db 46, CUBONE
	db $fe
	
	;10
	db 0
	
	db 50, DROWZEE
	db 50, CUBONE
	db $fe
	
	;11
	db 0
	
	db 55, DROWZEE
	db 55, CUBONE
	db $fe

    ;12
	db 0
	
	db 60, DROWZEE
	db 60, CUBONE
	db $fe
	
	;13
	db 0
	
	db 63, DROWZEE
	db 63, CUBONE
	db $fe
	
	;14
	db 0
	
	db 70, DROWZEE
	db 70, CUBONE
	db $fe
	
	;15
	db 0
	
	db 75, DROWZEE
	db 75, CUBONE
	db $fe
	
	;16
	db 0
	
	db 85, DROWZEE
	db 85, CUBONE
	db $ff

	; FISHER (17)
	db "RALPH@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 9, ONIX
	db 9, EXEGGUTOR
	db $fe
    
	;1
	db 0
	
	db 12, ONIX
	db 12, EXEGGUTOR
	db $fe
	
	;2
	db 0
	
	db 19, ONIX
	db 19, EXEGGUTOR
	db $fe
	
	;3
	db 0
	
	db 24, ONIX
	db 24, EXEGGUTOR
	db $fe
	
	;4
	db 0
	
	db 28, ONIX
	db 28, EXEGGUTOR
	db $fe
	
	;5
	db 0
	
	db 30, DROWZEE
	db 30, CUBONE
	db $fe
	
	;6
	db 0
	
	db 32, DROWZEE
	db 32, CUBONE
	db $fe
	
	;7
	db 0
	
	db 36, DROWZEE
	db 36, CUBONE
	db $fe
	
	;8
	db 0
	
	db 41, DROWZEE
	db 41, CUBONE
	db $fe
	
	;9
	db 0
	
	db 46, DROWZEE
	db 46, CUBONE
	db $fe
	
	;10
	db 0
	
	db 50, DROWZEE
	db 50, CUBONE
	db $fe
	
	;11
	db 0
	
	db 55, DROWZEE
	db 55, CUBONE
	db $fe

    ;12
	db 0
	
	db 60, DROWZEE
	db 60, CUBONE
	db $fe
	
	;13
	db 0
	
	db 63, DROWZEE
	db 63, CUBONE
	db $fe
	
	;14
	db 0
	
	db 70, DROWZEE
	db 70, CUBONE
	db $fe
	
	;15
	db 0
	
	db 75, DROWZEE
	db 75, CUBONE
	db $fe
	
	;16
	db 0
	
	db 85, DROWZEE
	db 85, CUBONE
	db $ff

	; FISHER (18)
	db "TULLY@", TRAINERTYPE_NORMAL
	db 23, QWILFISH
	db $ff ; end

	; FISHER (19)
	db "TULLY@", TRAINERTYPE_NORMAL
	db 32, GOLDEEN
	db 32, GOLDEEN
	db 32, QWILFISH
	db $ff ; end

	; FISHER (20)
	db "WILTON@", TRAINERTYPE_NORMAL
	db 29, GOLDEEN
	db 29, GOLDEEN
	db 32, SEAKING
	db $ff ; end

	; FISHER (21)
	db "SCOTT@", TRAINERTYPE_NORMAL
	db 30, QWILFISH
	db 30, QWILFISH
	db 34, SEAKING
	db $ff ; end

	; FISHER (22)
	db "WILTON@", TRAINERTYPE_MOVES
	db 34, SEAKING,    SUPERSONIC, WATERFALL, FLAIL, FURY_ATTACK
	db 34, SEAKING,    SUPERSONIC, WATERFALL, FLAIL, FURY_ATTACK
	db 38, REMORAID,   PSYBEAM, AURORA_BEAM, BUBBLEBEAM, HYPER_BEAM
	db $ff ; end

	; FISHER (23)
	db "RALPH@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 9, ONIX
	db 9, EXEGGUTOR
	db $fe
    
	;1
	db 0
	
	db 12, ONIX
	db 12, EXEGGUTOR
	db $fe
	
	;2
	db 0
	
	db 19, ONIX
	db 19, EXEGGUTOR
	db $fe
	
	;3
	db 0
	
	db 24, ONIX
	db 24, EXEGGUTOR
	db $fe
	
	;4
	db 0
	
	db 28, ONIX
	db 28, EXEGGUTOR
	db $fe
	
	;5
	db 0
	
	db 30, DROWZEE
	db 30, CUBONE
	db $fe
	
	;6
	db 0
	
	db 32, DROWZEE
	db 32, CUBONE
	db $fe
	
	;7
	db 0
	
	db 36, DROWZEE
	db 36, CUBONE
	db $fe
	
	;8
	db 0
	
	db 41, DROWZEE
	db 41, CUBONE
	db $fe
	
	;9
	db 0
	
	db 46, DROWZEE
	db 46, CUBONE
	db $fe
	
	;10
	db 0
	
	db 50, DROWZEE
	db 50, CUBONE
	db $fe
	
	;11
	db 0
	
	db 55, DROWZEE
	db 55, CUBONE
	db $fe

    ;12
	db 0
	
	db 60, DROWZEE
	db 60, CUBONE
	db $fe
	
	;13
	db 0
	
	db 63, DROWZEE
	db 63, CUBONE
	db $fe
	
	;14
	db 0
	
	db 70, DROWZEE
	db 70, CUBONE
	db $fe
	
	;15
	db 0
	
	db 75, DROWZEE
	db 75, CUBONE
	db $fe
	
	;16
	db 0
	
	db 85, DROWZEE
	db 85, CUBONE
	db $ff

	; FISHER (24)
	db "RALPH@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 9, ONIX
	db 9, EXEGGUTOR
	db $fe
    
	;1
	db 0
	
	db 12, ONIX
	db 12, EXEGGUTOR
	db $fe
	
	;2
	db 0
	
	db 19, ONIX
	db 19, EXEGGUTOR
	db $fe
	
	;3
	db 0
	
	db 24, ONIX
	db 24, EXEGGUTOR
	db $fe
	
	;4
	db 0
	
	db 28, ONIX
	db 28, EXEGGUTOR
	db $fe
	
	;5
	db 0
	
	db 30, DROWZEE
	db 30, CUBONE
	db $fe
	
	;6
	db 0
	
	db 32, DROWZEE
	db 32, CUBONE
	db $fe
	
	;7
	db 0
	
	db 36, DROWZEE
	db 36, CUBONE
	db $fe
	
	;8
	db 0
	
	db 41, DROWZEE
	db 41, CUBONE
	db $fe
	
	;9
	db 0
	
	db 46, DROWZEE
	db 46, CUBONE
	db $fe
	
	;10
	db 0
	
	db 50, DROWZEE
	db 50, CUBONE
	db $fe
	
	;11
	db 0
	
	db 55, DROWZEE
	db 55, CUBONE
	db $fe

    ;12
	db 0
	
	db 60, DROWZEE
	db 60, CUBONE
	db $fe
	
	;13
	db 0
	
	db 63, DROWZEE
	db 63, CUBONE
	db $fe
	
	;14
	db 0
	
	db 70, DROWZEE
	db 70, CUBONE
	db $fe
	
	;15
	db 0
	
	db 75, DROWZEE
	db 75, CUBONE
	db $fe
	
	;16
	db 0
	
	db 85, DROWZEE
	db 85, CUBONE
	db $ff

	; FISHER (25)
	db "TULLY@", TRAINERTYPE_MOVES
	db 34, SEAKING,    SUPERSONIC, RAIN_DANCE, WATERFALL, FURY_ATTACK
	db 34, SEAKING,    SUPERSONIC, RAIN_DANCE, WATERFALL, FURY_ATTACK
	db 37, QWILFISH,   ROLLOUT, SURF, PIN_MISSILE, TAKE_DOWN
	db $ff ; end


SwimmerMGroup:
	; SWIMMERM (1)
	db "HAROLD@", TRAINERTYPE_NORMAL
	db 32, REMORAID
	db 30, SEADRA
	db $ff ; end

	; SWIMMERM (2)
	db "SIMON@", TRAINERTYPE_NORMAL
	db 20, TENTACOOL
	db 20, TENTACOOL
	db $ff ; end

	; SWIMMERM (3)
	db "RANDALL@", TRAINERTYPE_NORMAL
	db 18, SHELLDER
	db 20, WARTORTLE
	db 18, SHELLDER
	db $ff ; end

	; SWIMMERM (4)
	db "CHARLIE@", TRAINERTYPE_NORMAL
	db 21, SHELLDER
	db 19, TENTACOOL
	db 19, TENTACRUEL
	db $ff ; end

	; SWIMMERM (5)
	db "GEORGE@", TRAINERTYPE_NORMAL
	db 16, TENTACOOL
	db 17, TENTACOOL
	db 16, TENTACOOL
	db 19, STARYU
	db 17, TENTACOOL
	db 19, REMORAID
	db $ff ; end

	; SWIMMERM (6)
	db "BERKE@", TRAINERTYPE_NORMAL
	db 23, QWILFISH
	db $ff ; end

	; SWIMMERM (7)
	db "KIRK@", TRAINERTYPE_NORMAL
	db 20, GYARADOS
	db 20, GYARADOS
	db $ff ; end

	; SWIMMERM (8)
	db "MATHEW@", TRAINERTYPE_NORMAL
	db 23, KRABBY
	db $ff ; end

	; SWIMMERM (9)
	db "HAL@", TRAINERTYPE_NORMAL
	db 24, SEEL
	db 25, DEWGONG
	db 24, SEEL
	db $ff ; end

	; SWIMMERM (10)
	db "PATON@", TRAINERTYPE_NORMAL
	db 26, PILOSWINE
	db 26, PILOSWINE
	db $ff ; end

	; SWIMMERM (11)
	db "DARYL@", TRAINERTYPE_NORMAL
	db 24, SHELLDER
	db 25, CLOYSTER
	db 24, SHELLDER
	db $ff ; end

	; SWIMMERM (12)
	db "WALTER@", TRAINERTYPE_NORMAL
	db 15, HORSEA
	db 15, HORSEA
	db 20, SEADRA
	db $ff ; end

	; SWIMMERM (13)
	db "TONY@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db $ff ; end

	; SWIMMERM (14)
	db "JEROME@", TRAINERTYPE_NORMAL
	db 26, SEADRA
	db 28, TENTACOOL
	db 30, TENTACRUEL
	db 28, GOLDEEN
	db $ff ; end

	; SWIMMERM (15)
	db "TUCKER@", TRAINERTYPE_NORMAL
	db 30, SHELLDER
	db 34, CLOYSTER
	db $ff ; end

	; SWIMMERM (16)
	db "RICK@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db $ff ; end

	; SWIMMERM (17)
	db "CAMERON@", TRAINERTYPE_NORMAL
	db 34, MARILL
	db $ff ; end

	; SWIMMERM (18)
	db "SETH@", TRAINERTYPE_NORMAL
	db 29, QUAGSIRE
	db 29, OCTILLERY
	db 32, QUAGSIRE
	db $ff ; end

	; SWIMMERM (19)
	db "JAMES@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db $ff ; end

	; SWIMMERM (20)
	db "LEWIS@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db $ff ; end

	; SWIMMERM (21)
	db "PARKER@", TRAINERTYPE_NORMAL
	db 32, HORSEA
	db 32, HORSEA
	db 35, SEADRA
	db $ff ; end


SwimmerFGroup:
	; SWIMMERF (1)
	db "ELAINE@", TRAINERTYPE_NORMAL
	db 21, STARYU
	db $ff ; end

	; SWIMMERF (2)
	db "PAULA@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 12, WEEZING
	db $fe
	
	;1
	db 0
	
	db 17, WEEZING
	db $fe
	
	;2
	db 0
	
	db 22, WEEZING
	db $fe
	
	;3
	db 0
	
	db 26, RHYHORN
	db $fe
	
	;4
	db 0
	
	db 30, RHYHORN
	db $fe
	
	;5
	db 0
	
	db 32, RHYHORN
	db $fe
	
	;6
	db 0
	
	db 34, RHYHORN
	db $fe
	
    ;7
	db 0
	
	db 38, RHYHORN
	db $fe
	
	;8
	db 0
	
	db 43, RHYHORN
	db $fe
	
	;9
	db 0
	
	db 48, RHYHORN
	db $fe
	
	;10
	db 0
	
	db 52, RHYHORN
	db $fe
	
	;11
	db 0
	
	db 57, RHYHORN
	db $fe
	
	;12
	db 0
	
	db 62, RHYHORN
	db $fe
	
	;13
	db 0
	
	db 64, RHYHORN
	db $fe
	
	;14
	db 0
	
	db 73, RHYHORN
	db $fe
	
	;15
	db 0
	
	db 79, RHYHORN
	db $fe
	
	;16
	db 0
	
	db 95, RHYHORN
	db $ff
	
	; SWIMMERF (3)
	db "KAYLEE@", TRAINERTYPE_NORMAL
	db 18, GOLDEEN
	db 20, GOLDEEN
	db 20, SEAKING
	db $ff ; end

	; SWIMMERF (4)
	db "SUSIE@", TRAINERTYPE_MOVES
	db 20, PSYDUCK,    SCRATCH, TAIL_WHIP, DISABLE, CONFUSION
	db 22, GOLDEEN,    PECK, TAIL_WHIP, SUPERSONIC, HORN_ATTACK
	db $ff ; end

	; SWIMMERF (5)
	db "DENISE@", TRAINERTYPE_NORMAL
	db 22, SEEL
	db $ff ; end

	; SWIMMERF (6)
	db "KARA@", TRAINERTYPE_NORMAL
	db 20, STARYU
	db 20, STARMIE
	db $ff ; end

	; SWIMMERF (7)
	db "WENDY@", TRAINERTYPE_MOVES
	db 21, HORSEA,     BUBBLE, SMOKESCREEN, LEER, WATER_GUN
	db 21, HORSEA,     DRAGON_RAGE, SMOKESCREEN, LEER, WATER_GUN
	db $ff ; end

	; SWIMMERF (8)
	db "LISA@", TRAINERTYPE_NORMAL
	db 28, JYNX
	db $ff ; end

	; SWIMMERF (9)
	db "JILL@", TRAINERTYPE_NORMAL
	db 28, DEWGONG
	db $ff ; end

	; SWIMMERF (10)
	db "MARY@", TRAINERTYPE_NORMAL
	db 20, SEAKING
	db $ff ; end

	; SWIMMERF (11)
	db "KATIE@", TRAINERTYPE_NORMAL
	db 33, DEWGONG
	db $ff ; end

	; SWIMMERF (12)
	db "DAWN@", TRAINERTYPE_NORMAL
	db 34, SEAKING
	db $ff ; end

	; SWIMMERF (13)
	db "TARA@", TRAINERTYPE_NORMAL
	db 20, SEAKING
	db $ff ; end

	; SWIMMERF (14)
	db "NICOLE@", TRAINERTYPE_NORMAL
	db 29, MARILL
	db 29, MARILL
	db 32, LAPRAS
	db $ff ; end

	; SWIMMERF (15)
	db "LORI@", TRAINERTYPE_NORMAL
	db 32, STARMIE
	db 32, STARMIE
	db $ff ; end

	; SWIMMERF (16)
	db "JODY@", TRAINERTYPE_NORMAL
	db 20, SEAKING
	db $ff ; end

	; SWIMMERF (17)
	db "NIKKI@", TRAINERTYPE_NORMAL
	db 28, SEEL
	db 28, SEEL
	db 28, SEEL
	db 28, DEWGONG
	db $ff ; end

	; SWIMMERF (18)
	db "DIANA@", TRAINERTYPE_NORMAL
	db 37, GOLDUCK
	db $ff ; end

	; SWIMMERF (19)
	db "BRIANA@", TRAINERTYPE_NORMAL
	db 35, SEAKING
	db 35, SEAKING
	db $ff ; end


SailorGroup:
	; SAILOR (1)
	db "EUGENE@", TRAINERTYPE_NORMAL
	db 17, POLIWHIRL
	db 17, RATICATE
	db 19, KRABBY
	db $ff ; end

	; SAILOR (2)
	db "HUEY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, SEAKING
    db $fe
	
	;1
	db 0
	
	db 17, SEAKING
    db $fe
	
	;2
	db 0
	
	db 22, STARYU
    db $fe
	
	;3
	db 0
	
	db 26, STARYU
    db $fe
	
	;4
	db 0
	
	db 30, STARYU
    db $fe
	
	;5
	db 0
	
	db 31, STARYU
    db $fe
	
	;6
	db 0
	
	db 33, STARYU
    db $fe
	
	;7
	db 0
	
	db 38, STARYU
    db $fe
	
	;8
	db 0
	
	db 43, STARYU
    db $fe
	
	;9
	db 0
	
	db 48, STARYU
    db $fe
	
	;10
	db 0
	
	db 52, STARYU
    db $fe
	
	;11
	db 0
	
	db 57, STARYU
    db $fe
	
	;12
	db 0
	
	db 62, STARYU
    db $fe
	
	;13
	db 0
	
	db 64, STARYU
    db $fe
	
	;14
	db 0
	
	db 73, STARYU
    db $fe
	
	;15
	db 0
	
	db 79, STARYU
    db $fe

	;16
	db 0
	
	db 85, STARYU
    db $ff
	
	; SAILOR (3)
	db "TERRELL@", TRAINERTYPE_NORMAL
	db 20, POLIWHIRL
	db $ff ; end

	; SAILOR (4)
	db "KENT@", TRAINERTYPE_MOVES
	db 18, KRABBY,     BUBBLE, LEER, VICEGRIP, HARDEN
	db 20, KRABBY,     BUBBLEBEAM, LEER, VICEGRIP, HARDEN
	db $ff ; end

	; SAILOR (5)
	db "ERNEST@", TRAINERTYPE_NORMAL
	db 18, MACHOP
	db 18, MACHOP
	db 18, POLIWHIRL
	db $ff ; end

	; SAILOR (6)
	db "JEFF@", TRAINERTYPE_NORMAL
	db 32, RATICATE
	db 32, RATICATE
	db $ff ; end

	; SAILOR (7)
	db "GARRETT@", TRAINERTYPE_NORMAL
	db 34, KINGLER
	db $ff ; end

	; SAILOR (8)
	db "KENNETH@", TRAINERTYPE_NORMAL
	db 28, MACHOP
	db 28, MACHOP
	db 28, POLIWRATH
	db 28, MACHOP
	db $ff ; end

	; SAILOR (9)
	db "STANLY@", TRAINERTYPE_NORMAL
	db 31, MACHOP
	db 33, MACHOKE
	db 26, PSYDUCK
	db $ff ; end

	; SAILOR (10)
	db "HARRY@", TRAINERTYPE_NORMAL
	db 19, WOOPER
	db $ff ; end

	; SAILOR (11)
	db "HUEY@", TRAINERTYPE_NORMAL
	db 28, POLIWHIRL
	db 28, POLIWHIRL
	db $ff ; end

	; SAILOR (12)
	db "HUEY@", TRAINERTYPE_NORMAL
	db 34, POLIWHIRL
	db 34, POLIWRATH
	db $ff ; end

	; SAILOR (13)
	db "HUEY@", TRAINERTYPE_MOVES
	db 38, POLITOED,   WHIRLPOOL, RAIN_DANCE, BODY_SLAM, PERISH_SONG
	db 38, POLIWRATH,  SURF, STRENGTH, ICE_PUNCH, SUBMISSION
	db $ff ; end


SuperNerdGroup:
	; SUPER_NERD (1)
	db "STAN@", TRAINERTYPE_NORMAL
	db 20, GRIMER
	db $ff ; end

	; SUPER_NERD (2)
	db "ERIC@", TRAINERTYPE_NORMAL
	db 11, GRIMER
	db 11, GRIMER
	db $ff ; end

	; SUPER_NERD (3)
	db "GREGG@", TRAINERTYPE_NORMAL
	db 20, MAGNEMITE
	db 20, MAGNEMITE
	db 20, MAGNEMITE
	db $ff ; end

	; SUPER_NERD (4)
	db "JAY@", TRAINERTYPE_NORMAL
	db 22, KOFFING
	db 22, KOFFING
	db $ff ; end

	; SUPER_NERD (5)
	db "DAVE@", TRAINERTYPE_NORMAL
	db 24, DITTO
	db $ff ; end

	; SUPER_NERD (6)
	db "SAM@", TRAINERTYPE_NORMAL
	db 34, GRIMER
	db 34, MUK
	db $ff ; end

	; SUPER_NERD (7)
	db "TOM@", TRAINERTYPE_NORMAL
	db 32, MAGNEMITE
	db 32, MAGNEMITE
	db 32, MAGNEMITE
	db $ff ; end

	; SUPER_NERD (8)
	db "PAT@", TRAINERTYPE_NORMAL
	db 36, PORYGON
	db $ff ; end

	; SUPER_NERD (9)
	db "SHAWN@", TRAINERTYPE_NORMAL
	db 31, MAGNEMITE
	db 33, MUK
	db 31, MAGNEMITE
	db $ff ; end

	; SUPER_NERD (10)
	db "TERU@", TRAINERTYPE_NORMAL
	db  7, MAGNEMITE
	db 11, VOLTORB
	db  7, MAGNEMITE
	db  9, MAGNEMITE
	db $ff ; end

	; SUPER_NERD (11)
	db "RUSS@", TRAINERTYPE_NORMAL
	db 27, MAGNEMITE
	db 27, MAGNEMITE
	db 27, MAGNEMITE
	db $ff ; end

	; SUPER_NERD (12)
	db "NORTON@", TRAINERTYPE_MOVES
	db 30, PORYGON,    CONVERSION, QUIVER_DANCE, RECOVER, TRI_ATTACK
	db $ff ; end

	; SUPER_NERD (13)
	db "HUGH@", TRAINERTYPE_MOVES
	db 39, SEADRA,     SMOKESCREEN, TWISTER, SURF, WATERFALL
	db $ff ; end

	; SUPER_NERD (14)
	db "MARKUS@", TRAINERTYPE_MOVES
	db 19, SLOWPOKE,   CURSE, WATER_GUN, GROWL, STRENGTH
	db $ff ; end


Rival2Group:
	; RIVAL2 (1)
	db "?@", TRAINERTYPE_MOVES
	db 41, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 42, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 41, MAGNETON,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 43, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 43, ALAKAZAM,   DISABLE, RECOVER, FUTURE_SIGHT, PSYCHIC_M
	db 45, MEGANIUM,   RAZOR_LEAF, POISONPOWDER, BODY_SLAM, LIGHT_SCREEN
	db $ff ; end

	; RIVAL2 (2)
	db "?@", TRAINERTYPE_MOVES
	db 41, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 42, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 41, MAGNETON,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 43, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 43, ALAKAZAM,   DISABLE, RECOVER, FUTURE_SIGHT, PSYCHIC_M
	db 45, TYPHLOSION, SMOKESCREEN, QUICK_ATTACK, FLAME_WHEEL, SWIFT
	db $ff ; end

	; RIVAL2 (3)
	db "?@", TRAINERTYPE_MOVES
	db 41, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 42, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 41, MAGNETON,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 43, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 43, ALAKAZAM,   DISABLE, RECOVER, FUTURE_SIGHT, PSYCHIC_M
	db 45, FERALIGATR, RAGE, WATER_GUN, SCARY_FACE, SLASH
	db $ff ; end

	; RIVAL2 (4)
	db "?@", TRAINERTYPE_MOVES
	db 45, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 48, CROBAT,     TOXIC, BITE, CONFUSE_RAY, WING_ATTACK
	db 45, MAGNETON,   THUNDER, SONICBOOM, THUNDER_WAVE, SWIFT
	db 46, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 46, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db 50, MEGANIUM,   GIGA_DRAIN, BODY_SLAM, LIGHT_SCREEN, SAFEGUARD
	db $ff ; end

	; RIVAL2 (5)
	db "?@", TRAINERTYPE_MOVES
	db 45, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 48, CROBAT,     TOXIC, BITE, CONFUSE_RAY, WING_ATTACK
	db 45, MAGNETON,   THUNDER, SONICBOOM, THUNDER_WAVE, SWIFT
	db 46, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 46, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db 50, TYPHLOSION, SMOKESCREEN, QUICK_ATTACK, FIRE_BLAST, SWIFT
	db $ff ; end

	; RIVAL2 (6)
	db "?@", TRAINERTYPE_MOVES
	db 45, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 48, CROBAT,     TOXIC, BITE, CONFUSE_RAY, WING_ATTACK
	db 45, MAGNETON,   THUNDER, SONICBOOM, THUNDER_WAVE, SWIFT
	db 46, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 46, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db 50, FERALIGATR, SURF, RAIN_DANCE, SLASH, SCREECH
	db $ff ; end


GuitaristGroup:
	; GUITARIST (1)
	db "CLYDE@", TRAINERTYPE_NORMAL
	db 34, ELECTABUZZ
	db $ff ; end
	
	db "ABE@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  8, SANDSLASH
	db  9, KABUTOPS
	db 10, NIDORINA
	db 11, WEEDLE
	db $fe
	
	;1
	db 0
	
	db 11, SANDSLASH
	db 11, KABUTOPS
	db 13, NIDORINA
	db 14, WEEDLE
	db $fe
	
	;2
	db 0
	
	db 18, SANDSLASH
	db 19, KABUTOPS
	db 20, NIDORINA
	db 20, WEEDLE
	db $fe
	
	;3
	db 0
	
	db 24, SANDSLASH
	db 24, AERODACTYL
	db 25, NIDORINA
	db 25, WEEDLE
	db $fe
	
	;4
	db 0
	
	db 27, SANDSLASH
	db 27, AERODACTYL
	db 28, NIDORINA
	db 29, KAKUNA
	db $fe

	;5
	db 0
	
	db 28, SANDSLASH
	db 28, AERODACTYL
	db 29, NIDORINA
	db 30, KAKUNA
	db $fe
	
	;6
	db 0
	
	db 30, NIDORAN_F
	db 30, SNORLAX
	db 31, NIDOQUEEN
	db 32, KAKUNA
	db $fe
	
	;7
	db 0
	
	db 34, NIDORAN_F
	db 34, SNORLAX
	db 35, NIDOQUEEN
	db 36, KAKUNA
	db $fe
	
	;8
	db 0
	
	db 39, NIDORAN_F
	db 39, SNORLAX
	db 40, NIDOQUEEN
	db 41, KAKUNA
	db $fe
	
	;9
	db 0
	
	db 44, NIDORAN_F
	db 44, SNORLAX
	db 45, NIDOQUEEN
	db 46, KAKUNA
	db $fe
	
	;10
	db 0
	
	db 49, NIDORAN_F
	db 49, SNORLAX
	db 50, NIDOQUEEN
	db 51, KAKUNA
	db $fe
	
	;11
	db 0
	
	db 53, NIDORAN_F
	db 53, SNORLAX
	db 54, NIDOQUEEN
	db 55, KAKUNA
	db $fe
	
	;12
	db 0
	
	db 57, NIDORAN_F
	db 57, SNORLAX
	db 58, NIDOQUEEN
	db 59, KAKUNA
	db $fe
	
	;13
	db 0
	
	db 62, NIDORAN_F
	db 62, SNORLAX
	db 63, NIDOQUEEN
	db 64, KAKUNA
	db $fe
	
	;14
	db 0
	
	db 65, NIDORAN_F
	db 65, SNORLAX
	db 66, NIDOQUEEN
	db 67, KAKUNA
	db $fe
	
	;15
	db 0
	
	db 75, NIDORAN_F
	db 75, SNORLAX
	db 76, NIDOQUEEN
	db 79, KAKUNA
	db $fe
	
	;16
	db 0
	
	db 80, NIDORAN_F
	db 80, SNORLAX
	db 84, NIDOQUEEN
	db 86, KAKUNA
	db $ff
	
	; GUITARIST (2)
	db "VINCENT@", TRAINERTYPE_NORMAL
	db 27, MAGNEMITE
	db 33, VOLTORB
	db 32, MAGNEMITE
	db 32, MAGNEMITE
	db $ff ; end


HikerGroup:
	; HIKER (1)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 16, GEODUDE
	db 18, MACHAMP
	db $ff ; end

	; HIKER (2)
	db "RUSSELL@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  5, PIDGEOTTO
	db  6, CLEFABLE
	db  7, ARBOK 
	db $fe
	
	
	;1
	db 0
	
	db  13, PIDGEOTTO
	db  14, CLEFABLE
	db  16, ARBOK 
	db $fe
	
	
	;2
	db 0
	
	db  15, PIDGEOTTO
	db  16, CLEFABLE
	db  19, ARBOK 
	db $fe
	
	
	;3
	db 0
	
	db  20, PIDGEOT
	db  21, CLEFABLE
	db  23, ARBOK 
	db  25, NINETALES
	db $fe
	
	
	;4
	db 0
	
	db  24, PIDGEOT
	db  25, VULPIX
	db  26, PIKACHU
	db  26, JIGGLYPUFF
	db $fe
	
	
	;5
	db 0
	
	db  27, PIDGEOT
	db  27, VULPIX
	db  28, PIKACHU 
	db  30, JIGGLYPUFF
	db $fe

	
	;6
	db 0
	
	db  32, RATTATA
	db  32, VULPIX
	db  33, PIKACHU 
	db  33, JIGGLYPUFF
	db $fe
	
	
	;7
	db 0
	
	db  35, RATTATA
	db  36, VULPIX
	db  38, PIKACHU 
	db  40, WIGGLYTUFF
	db $fe
	
	
	;8
	db 0
	
	db  40, RATTATA
	db  40, VULPIX
	db  42, PIKACHU 
	db  45, WIGGLYTUFF
	db $fe
	
	
	;9
	db 0
	
	db  45, RATTATA
	db  45, VULPIX
	db  46, PIKACHU 
	db  47, WIGGLYTUFF
	db $fe
	
	
	;10
	db 0
	
	db  48, RATTATA
	db  48, VULPIX
	db  49, PIKACHU 
	db  50, WIGGLYTUFF
	db $fe
	
	
	;11
	db 0
	
	db  50, RATTATA
	db  50, VULPIX
	db  51, PIKACHU 
	db  54, WIGGLYTUFF
	db $fe
	
	
	;12
	db 0
	
	db  52, RATTATA
	db  52, VULPIX
	db  53, PIKACHU 
	db  56, WIGGLYTUFF
	db $fe
	
	
	;13
	db 0
	
	db  56, RATTATA
	db  56, VULPIX
	db  58, PIKACHU 
	db  60, WIGGLYTUFF
	db $fe
	
	;14
	db 0
	
	db  59, RATTATA
	db  59, VULPIX
	db  60, PIKACHU 
	db  63, WIGGLYTUFF
	db $fe
	
	;15
	db 0
	
	db  64, RATTATA
	db  64, VULPIX
	db  69, PIKACHU 
	db  74, WIGGLYTUFF
	db $fe
	
	;16
	db 0
	
	db  80, RATTATA
	db  80, VULPIX
	db  84, PIKACHU 
	db  90, WIGGLYTUFF
	db $ff
	
	; HIKER (3)
	db "PHILLIP@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, ARBOK
	db 10, ARBOK
	db 10, MAGNETON
	db $fe
	
	;1
	db 0
	
	db 16, ARBOK
	db 16, ARBOK
	db 16, MAGNETON
	db $fe
	
	;2
	db 0
	
	db 20, ARBOK
	db 20, ARBOK
	db 20, MAGNETON
	db $fe
	
    ;3
	db 0
	
	db 23, ARBOK
	db 23, ARBOK
	db 23, MAGNETON
	db $fe
	
	;4
	db 0
	
	db 26, ARBOK
	db 26, ARBOK
	db 26, MAGNETON
	db $fe
	
	;5
	db 0
	
	db 30, PIKACHU
	db 30, PIKACHU
	db 30, FARFETCH_D
	db $fe
	
	;6
	db 0
	
	db 34, PIKACHU
	db 34, PIKACHU
	db 34, FARFETCH_D
	db $fe
	
	;7
	db 0
	
	db 38, PIKACHU
	db 38, PIKACHU
	db 38, FARFETCH_D
	db $fe
	
	;8
	db 0
	
	db 41, PIKACHU
	db 41, PIKACHU
	db 41, FARFETCH_D
	db $fe
	
	;9
	db 0
	
	db 45, PIKACHU
	db 45, PIKACHU
	db 45, FARFETCH_D
	db $fe
	
	;10
	db 0
	
	db 49, PIKACHU
	db 49, PIKACHU
	db 49, FARFETCH_D
	db $fe
	
	;11
	db 0
	
	db 52, PIKACHU
	db 52, PIKACHU
	db 52, FARFETCH_D
	db $fe
	
	;12
	db 0
	
	db 57, PIKACHU
	db 57, PIKACHU
	db 57, FARFETCH_D
	db $fe
	
	;13
	db 0
	
	db 61, PIKACHU
	db 61, PIKACHU
	db 61, FARFETCH_D
	db $fe
	
	;14
	db 0
	
	db 68, PIKACHU
	db 68, PIKACHU
	db 68, FARFETCH_D
	db $fe
	
	;15
	db 0
	
	db 74, PIKACHU
	db 74, PIKACHU
	db 74, FARFETCH_D
	db $fe
	
	;16
	db 0
	
	db 88, PIKACHU
	db 88, PIKACHU
	db 88, FARFETCH_D
	db $ff
	
	; HIKER (4)
	db "LEONARD@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 10, SEEL 
	db 10, SEEL
	db 10, SLOWBRO
	db $fe
	
	;1
	db 0
	
	db 20, SEEL
	db 20, SEEL
	db 20, SLOWBRO
	db $fe
	
    ;3
	db 0
	
	db 23, SEEL
	db 23, SEEL
	db 23, SLOWBRO
	db $fe
	
	;4
	db 0
	
	db 26, SEEL
	db 26, SEEL
	db 26, SLOWBRO
	db $fe
	
	;5
	db 0
	
	db 30, DEWGONG
	db 30, DEWGONG
	db 30, MAGNEMITE
	db $fe
	
	;6
	db 0
	
	db 34, DEWGONG
	db 34, DEWGONG
	db 34, MAGNEMITE
	db $fe
	
	;7
	db 0
	
	db 38, DEWGONG
	db 38, DEWGONG
	db 38, MAGNEMITE
	db $fe
	
	;8
	db 0
	
	db 41, DEWGONG
	db 41, DEWGONG
	db 41, MAGNEMITE
	db $fe
	
	;9
	db 0
	
	db 45, DEWGONG
	db 45, DEWGONG
	db 45, MAGNEMITE
	db $fe
	
	;10
	db 0
	
	db 49, DEWGONG
	db 49, DEWGONG
	db 49, MAGNEMITE
	db $fe
	
	;11
	db 0
	
	db 52, DEWGONG
	db 52, DEWGONG
	db 52, MAGNEMITE
	db $fe
	
	;12
	db 0
	
	db 57, DEWGONG
	db 57, DEWGONG
	db 57, MAGNEMITE
	db $fe
	
	;13
	db 0
	
	db 61, DEWGONG
	db 61, DEWGONG
	db 61, MAGNEMITE
	db $fe
	
	;14
	db 0
	
	db 68, DEWGONG
	db 68, DEWGONG
	db 68, MAGNEMITE
	db $fe
	
	;15
	db 0
	
	db 74, DEWGONG
	db 74, DEWGONG
	db 74, MAGNEMITE
	db $fe
	
	;16
	db 0
	
	db 88, DEWGONG
	db 88, DEWGONG
	db 88, MAGNEMITE
	db $ff

	; HIKER (5)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 11, GEODUDE
	db 11, MACHOP
	db $ff ; end

	; HIKER (6)
	db "BENJAMIN@", TRAINERTYPE_NORMAL
	db 14, DIGLETT
	db 14, GEODUDE
	db 16, DUGTRIO
	db $ff ; end

	; HIKER (7)
	db "ERIK@", TRAINERTYPE_NORMAL
	db 24, MACHOP
	db 27, GRAVELER
	db 27, MACHOP
	db $ff ; end

	; HIKER (8)
	db "MICHAEL@", TRAINERTYPE_NORMAL
	db 25, GEODUDE
	db 25, GRAVELER
	db 25, GOLEM
	db $ff ; end

	; HIKER (9)
	db "PARRY@", TRAINERTYPE_NORMAL
	db 35, ONIX
	db 33, SWINUB
	db $ff ; end

	; HIKER (10)
	db "TIMOTHY@", TRAINERTYPE_MOVES
	db 27, DIGLETT,    MAGNITUDE, DIG, SAND_ATTACK, SLASH
	db 27, DUGTRIO,    MAGNITUDE, DIG, SAND_ATTACK, SLASH
	db $ff ; end

	; HIKER (11)
	db "BAILEY@", TRAINERTYPE_NORMAL
	db 13, GEODUDE
	db 13, GEODUDE
	db 13, GEODUDE
	db 13, GEODUDE
	db 13, GEODUDE
	db $ff ; end

	; HIKER (12)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 25, GRAVELER
	db 27, GRAVELER
	db 29, MACHOKE
	db $ff ; end

	; HIKER (13)
	db "TIM@", TRAINERTYPE_NORMAL
	db 31, GRAVELER
	db 31, GRAVELER
	db 31, GRAVELER
	db $ff ; end

	; HIKER (14)
	db "NOLAND@", TRAINERTYPE_NORMAL
	db 31, SANDSLASH
	db 33, GOLEM
	db $ff ; end

	; HIKER (15)
	db "SIDNEY@", TRAINERTYPE_NORMAL
	db 34, DUGTRIO
	db 32, ONIX
	db $ff ; end

	; HIKER (16)
	db "KENNY@", TRAINERTYPE_NORMAL
	db 27, SANDSLASH
	db 29, GRAVELER
	db 31, GOLEM
	db 29, GRAVELER
	db $ff ; end

	; HIKER (17)
	db "JIM@", TRAINERTYPE_NORMAL
	db 35, MACHAMP
	db $ff ; end

	; HIKER (18)
	db "DANIEL@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 8, CLEFABLE
	db $fe
	
	;1
	db 0
	
	db 16, CLEFABLE
	db $fe
	
	;2
	db 0
	
	db 22, CLEFABLE
	db $fe
	
	;3
	db 0
	
	db 26, CLEFABLE
	db $fe

	;4
	db 0
	
	db 30, CLEFABLE
	db $fe
	
	;5
	db 0
	
	db 36, CLEFABLE
	db $fe
	
	;6
	db 0
	
	db 40, CLEFABLE
	db $fe
	
	;7
	db 0
	
	db 44, CLEFABLE
	db $fe
	
	;8
	db 0
	
	db 50, CLEFABLE
	db $fe
	
	;9
	db 0
	
	db 56, CLEFABLE
	db $fe
	
	;10
	db 0
	
	db 60, CLEFABLE
	db $fe
	
	;11
	db 0
	
	db 66, CLEFABLE
	db $fe
	
	;12
	db 0
	
	db 70, CLEFABLE
	db $fe
	
	;13
	db 0
	
	db 74, CLEFABLE
	db $fe
	
	;14
	db 0
	
	db 79, CLEFABLE
	db $fe
	
	;15
	db 0
	
	db 84, CLEFABLE
	db $fe
	
	;16
	db 0
	
	db 92, CLEFABLE
	db $ff
	
	; HIKER (19)
	db "PARRY@", TRAINERTYPE_MOVES
	db 35, PILOSWINE,  EARTHQUAKE, BLIZZARD, REST, TAKE_DOWN
	db 35, DUGTRIO,    MAGNITUDE, DIG, MUD_SLAP, SLASH
	db 38, STEELIX,    DIG, IRON_TAIL, SANDSTORM, SLAM
	db $ff ; end

	; HIKER (20)
	db "PARRY@", TRAINERTYPE_NORMAL
	db 29, ONIX
	db $ff ; end

	; HIKER (21)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 30, GRAVELER
	db 30, GRAVELER
	db 32, MACHOKE
	db $ff ; end

	; HIKER (22)
	db "ANTHONY@", TRAINERTYPE_MOVES
	db 34, GRAVELER,   MAGNITUDE, SELFDESTRUCT, DEFENSE_CURL, ROLLOUT
	db 36, GOLEM,      MAGNITUDE, SELFDESTRUCT, DEFENSE_CURL, ROLLOUT
	db 34, MACHOKE,    KARATE_CHOP, VITAL_THROW, HEADBUTT, DIG
	db $ff ; end


BikerGroup:
	; BIKER (1)
	db "BENNY@", TRAINERTYPE_NORMAL
	db 20, KOFFING
	db 20, KOFFING
	db 20, KOFFING
	db $ff ; end

	; BIKER (2)
	db "KAZU@", TRAINERTYPE_NORMAL
	db 20, KOFFING
	db 20, KOFFING
	db 20, KOFFING
	db $ff ; end

	; BIKER (3)
	db "DWAYNE@", TRAINERTYPE_NORMAL
	db 27, KOFFING
	db 28, KOFFING
	db 29, KOFFING
	db 30, KOFFING
	db $ff ; end

	; BIKER (4)
	db "HARRIS@", TRAINERTYPE_NORMAL
	db 34, FLAREON
	db $ff ; end

	; BIKER (5)
	db "ZEKE@", TRAINERTYPE_NORMAL
	db 32, KOFFING
	db 32, KOFFING
	db $ff ; end

	; BIKER (6)
	db "CHARLES@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db 30, CHARMELEON
	db 30, WEEZING
	db $ff ; end

	; BIKER (7)
	db "RILEY@", TRAINERTYPE_NORMAL
	db 34, WEEZING
	db $ff ; end

	; BIKER (8)
	db "JOEL@", TRAINERTYPE_NORMAL
	db 32, MAGMAR
	db 32, MAGMAR
	db $ff ; end

	; BIKER (9)
	db "GLENN@", TRAINERTYPE_NORMAL
	db 28, KOFFING
	db 30, MAGMAR
	db 32, WEEZING
	db $ff ; end


BlaineGroup:
	; BLAINE (1)
	db "BLAINE@", TRAINERTYPE_MOVES
	db 45, MAGCARGO,   CURSE, SMOG, FLAMETHROWER, ROCK_SLIDE
	db 45, MAGMAR,     THUNDERPUNCH, FIRE_PUNCH, SUNNY_DAY, CONFUSE_RAY
	db 50, RAPIDASH,   QUICK_ATTACK, FIRE_SPIN, FURY_ATTACK, FIRE_BLAST
	db $ff ; end


BurglarGroup:
	; BURGLAR (1)
	db "DUNCAN@", TRAINERTYPE_NORMAL
	db 23, KOFFING
	db 25, MAGMAR
	db 23, KOFFING
	db $ff ; end

	; BURGLAR (2)
	db "EDDIE@", TRAINERTYPE_MOVES
	db 26, GROWLITHE,  ROAR, EMBER, LEER, TAKE_DOWN
	db 24, KOFFING,    TACKLE, SMOG, SLUDGE, SMOKESCREEN
	db $ff ; end

	; BURGLAR (3)
	db "COREY@", TRAINERTYPE_NORMAL
	db 25, KOFFING
	db 28, MAGMAR
	db 25, KOFFING
	db 30, KOFFING
	db $ff ; end


FirebreatherGroup:
	; FIREBREATHER (1)
	db "OTIS@", TRAINERTYPE_NORMAL
	db 29, MAGMAR
	db 32, WEEZING
	db 29, MAGMAR
	db $ff ; end

	; FIREBREATHER (2)
	db "DICK@", TRAINERTYPE_NORMAL
	db 17, CHARMELEON
	db $ff ; end

	; FIREBREATHER (3)
	db "NED@", TRAINERTYPE_NORMAL
	db 15, KOFFING
	db 16, GROWLITHE
	db 15, KOFFING
	db $ff ; end

	; FIREBREATHER (4)
	db "BURT@", TRAINERTYPE_NORMAL
	db 32, KOFFING
	db 32, SLUGMA
	db $ff ; end

	; FIREBREATHER (5)
	db "BILL@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  6, ARBOK
	db  7, PIDGEOTTO
	db $fe
	
	;1
	db 0
	
	db  16, ARBOK
	db  17, PIDGEOT
	db $fe

	;2
	db 0
	
	db 19, ARBOK
	db 20, PIDGEOT
	db 20, SEEL
	db $fe
	
	;3
	db 0
	
	db 22, PIKACHU
	db 24, PIDGEOT
	db 24, DEWGONG
	db $fe
	
	;4
	db 0
	
	db 24, PIKACHU
	db 26, PIDGEOT
	db 26, DEWGONG
	db $fe
	
	;5
	db 0
	
	db 26, PIKACHU
	db 28, PIDGEOT
	db 28, DEWGONG
	db $fe
	
	;6
	db 0
	
	db 28, PIKACHU
	db 30, PIDGEOT
	db 30, DEWGONG
	db $fe
	
	;7
	db 0
	
	db 31, PIKACHU
	db 34, RATTATA
	db 34, GRIMER
	db $fe
	
	;8
	db 0
	
	db 38, PIKACHU
	db 40, RATTATA
	db 40, GRIMER
	db $fe
	
	;9
	db 0
	
	db 42, PIKACHU
	db 45, RATTATA
	db 45, GRIMER
	db $fe
	
	;10
	db 0
	
	db 45, PIKACHU
	db 48, RATTATA
	db 48, GRIMER
	db $fe
	
	;11
	db 0
	
	db 49, PIKACHU
	db 54, RATTATA
	db 54, GRIMER
	db $fe
	
	;12
	db 0
	
	db 55, PIKACHU
	db 60, RATTATA
	db 60, GRIMER
	db $fe
	
	;13
	db 0
	
	db 60, PIKACHU
	db 65, RATTATA
	db 65, GRIMER
	db $fe
	
	;14
	db 0
	
	db 65, PIKACHU
	db 70, RATTATA
	db 70, GRIMER
	db $fe
	
	;15
	db 0
	
	db 70, PIKACHU
	db 70, PIKACHU
	db 80, RATTATA
	db 80, GRIMER
	db $fe
	
	;16
	db 0
	
	db 80, PIKACHU
	db 80, PIKACHU
	db 85, RATTATA
	db 85, RATTATA
	db 90, GRIMER
	db 90, GRIMER
	db $ff
	
	; FIREBREATHER (6)
	db "WALT@", TRAINERTYPE_NORMAL
	db 11, MAGMAR
	db 13, MAGMAR
	db $ff ; end

	; FIREBREATHER (7)
	db "RAY@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  10, ZUBAT
	db $fe
	
	;1
	db 0
	
	db  18, ZUBAT
	db $fe
	
	;2
	db 0
	
	db  24, ZUBAT
	db $fe
	
	;3
    db 0
	
	db  27, ZUBAT
	db $fe
	
	;4
	db 0
	
	db  32, GOLBAT
	db $fe
	
	;5
	db 0
	
	db  32, ZUBAT
	db  37, GOLBAT
	db $fe
	
	;6
	db 0
	
	db  37, ZUBAT
	db  40, GOLBAT
	db $fe
	
	;7
	db 0
	
	db  40, ZUBAT
	db  42, GOLBAT
	db $fe
	
	;8
	db 0
	
	db  45, GOLBAT
	db  47, GOLBAT
	db $fe
	
	;9
	db 0
	
	db  50, GOLBAT
	db  52, GOLBAT
	db  54, GOLBAT
	db $fe
	
	;10
	db 0
	
	db  56, GOLBAT
	db  59, GOLBAT
	db  60, GOLBAT
	db $fe
	
	;11
	db 0
	
	db  60, GOLBAT
	db  63, GOLBAT
	db  68, GOLBAT
	db $fe
	
	;12
	db 0
	
	db  64, GOLBAT
	db  70, GOLBAT
	db  72, GOLBAT
	db $fe
	
	;13
	db 0
	
	db  74, GOLBAT
	db  79, GOLBAT
	db  80, GOLBAT
	db $fe
	
	;14
	db 0
	
	db  80, GOLBAT
	db  84, GOLBAT
	db  84, GOLBAT
	db  85, GOLBAT
	db $fe
	
	;15
	db 0
	
	db  85, GOLBAT
	db  89, GOLBAT
	db  89, GOLBAT
	db  90, GOLBAT
	db $fe
	
	;16
	db 0
	
	db  90, GOLBAT
	db  90, GOLBAT
	db  92, GOLBAT
	db  95, GOLBAT
	db $ff
	
	; FIREBREATHER (8)
	db "LYLE@", TRAINERTYPE_NORMAL
	db 28, KOFFING
	db 31, FLAREON
	db 28, KOFFING
	db $ff ; end


JugglerGroup:
	; JUGGLER (1)
	db "IRWIN@", TRAINERTYPE_NORMAL
	db  2, VOLTORB
	db  6, VOLTORB
	db 10, VOLTORB
	db 14, VOLTORB
	db $ff ; end

	; JUGGLER (2)
	db "FRITZ@", TRAINERTYPE_NORMAL
	db 29, MR__MIME
	db 29, MAGMAR
	db 29, MACHOKE
	db $ff ; end

	; JUGGLER (3)
	db "HORTON@", TRAINERTYPE_NORMAL
	db 33, ELECTRODE
	db 33, ELECTRODE
	db 33, ELECTRODE
	db 33, ELECTRODE
	db $ff ; end

	; JUGGLER (4)
	db "IRWIN@", TRAINERTYPE_NORMAL
	db  6, VOLTORB
	db 10, VOLTORB
	db 14, VOLTORB
	db 18, VOLTORB
	db $ff ; end

	; JUGGLER (5)
	db "IRWIN@", TRAINERTYPE_NORMAL
	db 18, VOLTORB
	db 22, VOLTORB
	db 26, VOLTORB
	db 30, ELECTRODE
	db $ff ; end

	; JUGGLER (6)
	db "IRWIN@", TRAINERTYPE_NORMAL
	db 18, VOLTORB
	db 22, VOLTORB
	db 26, VOLTORB
	db 30, ELECTRODE
	db $ff ; end


BlackbeltGroup:
	; BLACKBELT_T (1)
	db "KENJI@", TRAINERTYPE_NORMAL
	db 27, ONIX
	db 30, HITMONLEE
	db 27, ONIX
	db 32, MACHOKE
	db $ff ; end

	; BLACKBELT_T (2)
	db "YOSHI@", TRAINERTYPE_MOVES
	db 27, HITMONLEE,  DOUBLE_KICK, MEDITATE, JUMP_KICK, FOCUS_ENERGY
	db $ff ; end

	; BLACKBELT_T (3)
	db "KENJI@", TRAINERTYPE_MOVES
	db 33, ONIX,       BIND, ROCK_THROW, TOXIC, DIG
	db 38, MACHAMP,    HEADBUTT, SWAGGER, THUNDERPUNCH, VITAL_THROW
	db 33, STEELIX,    EARTHQUAKE, ROCK_THROW, IRON_TAIL, SANDSTORM
	db 36, HITMONLEE,  DOUBLE_TEAM, HI_JUMP_KICK, MUD_SLAP, SWIFT
	db $ff ; end

	; BLACKBELT_T (4)
	db "LAO@", TRAINERTYPE_MOVES
	db 27, HITMONCHAN, COMET_PUNCH, THUNDERPUNCH, ICE_PUNCH, FIRE_PUNCH
	db $ff ; end

	; BLACKBELT_T (5)
	db "NOB@", TRAINERTYPE_MOVES
	db 25, MACHOP,     LEER, FOCUS_ENERGY, KARATE_CHOP, SEISMIC_TOSS
	db 25, MACHOKE,    LEER, KARATE_CHOP, SEISMIC_TOSS, ROCK_SLIDE
	db $ff ; end

	; BLACKBELT_T (6)
	db "KIYO@", TRAINERTYPE_NORMAL
	db 34, HITMONLEE
	db 34, HITMONCHAN
	db $ff ; end

	; BLACKBELT_T (7)
	db "LUNG@", TRAINERTYPE_NORMAL
	db 23, MANKEY
	db 23, MANKEY
	db 25, PRIMEAPE
	db $ff ; end

	; BLACKBELT_T (8)
	db "KENJI@", TRAINERTYPE_NORMAL
	db 28, MACHOKE
	db $ff ; end

	; BLACKBELT_T (9)
	db "WAI@", TRAINERTYPE_NORMAL
	db 30, MACHOKE
	db 32, MACHOKE
	db 34, MACHOKE
	db $ff ; end


ExecutiveMGroup:
    ; GRUNTM (1)
	db "ADAM@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 16, MEGANIUM
	db 20, FERALIGATR
	db $fe
	
	;1
	db 0
	
	db 23, MEGANIUM
	db 25, FERALIGATR
	db $fe
	
	;2
	db 0
	
	db 29, MEGANIUM
	db 32, FERALIGATR
	db $fe
	
	;3
	db 0
	
	db 32, MEGANIUM
	db 35, FERALIGATR
	db $fe
	
	;4
	db 0
	
	db 34, MEGANIUM
	db 37, FERALIGATR
	db $fe
	
	;5
	db 0
	
	db 36, MEGANIUM
	db 40, FERALIGATR
	db $fe
	
	;6
	db 0
	
	db 39, MEGANIUM
	db 42, FERALIGATR
	db $fe
	
	;7
	db 0
	
	db 44, MEGANIUM
	db 47, FERALIGATR
	db $fe
	
	;8
	db 0
	
	db 50, MEGANIUM
	db 52, FERALIGATR
	db $fe
	
	;9
	db 0
	
	db 55, MEGANIUM
	db 58, FERALIGATR
	db $fe
	
	;10
	db 0
	
	db 60, MEGANIUM
	db 63, FERALIGATR
	db $fe
	
	;11
	db 0
	
	db 65, MEGANIUM
	db 68, FERALIGATR
	db $fe
	
	;12
	db 0
	
	db 70, MEGANIUM
	db 73, FERALIGATR
	db $fe
	
	;13
	db 0
	
	db 74, MEGANIUM
	db 77, FERALIGATR
	db $fe
	
	;14
	db 0
	
	db 80, MEGANIUM
	db 82, FERALIGATR
	db $fe
	
	;15
	db 0
	
	db 90, MEGANIUM
	db 93, FERALIGATR
	db $fe
	
	;16
	db 0
	
	db 100, MEGANIUM
	db 100, FERALIGATR
	db $ff
	
	; EXECUTIVEM (1)
	db "EXECUTIVE@", TRAINERTYPE_MOVES
	db 33, HOUNDOUR,   EMBER, ROAR, BITE, FAINT_ATTACK
	db 33, KOFFING,    TACKLE, SLUDGE, SMOKESCREEN, HAZE
	db 35, HOUNDOOM,   EMBER, SMOG, BITE, FAINT_ATTACK
	db $ff ; end

	; EXECUTIVEM (2)
	db "EXECUTIVE@", TRAINERTYPE_MOVES
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db $ff ; end

	; EXECUTIVEM (3)
	db "EXECUTIVE@", TRAINERTYPE_MOVES
	db 30, KOFFING,    TACKLE, SELFDESTRUCT, SLUDGE, SMOKESCREEN
	db 30, KOFFING,    TACKLE, SELFDESTRUCT, SLUDGE, SMOKESCREEN
	db 30, KOFFING,    TACKLE, SELFDESTRUCT, SLUDGE, SMOKESCREEN
	db 32, WEEZING,    TACKLE, EXPLOSION, SLUDGE, SMOKESCREEN
	db 30, KOFFING,    TACKLE, SELFDESTRUCT, SLUDGE, SMOKESCREEN
	db 30, KOFFING,    TACKLE, SMOG, SLUDGE, SMOKESCREEN
	db $ff ; end

	; EXECUTIVEM (4)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 22, ZUBAT
	db 24, RATICATE
	db 22, KOFFING
	db $ff ; end


PsychicGroup:
	; PSYCHIC_T (1)
	db "NATHAN@", TRAINERTYPE_NORMAL
	db 26, GIRAFARIG
	db $ff ; end

	; PSYCHIC_T (2)
	db "FRANKLIN@", TRAINERTYPE_NORMAL
	db 37, KADABRA
	db $ff ; end

	; PSYCHIC_T (3)
	db "HERMAN@", TRAINERTYPE_NORMAL
	db 30, EXEGGCUTE
	db 30, EXEGGCUTE
	db 30, EXEGGUTOR
	db $ff ; end

	; PSYCHIC_T (4)
	db "FIDEL@", TRAINERTYPE_NORMAL
	db 34, XATU
	db $ff ; end

	; PSYCHIC_T (5)
	db "GREG@", TRAINERTYPE_MOVES
	db 17, DROWZEE,    HYPNOSIS, DISABLE, DREAM_EATER, NO_MOVE
	db $ff ; end

	; PSYCHIC_T (6)
	db "NORMAN@", TRAINERTYPE_MOVES
	db 17, SLOWPOKE,   TACKLE, GROWL, WATER_GUN, NO_MOVE
	db 20, SLOWPOKE,   CURSE, BODY_SLAM, WATER_GUN, CONFUSION
	db $ff ; end

	; PSYCHIC_T (7)
	db "MARK@", TRAINERTYPE_MOVES
	db 13, ABRA,       TELEPORT, FLASH, NO_MOVE, NO_MOVE
	db 13, ABRA,       TELEPORT, FLASH, NO_MOVE, NO_MOVE
	db 15, KADABRA,    TELEPORT, KINESIS, CONFUSION, NO_MOVE
	db $ff ; end

	; PSYCHIC_T (8)
	db "PHIL@", TRAINERTYPE_MOVES
	db 24, NATU,       LEER, NIGHT_SHADE, FUTURE_SIGHT, CONFUSE_RAY
	db 26, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db $ff ; end

	; PSYCHIC_T (9)
	db "RICHARD@", TRAINERTYPE_NORMAL
	db 36, ESPEON
	db $ff ; end

	; PSYCHIC_T (10)
	db "GILBERT@", TRAINERTYPE_NORMAL
	db 30, STARMIE
	db 30, EXEGGCUTE
	db 34, GIRAFARIG
	db $ff ; end

	; PSYCHIC_T (11)
	db "JARED@", TRAINERTYPE_NORMAL
	db 32, MR__MIME
	db 32, EXEGGCUTE
	db 35, EXEGGCUTE
	db $ff ; end

	; PSYCHIC_T (12)
	db "RODNEY@", TRAINERTYPE_NORMAL
	db 29, DROWZEE
	db 33, HYPNO
	db $ff ; end


PicnickerGroup:
	; PICNICKER (1)
	db "LIZ@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, TANGELA
	db  9, GEODUDE
	db $fe
	
	;1
	db 0
	
	db 15, TANGELA
	db 15, GEODUDE
	db $fe
	
	;2
	db 0
	
	db 20, TANGELA
	db 20, GEODUDE
	db $fe
	
	;3
	db 0
	
	db 25, KANGASKHAN
	db 25, GRAVELER
	db $fe
	
	;4
	db 0
	
	db 29, KANGASKHAN
	db 29, GRAVELER
	db $fe
	
	;5
	db 0
	
	db 31, KANGASKHAN
	db 31, GRAVELER
	db $fe
	
	;6
	db 0
	
	db 33, KANGASKHAN
	db 33, GRAVELER
	db $fe
	
	;7
	db 0
	
	db 37, KANGASKHAN
	db 37, GRAVELER
	db $fe
	
	;8
	db 0
	
	db 42, KANGASKHAN
	db 42, GRAVELER
	db $fe
	
	;9
	db 0
	
	db 47, KANGASKHAN
	db 47, GRAVELER
	db $fe
	
	;10
	db 0
	
	db 51, KANGASKHAN
	db 51, GRAVELER
	db $fe
	
	;11
	db 0
	
	db 56, KANGASKHAN
	db 56, GRAVELER
	db $fe
	
	;12
	db 0
	
	db 61, KANGASKHAN
	db 61, GRAVELER
	db $fe
	
	;13
	db 0
	
	db 63, KANGASKHAN
	db 63, GRAVELER
	db $fe
	
	;14
	db 0
	
	db 72, KANGASKHAN
	db 72, GRAVELER
	db $fe
	
	;15
	db 0
	
	db 78, KANGASKHAN
	db 78, GRAVELER
	db $fe
	
	;16
	db 0
	
	db 92, KANGASKHAN
	db 92, GRAVELER
	db $ff
	
	

	; PICNICKER (2)
	db "GINA@", TRAINERTYPE_NORMAL
	db  9, HOPPIP
	db  9, HOPPIP
	db 12, BULBASAUR
	db $ff ; end

	; PICNICKER (3)
	db "BROOKE@", TRAINERTYPE_MOVES
	db 16, PIKACHU,    THUNDERSHOCK, GROWL, QUICK_ATTACK, DOUBLE_TEAM
	db $ff ; end

	; PICNICKER (4)
	db "KIM@", TRAINERTYPE_NORMAL
	db 15, VULPIX
	db $ff ; end

	; PICNICKER (5)
	db "CINDY@", TRAINERTYPE_NORMAL
	db 36, NIDOQUEEN
	db $ff ; end

	; PICNICKER (6)
	db "HOPE@", TRAINERTYPE_NORMAL
	db 34, FLAAFFY
	db $ff ; end

	; PICNICKER (7)
	db "SHARON@", TRAINERTYPE_NORMAL
	db 31, FURRET
	db 33, RAPIDASH
	db $ff ; end

	; PICNICKER (8)
	db "DEBRA@", TRAINERTYPE_NORMAL
	db 33, SEAKING
	db $ff ; end

	; PICNICKER (9)
	db "GINA@", TRAINERTYPE_NORMAL
	db 14, HOPPIP
	db 14, HOPPIP
	db 17, IVYSAUR
	db $ff ; end

	; PICNICKER (10)
	db "ERIN@", TRAINERTYPE_NORMAL
	db 16, PONYTA
	db 16, PONYTA
	db $ff ; end

	; PICNICKER (11)
	db "LIZ@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, TANGELA
	db  9, GEODUDE
	db $fe
	
	;1
	db 0
	
	db 15, TANGELA
	db 15, GEODUDE
	db $fe
	
	;2
	db 0
	
	db 20, TANGELA
	db 20, GEODUDE
	db $fe
	
	;3
	db 0
	
	db 25, KANGASKHAN
	db 25, GRAVELER
	db $fe
	
	;4
	db 0
	
	db 29, KANGASKHAN
	db 29, GRAVELER
	db $fe
	
	;5
	db 0
	
	db 31, KANGASKHAN
	db 31, GRAVELER
	db $fe
	
	;6
	db 0
	
	db 33, KANGASKHAN
	db 33, GRAVELER
	db $fe
	
	;7
	db 0
	
	db 37, KANGASKHAN
	db 37, GRAVELER
	db $fe
	
	;8
	db 0
	
	db 42, KANGASKHAN
	db 42, GRAVELER
	db $fe
	
	;9
	db 0
	
	db 47, KANGASKHAN
	db 47, GRAVELER
	db $fe
	
	;10
	db 0
	
	db 51, KANGASKHAN
	db 51, GRAVELER
	db $fe
	
	;11
	db 0
	
	db 56, KANGASKHAN
	db 56, GRAVELER
	db $fe
	
	;12
	db 0
	
	db 61, KANGASKHAN
	db 61, GRAVELER
	db $fe
	
	;13
	db 0
	
	db 63, KANGASKHAN
	db 63, GRAVELER
	db $fe
	
	;14
	db 0
	
	db 72, KANGASKHAN
	db 72, GRAVELER
	db $fe
	
	;15
	db 0
	
	db 78, KANGASKHAN
	db 78, GRAVELER
	db $fe
	
	;16
	db 0
	
	db 92, KANGASKHAN
	db 92, GRAVELER
	db $ff

	; PICNICKER (12)
	db "LIZ@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, TANGELA
	db  9, GEODUDE
	db $fe
	
	;1
	db 0
	
	db 15, TANGELA
	db 15, GEODUDE
	db $fe
	
	;2
	db 0
	
	db 20, TANGELA
	db 20, GEODUDE
	db $fe
	
	;3
	db 0
	
	db 25, KANGASKHAN
	db 25, GRAVELER
	db $fe
	
	;4
	db 0
	
	db 29, KANGASKHAN
	db 29, GRAVELER
	db $fe
	
	;5
	db 0
	
	db 31, KANGASKHAN
	db 31, GRAVELER
	db $fe
	
	;6
	db 0
	
	db 33, KANGASKHAN
	db 33, GRAVELER
	db $fe
	
	;7
	db 0
	
	db 37, KANGASKHAN
	db 37, GRAVELER
	db $fe
	
	;8
	db 0
	
	db 42, KANGASKHAN
	db 42, GRAVELER
	db $fe
	
	;9
	db 0
	
	db 47, KANGASKHAN
	db 47, GRAVELER
	db $fe
	
	;10
	db 0
	
	db 51, KANGASKHAN
	db 51, GRAVELER
	db $fe
	
	;11
	db 0
	
	db 56, KANGASKHAN
	db 56, GRAVELER
	db $fe
	
	;12
	db 0
	
	db 61, KANGASKHAN
	db 61, GRAVELER
	db $fe
	
	;13
	db 0
	
	db 63, KANGASKHAN
	db 63, GRAVELER
	db $fe
	
	;14
	db 0
	
	db 72, KANGASKHAN
	db 72, GRAVELER
	db $fe
	
	;15
	db 0
	
	db 78, KANGASKHAN
	db 78, GRAVELER
	db $fe
	
	;16
	db 0
	
	db 92, KANGASKHAN
	db 92, GRAVELER
	db $ff

	; PICNICKER (13)
	db "HEIDI@", TRAINERTYPE_NORMAL
	db 32, SKIPLOOM
	db 32, SKIPLOOM
	db $ff ; end

	; PICNICKER (14)
	db "EDNA@", TRAINERTYPE_NORMAL
	db 30, NIDORINA
	db 34, RAICHU
	db $ff ; end

	; PICNICKER (15)
	db "GINA@", TRAINERTYPE_NORMAL
	db 26, SKIPLOOM
	db 26, SKIPLOOM
	db 29, IVYSAUR
	db $ff ; end

	; PICNICKER (16)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 31, CLEFAIRY,   ENCORE, SING, DOUBLESLAP, MINIMIZE
	db $ff ; end

	; PICNICKER (17)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 37, CLEFAIRY,   ENCORE, DOUBLESLAP, MINIMIZE, METRONOME
	db $ff ; end

	; PICNICKER (18)
	db "ERIN@", TRAINERTYPE_NORMAL
	db 32, PONYTA
	db 32, PONYTA
	db $ff ; end

	; PICNICKER (19)
	db "TANYA@", TRAINERTYPE_NORMAL
	db 37, EXEGGUTOR
	db $ff ; end

	; PICNICKER (20)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 20, CLEFAIRY,   ENCORE, SING, DOUBLESLAP, MINIMIZE
	db $ff ; end

	; PICNICKER (21)
	db "ERIN@", TRAINERTYPE_MOVES
	db 36, PONYTA,     DOUBLE_TEAM, STOMP, FIRE_SPIN, SUNNY_DAY
	db 34, RAICHU,     SWIFT, MUD_SLAP, QUICK_ATTACK, THUNDERBOLT
	db 36, PONYTA,     DOUBLE_TEAM, STOMP, FIRE_SPIN, SUNNY_DAY
	db $ff ; end

	; PICNICKER (22)
	db "LIZ@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, TANGELA
	db  9, GEODUDE
	db $fe
	
	;1
	db 0
	
	db 15, TANGELA
	db 15, GEODUDE
	db $fe
	
	;2
	db 0
	
	db 20, TANGELA
	db 20, GEODUDE
	db $fe
	
	;3
	db 0
	
	db 25, KANGASKHAN
	db 25, GRAVELER
	db $fe
	
	;4
	db 0
	
	db 29, KANGASKHAN
	db 29, GRAVELER
	db $fe
	
	;5
	db 0
	
	db 31, KANGASKHAN
	db 31, GRAVELER
	db $fe
	
	;6
	db 0
	
	db 33, KANGASKHAN
	db 33, GRAVELER
	db $fe
	
	;7
	db 0
	
	db 37, KANGASKHAN
	db 37, GRAVELER
	db $fe
	
	;8
	db 0
	
	db 42, KANGASKHAN
	db 42, GRAVELER
	db $fe
	
	;9
	db 0
	
	db 47, KANGASKHAN
	db 47, GRAVELER
	db $fe
	
	;10
	db 0
	
	db 51, KANGASKHAN
	db 51, GRAVELER
	db $fe
	
	;11
	db 0
	
	db 56, KANGASKHAN
	db 56, GRAVELER
	db $fe
	
	;12
	db 0
	
	db 61, KANGASKHAN
	db 61, GRAVELER
	db $fe
	
	;13
	db 0
	
	db 63, KANGASKHAN
	db 63, GRAVELER
	db $fe
	
	;14
	db 0
	
	db 72, KANGASKHAN
	db 72, GRAVELER
	db $fe
	
	;15
	db 0
	
	db 78, KANGASKHAN
	db 78, GRAVELER
	db $fe
	
	;16
	db 0
	
	db 92, KANGASKHAN
	db 92, GRAVELER
	db $ff

	; PICNICKER (23)
	db "LIZ@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db  9, TANGELA
	db  9, GEODUDE
	db $fe
	
	;1
	db 0
	
	db 15, TANGELA
	db 15, GEODUDE
	db $fe
	
	;2
	db 0
	
	db 20, TANGELA
	db 20, GEODUDE
	db $fe
	
	;3
	db 0
	
	db 25, KANGASKHAN
	db 25, GRAVELER
	db $fe
	
	;4
	db 0
	
	db 29, KANGASKHAN
	db 29, GRAVELER
	db $fe
	
	;5
	db 0
	
	db 31, KANGASKHAN
	db 31, GRAVELER
	db $fe
	
	;6
	db 0
	
	db 33, KANGASKHAN
	db 33, GRAVELER
	db $fe
	
	;7
	db 0
	
	db 37, KANGASKHAN
	db 37, GRAVELER
	db $fe
	
	;8
	db 0
	
	db 42, KANGASKHAN
	db 42, GRAVELER
	db $fe
	
	;9
	db 0
	
	db 47, KANGASKHAN
	db 47, GRAVELER
	db $fe
	
	;10
	db 0
	
	db 51, KANGASKHAN
	db 51, GRAVELER
	db $fe
	
	;11
	db 0
	
	db 56, KANGASKHAN
	db 56, GRAVELER
	db $fe
	
	;12
	db 0
	
	db 61, KANGASKHAN
	db 61, GRAVELER
	db $fe
	
	;13
	db 0
	
	db 63, KANGASKHAN
	db 63, GRAVELER
	db $fe
	
	;14
	db 0
	
	db 72, KANGASKHAN
	db 72, GRAVELER
	db $fe
	
	;15
	db 0
	
	db 78, KANGASKHAN
	db 78, GRAVELER
	db $fe
	
	;16
	db 0
	
	db 92, KANGASKHAN
	db 92, GRAVELER
	db $ff

	; PICNICKER (24)
	db "GINA@", TRAINERTYPE_NORMAL
	db 30, SKIPLOOM
	db 30, SKIPLOOM
	db 32, IVYSAUR
	db $ff ; end

	; PICNICKER (25)
	db "GINA@", TRAINERTYPE_MOVES
	db 33, JUMPLUFF,   STUN_SPORE, SUNNY_DAY, LEECH_SEED, COTTON_SPORE
	db 33, JUMPLUFF,   SUNNY_DAY, SLEEP_POWDER, LEECH_SEED, COTTON_SPORE
	db 38, VENUSAUR,   SOLARBEAM, RAZOR_LEAF, HEADBUTT, MUD_SLAP
	db $ff ; end

	; PICNICKER (26)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 43, CLEFAIRY,   METRONOME, ENCORE, MOONLIGHT, MINIMIZE
	db $ff ; end


CamperGroup:
	; CAMPER (1)
	db "ROLAND@", TRAINERTYPE_VARIABLE
	
	;0
	db 0
	
	db 7, MAROWAK
	db 7, ELECTRODE
	db 8, HITMONCHAN
	db $fe
	
	;1
	db 0
	
	db 12, MAROWAK
	db 12, ELECTRODE
	db 13, HITMONCHAN
	db $fe
	
	;2
	db 0
	
	db 18, MAROWAK
	db 18, ELECTRODE
	db 19, HITMONCHAN
	db $fe
	
	;3
	db 0
	
	db 23, MAROWAK
	db 23, ELECTRODE
	db 24, LICKITUNG
	db $fe
	
	;4
	db 0
	
	db 27, HITMONLEE
	db 27, EXEGGCUTE
	db 28, LICKITUNG
	db $fe
	
	;5
	db 0
	
	db 29, HITMONLEE
	db 29, EXEGGCUTE
	db 30, LICKITUNG
	db $fe
	
	;6
	db 0
	
	db 30, HITMONLEE
	db 30, EXEGGCUTE
	db 31, KOFFING
	db $fe
	
	;7
	db 0
	
	db 35, HITMONLEE
	db 35, EXEGGCUTE
	db 36, KOFFING
	db $fe
	
	;8
	db 0
	
	db 39, HITMONLEE
	db 39, EXEGGCUTE
	db 40, KOFFING
	db $fe
	
	;9
	db 0
	
	db 44, HITMONLEE
	db 44, EXEGGCUTE
	db 45, KOFFING
	db $fe
	
	;10
	db 0
	
	db 49, HITMONLEE
	db 49, EXEGGCUTE
	db 50, KOFFING
	db $fe

	;11
	db 0
	
	db 53, HITMONLEE
	db 53, EXEGGCUTE
	db 54, KOFFING
	db $fe
	
	;12
	db 0
	
	db 57, HITMONLEE
	db 57, EXEGGCUTE
	db 58, KOFFING
	db $fe
	
	;13
	db 0
	
	db 62, HITMONLEE
	db 62, EXEGGCUTE
	db 63, KOFFING
	db $fe
	
	;14
	db 0
	
	db 65, HITMONLEE
	db 65, EXEGGCUTE
	db 66, KOFFING
	db $fe
	
	;15
	db 0
	
	db 73, HITMONLEE
	db 73, EXEGGCUTE
	db 74, KOFFING
	db $fe
	
	;16
	db 0
	
	db 80, HITMONLEE
	db 80, EXEGGCUTE
	db 81, KOFFING
	db $ff
	
	; CAMPER (2)
	db "TODD@", TRAINERTYPE_NORMAL
	db 14, PSYDUCK
	db $ff ; end

	; CAMPER (3)
	db "IVAN@", TRAINERTYPE_NORMAL
	db 10, DIGLETT
	db 10, ZUBAT
	db 14, DIGLETT
	db $ff ; end

	; CAMPER (4)
	db "ELLIOT@", TRAINERTYPE_NORMAL
	db 13, SANDSHREW
	db 15, MARILL
	db $ff ; end

	; CAMPER (5)
	db "BARRY@", TRAINERTYPE_NORMAL
	db 36, NIDOKING
	db $ff ; end

	; CAMPER (6)
	db "LLOYD@", TRAINERTYPE_NORMAL
	db 34, NIDOKING
	db $ff ; end

	; CAMPER (7)
	db "DEAN@", TRAINERTYPE_NORMAL
	db 33, GOLDUCK
	db 31, SANDSLASH
	db $ff ; end

	; CAMPER (8)
	db "SID@", TRAINERTYPE_NORMAL
	db 32, DUGTRIO
	db 29, PRIMEAPE
	db 29, POLIWRATH
	db $ff ; end

	; CAMPER (9)
	db "HARVEY@", TRAINERTYPE_NORMAL
	db 15, NIDORINO
	db $ff ; end

	; CAMPER (10)
	db "DALE@", TRAINERTYPE_NORMAL
	db 15, NIDORINO
	db $ff ; end

	; CAMPER (11)
	db "TED@", TRAINERTYPE_NORMAL
	db 17, MANKEY
	db $ff ; end

	; CAMPER (12)
	db "TODD@", TRAINERTYPE_NORMAL
	db 17, GEODUDE
	db 17, GEODUDE
	db 23, PSYDUCK
	db $ff ; end

	; CAMPER (13)
	db "TODD@", TRAINERTYPE_NORMAL
	db 23, GEODUDE
	db 23, GEODUDE
	db 26, PSYDUCK
	db $ff ; end

	; CAMPER (14)
	db "THOMAS@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db $ff ; end

	; CAMPER (15)
	db "LEROY@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db $ff ; end

	; CAMPER (16)
	db "DAVID@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db $ff ; end

	; CAMPER (17)
	db "JOHN@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db $ff ; end

	; CAMPER (18)
	db "JERRY@", TRAINERTYPE_NORMAL
	db 37, SANDSLASH
	db $ff ; end

	; CAMPER (19)
	db "SPENCER@", TRAINERTYPE_NORMAL
	db 17, SANDSHREW
	db 17, SANDSLASH
	db 19, ZUBAT
	db $ff ; end

	; CAMPER (20)
	db "TODD@", TRAINERTYPE_NORMAL
	db 30, GRAVELER
	db 30, GRAVELER
	db 30, SLUGMA
	db 32, PSYDUCK
	db $ff ; end

	; CAMPER (21)
	db "TODD@", TRAINERTYPE_MOVES
	db 33, GRAVELER,   SELFDESTRUCT, ROCK_THROW, HARDEN, MAGNITUDE
	db 33, GRAVELER,   SELFDESTRUCT, ROCK_THROW, HARDEN, MAGNITUDE
	db 36, MAGCARGO,   ROCK_THROW, HARDEN, AMNESIA, FLAMETHROWER
	db 34, GOLDUCK,    DISABLE, PSYCHIC_M, SURF, PSYCH_UP
	db $ff ; end

	; CAMPER (22)
	db "QUENTIN@", TRAINERTYPE_NORMAL
	db 30, FEAROW
	db 30, PRIMEAPE
	db 30, TAUROS
	db $ff ; end


ExecutiveFGroup:
	; EXECUTIVEF (1)
	db "EXECUTIVE@", TRAINERTYPE_MOVES
	db 32, ARBOK,      WRAP, POISON_STING, BITE, GLARE
	db 32, VILEPLUME,  ABSORB, SWEET_SCENT, SLEEP_POWDER, ACID
	db 32, MURKROW,    PECK, PURSUIT, HAZE, NIGHT_SHADE
	db $ff ; end

	; EXECUTIVEF (2)
	db "EXECUTIVE@", TRAINERTYPE_MOVES
	db 23, ARBOK,      WRAP, LEER, POISON_STING, BITE
	db 23, GLOOM,      ABSORB, SWEET_SCENT, SLEEP_POWDER, ACID
	db 25, MURKROW,    PECK, PURSUIT, HAZE, NO_MOVE
	db $ff ; end


SageGroup:
	; SAGE (1)
	db "CHOW@", TRAINERTYPE_NORMAL
	db  3, BELLSPROUT
	db  3, BELLSPROUT
	db  3, BELLSPROUT
	db $ff ; end

	; SAGE (2)
	db "NICO@", TRAINERTYPE_NORMAL
	db  3, BELLSPROUT
	db  3, BELLSPROUT
	db  3, BELLSPROUT
	db $ff ; end

	; SAGE (3)
	db "JIN@", TRAINERTYPE_NORMAL
	db  6, BELLSPROUT
	db $ff ; end

	; SAGE (4)
	db "TROY@", TRAINERTYPE_NORMAL
	db  7, BELLSPROUT
	db  7, HOOTHOOT
	db $ff ; end

	; SAGE (5)
	db "JEFFREY@", TRAINERTYPE_NORMAL
	db 22, HAUNTER
	db $ff ; end

	; SAGE (6)
	db "PING@", TRAINERTYPE_NORMAL
	db 16, GASTLY
	db 16, GASTLY
	db 16, GASTLY
	db 16, GASTLY
	db 16, GASTLY
	db $ff ; end

	; SAGE (7)
	db "EDMOND@", TRAINERTYPE_NORMAL
	db  3, BELLSPROUT
	db  3, BELLSPROUT
	db  3, BELLSPROUT
	db $ff ; end

	; SAGE (8)
	db "NEAL@", TRAINERTYPE_NORMAL
	db  6, BELLSPROUT
	db $ff ; end

	; SAGE (9)
	db "LI@", TRAINERTYPE_NORMAL
	db  7, BELLSPROUT
	db  7, BELLSPROUT
	db 10, HOOTHOOT
	db $ff ; end

	; SAGE (10)
	db "GOKU@", TRAINERTYPE_NORMAL
	db 32, NOCTOWL
	db 32, FLAREON
	db $ff ; end

	; SAGE (11)
	db "MASA@", TRAINERTYPE_NORMAL
	db 32, NOCTOWL
	db 32, JOLTEON
	db $ff ; end

	; SAGE (12)
	db "KOJI@", TRAINERTYPE_NORMAL
	db 32, NOCTOWL
	db 32, VAPOREON
	db $ff ; end


MediumGroup:
	; MEDIUM (1)
	db "MARTHA@", TRAINERTYPE_NORMAL
	db 18, GASTLY
	db 20, HAUNTER
	db 20, GASTLY
	db $ff ; end

	; MEDIUM (2)
	db "GRACE@", TRAINERTYPE_NORMAL
	db 20, HAUNTER
	db 20, HAUNTER
	db $ff ; end

	; MEDIUM (3)
	db "BETHANY@", TRAINERTYPE_NORMAL
	db 25, HAUNTER
	db $ff ; end

	; MEDIUM (4)
	db "MARGRET@", TRAINERTYPE_NORMAL
	db 25, HAUNTER
	db $ff ; end

	; MEDIUM (5)
	db "ETHEL@", TRAINERTYPE_NORMAL
	db 25, HAUNTER
	db $ff ; end

	; MEDIUM (6)
	db "REBECCA@", TRAINERTYPE_NORMAL
	db 35, DROWZEE
	db 35, HYPNO
	db $ff ; end

	; MEDIUM (7)
	db "DORIS@", TRAINERTYPE_NORMAL
	db 34, SLOWPOKE
	db 36, SLOWBRO
	db $ff ; end


BoarderGroup:
	; BOARDER (1)
	db "RONALD@", TRAINERTYPE_NORMAL
	db 24, SEEL
	db 25, DEWGONG
	db 24, SEEL
	db $ff ; end

	; BOARDER (2)
	db "BRAD@", TRAINERTYPE_NORMAL
	db 26, SWINUB
	db 26, SWINUB
	db $ff ; end

	; BOARDER (3)
	db "DOUGLAS@", TRAINERTYPE_NORMAL
	db 24, SHELLDER
	db 25, CLOYSTER
	db 24, SHELLDER
	db $ff ; end


PokefanMGroup:
	; POKEFANM (1)
	db "WILLIAM@", TRAINERTYPE_ITEM
	db 14, RAICHU,     BERRY
	db $ff ; end

	; POKEFANM (2)
	db "DEREK@", TRAINERTYPE_ITEM
	db 17, PIKACHU,    BERRY
	db $ff ; end

	; POKEFANM (3)
	db "ROBERT@", TRAINERTYPE_ITEM
	db 33, QUAGSIRE,   BERRY
	db $ff ; end

	; POKEFANM (4)
	db "JOSHUA@", TRAINERTYPE_ITEM
	db 23, PIKACHU,    BERRY
	db 23, PIKACHU,    BERRY
	db 23, PIKACHU,    BERRY
	db 23, PIKACHU,    BERRY
	db 23, PIKACHU,    BERRY
	db 23, PIKACHU,    BERRY
	db $ff ; end

	; POKEFANM (5)
	db "CARTER@", TRAINERTYPE_ITEM
	db 29, BULBASAUR,  BERRY
	db 29, CHARMANDER, BERRY
	db 29, SQUIRTLE,   BERRY
	db $ff ; end

	; POKEFANM (6)
	db "TREVOR@", TRAINERTYPE_ITEM
	db 33, PSYDUCK,    BERRY
	db $ff ; end

	; POKEFANM (7)
	db "BRANDON@", TRAINERTYPE_ITEM
	db 13, SNUBBULL,   BERRY
	db $ff ; end

	; POKEFANM (8)
	db "JEREMY@", TRAINERTYPE_ITEM
	db 28, MEOWTH,     BERRY
	db 28, MEOWTH,     BERRY
	db 28, MEOWTH,     BERRY
	db $ff ; end

	; POKEFANM (9)
	db "COLIN@", TRAINERTYPE_ITEM
	db 32, DELIBIRD,   BERRY
	db $ff ; end

	; POKEFANM (10)
	db "DEREK@", TRAINERTYPE_ITEM
	db 19, PIKACHU,    BERRY
	db $ff ; end

	; POKEFANM (11)
	db "DEREK@", TRAINERTYPE_ITEM
	db 36, PIKACHU,    BERRY
	db $ff ; end

	; POKEFANM (12)
	db "ALEX@", TRAINERTYPE_ITEM
	db 29, NIDOKING,   BERRY
	db 29, SLOWKING,   BERRY
	db 29, SEAKING,    BERRY
	db $ff ; end

	; POKEFANM (13)
	db "REX@", TRAINERTYPE_ITEM
	db 35, PHANPY,     BERRY
	db $ff ; end

	; POKEFANM (14)
	db "ALLAN@", TRAINERTYPE_ITEM
	db 35, TEDDIURSA,  BERRY
	db $ff ; end


KimonoGirlGroup:
	; KIMONO_GIRL (1)
	db "NAOKO@", TRAINERTYPE_NORMAL
	db 20, SKIPLOOM
	db 20, VULPIX
	db 18, SKIPLOOM
	db $ff ; end

	; KIMONO_GIRL (2)
	db "NAOKO@", TRAINERTYPE_NORMAL
	db 17, FLAREON
	db $ff ; end

	; KIMONO_GIRL (3)
	db "SAYO@", TRAINERTYPE_NORMAL
	db 17, ESPEON
	db $ff ; end

	; KIMONO_GIRL (4)
	db "ZUKI@", TRAINERTYPE_NORMAL
	db 17, UMBREON
	db $ff ; end

	; KIMONO_GIRL (5)
	db "KUNI@", TRAINERTYPE_NORMAL
	db 17, VAPOREON
	db $ff ; end

	; KIMONO_GIRL (6)
	db "MIKI@", TRAINERTYPE_NORMAL
	db 17, JOLTEON
	db $ff ; end


TwinsGroup:
	; TWINS (1)
	db "AMY & MAY@", TRAINERTYPE_NORMAL
	db 10, SPINARAK
	db 10, LEDYBA
	db $ff ; end

	; TWINS (2)
	db "ANN & ANNE@", TRAINERTYPE_MOVES
	db 16, CLEFAIRY,   GROWL, ENCORE, DOUBLESLAP, METRONOME
	db 16, JIGGLYPUFF, SING, DEFENSE_CURL, POUND, DISABLE
	db $ff ; end

	; TWINS (3)
	db "ANN & ANNE@", TRAINERTYPE_MOVES
	db 16, JIGGLYPUFF, SING, DEFENSE_CURL, POUND, DISABLE
	db 16, CLEFAIRY,   GROWL, ENCORE, DOUBLESLAP, METRONOME
	db $ff ; end

	; TWINS (4)
	db "AMY & MAY@", TRAINERTYPE_NORMAL
	db 10, LEDYBA
	db 10, SPINARAK
	db $ff ; end

	; TWINS (5)
	db "JO & ZOE@", TRAINERTYPE_NORMAL
	db 35, VICTREEBEL
	db 35, VILEPLUME
	db $ff ; end

	; TWINS (6)
	db "JO & ZOE@", TRAINERTYPE_NORMAL
	db 35, VILEPLUME
	db 35, VICTREEBEL
	db $ff ; end

	; TWINS (7)
	db "MEG & PEG@", TRAINERTYPE_NORMAL
	db 31, TEDDIURSA
	db 31, PHANPY
	db $ff ; end

	; TWINS (8)
	db "MEG & PEG@", TRAINERTYPE_NORMAL
	db 31, PHANPY
	db 31, TEDDIURSA
	db $ff ; end

	; TWINS (9)
	db "LEA & PIA@", TRAINERTYPE_MOVES
	db 35, DRATINI,    THUNDER_WAVE, TWISTER, FLAMETHROWER, HEADBUTT
	db 35, DRATINI,    THUNDER_WAVE, TWISTER, ICE_BEAM, HEADBUTT
	db $ff ; end

	; TWINS (10)
	db "LEA & PIA@", TRAINERTYPE_MOVES
	db 38, DRATINI,    THUNDER_WAVE, TWISTER, ICE_BEAM, HEADBUTT
	db 38, DRATINI,    THUNDER_WAVE, TWISTER, FLAMETHROWER, HEADBUTT
	db $ff ; end


PokefanFGroup:
	; POKEFANF (1)
	db "BEVERLY@", TRAINERTYPE_ITEM
	db 14, SNUBBULL,   BERRY
	db $ff ; end

	; POKEFANF (2)
	db "RUTH@", TRAINERTYPE_ITEM
	db 17, PIKACHU,    BERRY
	db $ff ; end

	; POKEFANF (3)
	db "BEVERLY@", TRAINERTYPE_ITEM
	db 18, SNUBBULL,   BERRY
	db $ff ; end

	; POKEFANF (4)
	db "BEVERLY@", TRAINERTYPE_ITEM
	db 30, GRANBULL,   BERRY
	db $ff ; end

	; POKEFANF (5)
	db "GEORGIA@", TRAINERTYPE_ITEM
	db 23, SENTRET,    BERRY
	db 23, SENTRET,    BERRY
	db 23, SENTRET,    BERRY
	db 28, FURRET,     BERRY
	db 23, SENTRET,    BERRY
	db $ff ; end

	; POKEFANF (6)
	db "JAIME@", TRAINERTYPE_ITEM
	db 16, MEOWTH,     BERRY
	db $ff ; end


RedGroup:
	; RED (1)
	db "RED@", TRAINERTYPE_MOVES
	db 81, PIKACHU,    CHARM, QUICK_ATTACK, THUNDERBOLT, THUNDER
	db 73, ESPEON,     MUD_SLAP, REFLECT, SWIFT, PSYCHIC_M
	db 75, SNORLAX,    AMNESIA, SNORE, REST, BODY_SLAM
	db 77, VENUSAUR,   SUNNY_DAY, GIGA_DRAIN, SYNTHESIS, SOLARBEAM
	db 77, CHARIZARD,  FLAMETHROWER, WING_ATTACK, SLASH, FIRE_SPIN
	db 77, BLASTOISE,  RAIN_DANCE, SURF, BLIZZARD, WHIRLPOOL
	db $ff ; end


BlueGroup:
	; BLUE (1)
	db "BLUE@", TRAINERTYPE_MOVES
	db 56, PIDGEOT,    QUICK_ATTACK, WHIRLWIND, WING_ATTACK, MIRROR_MOVE
	db 54, ALAKAZAM,   DISABLE, RECOVER, PSYCHIC_M, REFLECT
	db 56, RHYDON,     FURY_ATTACK, SANDSTORM, ROCK_SLIDE, EARTHQUAKE
	db 58, GYARADOS,   TWISTER, HYDRO_PUMP, RAIN_DANCE, HYPER_BEAM
	db 58, EXEGGUTOR,  SUNNY_DAY, LEECH_SEED, EGG_BOMB, SOLARBEAM
	db 58, ARCANINE,   ROAR, SWIFT, FLAMETHROWER, EXTREMESPEED
	db $ff ; end


OfficerGroup:
	; OFFICER (1)
	db "KEITH@", TRAINERTYPE_NORMAL
	db 17, GROWLITHE
	db $ff ; end

	; OFFICER (2)
	db "DIRK@", TRAINERTYPE_NORMAL
	db 14, GROWLITHE
	db 14, GROWLITHE
	db $ff ; end


GruntFGroup:
	; GRUNTF (2)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 26, ARBOK
	db $ff ; end

	; GRUNTF (3)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 25, GLOOM
	db 25, GLOOM
	db $ff ; end

	; GRUNTF (4)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 21, EKANS
	db 23, ODDISH
	db 21, EKANS
	db 24, GLOOM
	db $ff ; end

	; GRUNTF (5)
	db "GRUNT@", TRAINERTYPE_MOVES
	db 18, EKANS,      WRAP, LEER, POISON_STING, BITE
	db 18, GLOOM,      ABSORB, SWEET_SCENT, STUN_SPORE, SLEEP_POWDER
	db $ff ; end


MysticalmanGroup:
	; MYSTICALMAN (1)
	db "EUSINE@", TRAINERTYPE_MOVES
	db 23, DROWZEE,    DREAM_EATER, HYPNOSIS, DISABLE, CONFUSION
	db 23, HAUNTER,    LICK, HYPNOSIS, MEAN_LOOK, CURSE
	db 25, ELECTRODE,  SCREECH, SONICBOOM, THUNDER, ROLLOUT
	db $ff ; end
